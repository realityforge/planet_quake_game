...
/*
===========================
SHUD : CG_LogStats_Obituary
===========================
*/
static void CG_LogStats_Obituary( entityState_t *ent ) {
	int			mod;
	int			target, attacker;
	...
	if ( target < 0 || target >= MAX_CLIENTS ) {
		CG_Error( "CG_LogStats: target out of range" );
  		return;
	}
   stats[target].killed++;
   if((mod==MOD_SUICIDE)|(mod==MOD_FALLING)|(mod==MOD_CRUSH)|(mod==MOD_WATER)|
     (mod==MOD_SLIME)|(mod==MOD_LAVA)|(mod==MOD_TARGET_LASER)|(mod==MOD_TRIGGER_HURT))
   {
      stats[target].frags[target]++;
      return;
   };
	if (attacker < 0 || attacker >= MAX_CLIENTS ) return;
   if(target!=attacker)
   {
   	stats[attacker].kills++;
      if(mod==MOD_GAUNTLET) stats[attacker].weapon[0]++;
      if(mod==MOD_MACHINEGUN) stats[attacker].weapon[1]++;
      if(mod==MOD_SHOTGUN) stats[attacker].weapon[2]++;
      if((mod==MOD_GRENADE)|(mod==MOD_GRENADE_SPLASH)) stats[attacker].weapon[3]++;
      if((mod==MOD_ROCKET)|(mod==MOD_ROCKET_SPLASH)) stats[attacker].weapon[4]++;
      if(mod==MOD_LIGHTNING) stats[attacker].weapon[5]++;
      if(mod==MOD_RAILGUN) stats[attacker].weapon[6]++;
      if((mod==MOD_PLASMA)|(mod==MOD_PLASMA_SPLASH)) stats[attacker].weapon[7]++;
      if((mod==MOD_BFG)|(mod==MOD_BFG_SPLASH)) stats[attacker].weapon[8]++;
      if(cg.time<stats[attacker].last_frag_time+3000)
         stats[attacker].excellent++;
      stats[attacker].last_frag_time = cg.time;
   };
   stats[attacker].frags[target]++;
}
...
static void CG_ItemPickup( int itemNum ) {
	...
	if ( bg_itemlist[itemNum].giType == IT_WEAPON ) {
 	...
 	// SHUD : count armor picked up
	if ( bg_itemlist[itemNum].giType == IT_ARMOR ) {
		armor_picked_up +=bg_itemlist[itemNum].quantity; };
}
	...
	case EV_GLOBAL_ITEM_PICKUP:
		DEBUGNAME("EV_GLOBAL_ITEM_PICKUP");
		{
			...
			if ( es->number == cg.snap->ps.clientNum ) {
   			...
   			// SHUD
            	if (strcmp(item->classname,"team_CTF_redflag")==0) teamblue_getflag++;
            	if (strcmp(item->classname,"team_CTF_blueflag")==0) teamred_getflag++;
 		}
		break;
