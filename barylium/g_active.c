...
void ClientTimerActions( gentity_t *ent, int msec ) {
	...
	while ( client->timeResidual >= 1000 ) {
		...
		//Now do health/ammo/armor regen added adamw
		if(be_regenhealthtime.integer && (ent->seccounta>=be_regenhealthtime.integer)) {
			ent->seccounta = 0;
			if(ent->health!=be_maxstats.value)
				ent->health += be_regenhealth.integer;
			if(ent->health>be_maxstats.value)
				ent->health = be_maxstats.value;
		}
		if(be_regenarmortime.integer && (ent->seccountb>=be_regenarmortime.integer)) {
			ent->seccountb = 0;
			if(ent->client->ps.stats[STAT_ARMOR]!=be_maxstats.value)
				ent->client->ps.stats[STAT_ARMOR] += be_regenarmor.integer;
			if(ent->client->ps.stats[STAT_ARMOR]>be_maxstats.value)
				ent->client->ps.stats[STAT_ARMOR] = be_maxstats.value;
		}
		if(be_regenammotime.integer && (ent->seccountc>=be_regenammotime.integer)) {
			ent->seccountc = 0;
			if(ent->client->ps.ammo[ent->client->ps.weapon]!=be_maxammo.value)
				ent->client->ps.ammo[ent->client->ps.weapon] += be_regenammo.integer;
			if(ent->client->ps.ammo[ent->client->ps.weapon] > be_maxammo.value)
				ent->client->ps.ammo[ent->client->ps.weapon] = be_maxammo.value;
		}
		//runes
		if(ClientHasRune(ent, RUNE_REGENERATION)) {
			if(ent->client->ps.stats[STAT_ARMOR]<be_maxstats.value) {
				ent->client->ps.stats[STAT_ARMOR] += be_rregeninc.integer;
				if(ent->client->ps.stats[STAT_ARMOR]>be_maxstats.value)
					ent->client->ps.stats[STAT_ARMOR] = be_maxstats.value;
			}
			if(ent->health<be_maxstats.value) {
				ent->health += be_rregeninc.integer;
				if(ent->health>be_maxstats.value)
					ent->health = be_maxstats.value;
			}
		}
...
void ClientThink_real( gentity_t *ent ) {
	...
	//added adamw
	if(!be_campprotect.value || (level.time%500!=0)
	   || ent->client->sess.sessionTeam==TEAM_SPECTATOR
	   || ent->client->ps.pm_type==PM_DEAD
	   || ent->client->ps.pm_type==PM_INTERMISSION
	   || ent->lastcheck==level.time) goto end;
	ent->lastcheck = level.time;
	if(be_campshuffle.value < 1)
		be_campshuffle.value = 1;
	if(VectorCompareShuffle(ent->lastlocale, ent->r.currentOrigin, (float)be_campshuffle.value)) {
		//They are STILL here
		...
		if(timeatlocale%1000==0) {
			...
			if(totaltime==be_camptimeout.value-5) {
				strcpy(tempmsg, va(be_campwarnmessage.string, ent->client->pers.netname));
				trap_SendServerCommand(-1, va("print \"%s\n\"", tempmsg));
			}
			if(totaltime==be_camptimeout.value) {
				strcpy(tempmsg, va(be_campdeathmessage.string, ent->client->pers.netname));
				trap_SendServerCommand(-1, va("print \"%s\n\"", tempmsg));
				//Nuke that bitch!
...
//		G_Printf("serverTime >>>>>\n" );
	...
	//added adamw - lets alter the speed
	//first, do the normal speed modifier
	client->ps.speed *= be_playerspeed.value;
	//now see what weapon is up, and perform the weight adjustment

	switch(client->ps.weapon) {
		case WP_GAUNTLET:
			client->ps.speed *= be_gauntweight.value;
			break;
		case WP_LIGHTNING:
			client->ps.speed *= be_lgweight.value;
			break;
		case WP_SHOTGUN:
			client->ps.speed *= be_sgweight.value;
			break;
		case WP_MACHINEGUN:
			client->ps.speed *= be_mgweight.value;
			break;
		case WP_GRENADE_LAUNCHER:
			client->ps.speed *= be_glweight.value;
			break;
		case WP_ROCKET_LAUNCHER:
			client->ps.speed *= be_rlweight.value;
			break;
		case WP_PLASMAGUN:
			client->ps.speed *= be_pgweight.value;
			break;
		case WP_RAILGUN:
			client->ps.speed *= be_rgweight.value;
			break;
		case WP_BFG:
			client->ps.speed *= be_bfgweight.value;
			break;
		default:
			break;
	};

	//The haste rune mod goes in here
	if(ClientHasRune(ent, RUNE_HASTE)) {
		client->ps.speed *= be_rhastemod.value;
	}
	//end added
	...
	//changed adamw - we dont want this is we have offhand
	...
	if ((pm.cmd.buttons & 32)  &&
		ent->client->ps.pm_type != PM_DEAD &&
		!ent->client->hookfired &&
		be_offhandhook.value)	{
		Weapon_GrapplingHook_Fire( ent );
		ent->client->hookfired = qtrue;
	}
	if (!(pm.cmd.buttons & 32)  &&
		ent->client->ps.pm_type != PM_DEAD &&
		ent->client->hookfired &&
		...
		ent->client->hookfired = qfalse;
	}
	if (client->hook && client->fireHeld == qfalse && be_offhandhook.value)
		Weapon_HookFree(client->hook);
