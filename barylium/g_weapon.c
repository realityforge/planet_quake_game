...
qboolean CheckGauntletAttack( gentity_t *ent ) {
	...
	if(be_usecustomdamage.value)
		damage = be_damagegauntlet.value * s_quadFactor;
	else
...
void Bullet_Fire (gentity_t *ent, float spread, int damage ) {
	...
	// Vlad - Overwrite damage with be_damagemgun
	damage = be_damagemgun.value;
	damage *= s_quadFactor;
...
qboolean ShotgunPellet( vec3_t start, vec3_t end, gentity_t *ent ) {
	...
	for (i = 0; i < 10; i++) {
		...
		if ( traceEnt->takedamage) {
			//changed adamw
			if(be_usecustomdamage.value)
				damage = be_damagesgun.value * s_quadFactor;
			else
...
void weapon_railgun_fire (gentity_t *ent) {
	...
	//changed adamw
	if(be_usecustomdamage.value)
		damage = be_damagergun.value * s_quadFactor;
	else
...
void Weapon_LightningFire( gentity_t *ent ) {
	...
	//changed adamw
	if(be_usecustomdamage.value)
		damage = be_damagelgun.value * s_quadFactor;
	else
