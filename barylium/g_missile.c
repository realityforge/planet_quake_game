		...
		if ( tr.surfaceFlags & SURF_NOIMPACT ) {
			...
			if (ent->parent && ent->parent->client->hook == ent) {
				if(!be_offhandhook.value) {
					...
					ent->parent->client->hook = NULL;
					ent->parent->client->hookfired = qfalse;
...
gentity_t *fire_plasma (gentity_t *self, vec3_t start, vec3_t dir) {
	...
	bolt->parent = self;
	//changed adamw
	if(be_usecustomdamage.value) {
		bolt->damage = be_damagepgun.value;
		bolt->splashDamage = be_damagepgunsplash.value;
		bolt->splashRadius = be_splashrplasma.value;
	} else {
		bolt->damage = 20;
	...
	VectorScale( dir, be_plasmaspeed.value, bolt->s.pos.trDelta ); //chagned Vlad
...
gentity_t *fire_grenade (gentity_t *self, vec3_t start, vec3_t dir) {
	...
	bolt->nextthink = level.time + be_grenadelife.value; //changed Vlad
	bolt->think = G_ExplodeMissile;
	bolt->s.eType = ET_MISSILE;
	bolt->r.svFlags = SVF_USE_CURRENT_ORIGIN;
	...
	//changed adamw
	if(be_usecustomdamage.value) {
		bolt->damage = be_damagegrenade.value;
		bolt->splashDamage = be_damagegrenadesplash.value;
		bolt->splashRadius = be_splashrgrenade.value;
	} else {
		bolt->damage = 100;
	...
	bolt->s.pos.trTime = level.time - MISSILE_PRESTEP_TIME;		// move a bit on the very first frame
	VectorCopy( start, bolt->s.pos.trBase );
	VectorScale( dir, be_grenadespeed.value, bolt->s.pos.trDelta ); //changed Vlad
	SnapVector( bolt->s.pos.trDelta );			// save net bandwidth
...
gentity_t *fire_bfg (gentity_t *self, vec3_t start, vec3_t dir) {
	...
	//changed adamw
	if(be_usecustomdamage.value) {
		bolt->damage = be_damagebfg.value;
		bolt->splashDamage = be_damagebfgsplash.value;
		bolt->splashRadius = be_splashrbfg.value;
	} else {
		bolt->damage = 100;
		bolt->splashDamage = 100;
	...
	bolt->s.pos.trTime = level.time - MISSILE_PRESTEP_TIME;		// move a bit on the very first frame
	VectorCopy( start, bolt->s.pos.trBase );
	VectorScale( dir, be_bfgspeed.value, bolt->s.pos.trDelta ); //changed Vlad
	SnapVector( bolt->s.pos.trDelta );			// save net bandwidth
	VectorCopy (start, bolt->r.currentOrigin);
...
gentity_t *fire_rocket (gentity_t *self, vec3_t start, vec3_t dir) {
	...
	//changed adamw
	if(be_usecustomdamage.value) {
		bolt->damage = be_damagerocket.value;
		bolt->splashDamage = be_damagerocketsplash.value;
		bolt->splashRadius = be_splashrrocket.value;
	} else {
		bolt->damage = 100;
		bolt->splashDamage = 100;
		bolt->splashRadius = 120;
	...
	bolt->s.pos.trTime = level.time - MISSILE_PRESTEP_TIME;		// move a bit on the very first frame
	VectorCopy( start, bolt->s.pos.trBase );
	VectorScale( dir, be_rocketspeed.value, bolt->s.pos.trDelta ); //changed chrisp
	SnapVector( bolt->s.pos.trDelta );			// save net bandwidth
	VectorCopy (start, bolt->r.currentOrigin);
