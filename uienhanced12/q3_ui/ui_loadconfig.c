...
static void LoadConfig_MenuEvent( void *ptr, int event ) {
	...
	case ID_GO:
		...
		if (s_configs.load) {
			COM_StripExtension(s_configs.configlist[s_configs.list.curvalue], s_configs.configname);
		}
		else {
			COM_StripExtension(s_configs.savename.field.buffer, s_configs.configname);
		}
		if (s_configs.configname[0])
