...
static void PlayerModel_BuildList( void )
{
	...
	for (i=0; i<numdirs && s_playermodel.nummodels < MAX_PLAYERMODELS; i++,dirptr+=dirlen+1)
	{
		...
		if (defaultskin) {
			COM_StripExtension(defaultskin,skinname);
...
=================
*/
static void PlayerModel_SwapOtherHeads( qboolean modelchange )
{
...
static void PlayerModel_MenuEvent( void* ptr, int event )
{
	...
	switch (((menucommon_s*)ptr)->id)
	{
		...
		case ID_TEAMMODEL:
			PlayerModel_SwapOtherHeads(qtrue);
		...
		case ID_FORCEHEADMATCH:
			PlayerModel_SwapOtherHeads(qfalse);
