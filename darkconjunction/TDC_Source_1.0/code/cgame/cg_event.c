...
static void CG_ItemPickup( int itemNum ) {
	...
	if ( bg_itemlist[itemNum].giType == IT_WEAPON ) {
		...
		if ( cg_autoswitch.integer /*&& bg_itemlist[itemNum].giTag != WP_MG42 */) {
			cg.weaponPresentSwitch = qtrue; //EHL - teleport effects on weapon selection disabled
...
//		trap_S_StartSound (NULL, es->number, CHAN_AUTO, cgs.media.noAmmoSound );
	...
	case EV_FIRE_WEAPON_BONE:
		DEBUGNAME("EV_FIRE_WEAPON_BONE");
		break;
	...
	// JPL, EHL
	case EV_EARTHQUAKE:
//		CG_Printf("%d ",es->eventParm & 0xFF);
		if ((es->eventParm & 0xFF) == 0xFF)
		{
			CG_StartEarthquake(5,HULK_QUAKE_LEN);			// HULK attack - JPL
			trap_S_StartSound(NULL,0,CHAN_AUTO,cgs.media.hulkQuakeSound);
		}
		else if ((es->eventParm & 0xFF) == 0xFE)
		{
			CG_StartEarthquake(4,100);			// MG42 earthquake - JPL
		}
		else if ((es->eventParm & 0xFF) == 0xFE)		// ??? seems a bug... 
		{
			CG_StartEarthquake(3,500);			// Sealord earthquake - JPL
		}
		else
...
//		CG_Printf("client stop begin cg.time = %d\nparam= %d   p_stop= %d\n",cg.time,(es->eventParm&0xFF),player_stop);
		break;
