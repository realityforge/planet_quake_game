...
//
// modified by JPL, EHL - ACE TEAM
// used code from codearena tutorials for rotating doors
...
#endif
		// push items, players, NPCs and AnimMD3s -- JPL
		if ( check->s.eType != ET_ITEM && check->s.eType != ET_NPC && check->s.eType!=ET_MODELANIM && 
...
void SetMoverState( gentity_t *ent, moverState_t moverState, int time ) {
	...
	ent->s.apos.trTime = time;	// JPL - rotating doors
	switch( moverState ) {
	case MOVER_POS1:
...
// JPL - end
	...
	BG_EvaluateTrajectory( &ent->s.apos, level.time, ent->r.currentAngles );	// JPL - rotating doors
	trap_LinkEntity( ent );
}
...
void Blocked_Door( gentity_t *ent, gentity_t *other ) {
	...
	if ( !other->client ) {
		...
		if (other->s.eType==ET_NPC)		// DO NOT remove NPCs!! - JPL
			return;
	...
	// reverse direction
//	Use_BinaryMover( ent, ent, other );	// DO not REVERSE if blocked
}
...
void Touch_DoorTrigger( gentity_t *ent, gentity_t *other, trace_t *trace ) {
	if ( other->client && other->client->sess.sessionTeam == TEAM_SPECTATOR ) {
		...
		ent->parent->moverState != ROTATOR_1TO2 ) {		// JPL - rotating doors
...
"sound1to2"	sound position 1 to 2
"sound2to1"	sound position 2 to 1
"soundPos1"	sound position 1
"soundPos2"	sound position 2
...
void SP_func_door (gentity_t *ent) {
	...
	G_SpawnString("sound1to2", "", &sound1to2);
	G_SpawnString("sound2to1", "", &sound2to1);
	G_SpawnString("soundPos1", "", &soundPos1);
	G_SpawnString("soundPos2", "", &soundPos1);

	ent->sound1to2 = G_SoundIndex(sound1to2);
	ent->sound2to1 = G_SoundIndex(sound2to1);
	ent->soundPos1 = G_SoundIndex(soundPos1);
	ent->soundPos2 = G_SoundIndex(soundPos2);
	//EHL-end
...
// JPL - triggered trains

void ActivateTrain( gentity_t *ent, gentity_t *other, gentity_t *activator ) {

	ent->use=NULL;//Use_BinaryMover;
...
"wait" seconds to wait before begining move to next corner (-1 = stop, -2 = wait trigger, -3 = wait trigger only first time)
*/
