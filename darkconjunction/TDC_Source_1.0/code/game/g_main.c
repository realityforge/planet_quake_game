...
// JPL: init AAS
	npc_skill = trap_Cvar_VariableValue( "g_spSkill" ) - 2; // 12 = ID_MEDIUM --> medium = 0, easy = -1, hard = 1
...
int start, end;
	...
	for (i=0 ; i<level.num_entities ; i++, ent++) {
		...
		if (ent->s.eType == ET_NPC) {		// JPL
			G_RunNPC(ent);
			continue;
		}

		if (ent->s.eType == ET_MODELANIM) {		// JPL
			G_RunMD3Anim(ent);
			continue;
		}
