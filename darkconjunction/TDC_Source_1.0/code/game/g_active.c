...
// EHL Damage due to fall. Set in function PM_CrashLand at file bg_pmove.c (game)
extern int fall_damage;
...
void ClientTimerActions( gentity_t *ent, int msec ) {
    ...
    for (i = 0; i < weapCount; i++) {
		  ...
		  switch(w) {
			  ...
			  case WP_TRISHOTGUN: max = 10; inc = 1; t = 1500; break;
			  ...
			  case WP_GUN: max = 50; inc = 5; t = 1500; break;
...
//	qboolean	fired;
	...
	for ( i = oldEventSequence ; i < client->ps.eventSequence ; i++ ) {
		...
		case EV_FALL_FAR:
			...
			G_Damage (ent, NULL, NULL, NULL, NULL, damage + fall_damage, 0, MOD_FALLING); // EHL - added '+ fall_damage'
			fall_damage = 0;
...
// JPL - death means level restart				
				char stmap[100];
				char buf[100];
				trap_Cvar_VariableStringBuffer( "mapname", stmap, sizeof(stmap) );

				Com_sprintf(buf,99,"map %s",stmap);
