...
static void PlayerSettings_DrawPlayer( void *self ) {
	...
	if ( strcmp( buf, s_playersettings.playerModel ) != 0 ) {
		...
		viewangles[PITCH] = 0;
		viewangles[ROLL]  = 0;
		UI_PlayerInfo_SetInfo( &s_playersettings.playerinfo, LEGS_IDLE, TORSO_STAND, viewangles, vec3_origin, WP_MG42, qfalse );
	}
...
static void PlayerSettings_MenuInit( void ) {
	...
	s_playersettings.back.generic.x				= 32;
	s_playersettings.back.generic.y				= 480-36;
	s_playersettings.back.string				= "back";
	s_playersettings.back.color					= tdc_text_color;
