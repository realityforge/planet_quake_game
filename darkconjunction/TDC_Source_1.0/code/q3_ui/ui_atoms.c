...
//	Demos_Cache();UI_CinematicsMenu_Cache();ServerInfo_Cache();SpecifyServer_Cache();ArenaServers_Cache();StartServer_Cache();ServerOptions_Cache();UI_NetworkOptionsMenu_Cache();TeamMain_Cache();UI_AddBots_Cache();UI_RemoveBots_Cache();UI_LoadConfig_Cache();UI_SaveConfigMenu_Cache();UI_BotSelectMenu_Cache();UI_ModsMenu_Cache();
}
...
void UI_Refresh( int realtime )
{
	...
	// caching images
	if (m_entersound )//ANITA
	{  if (m_specialsound )
			{
			trap_S_StartLocalSound( menu_special_sound ,CHAN_LOCAL_SOUND );
			m_specialsound = qfalse;
			m_entersound = qfalse;
			}	
		else 
			{
