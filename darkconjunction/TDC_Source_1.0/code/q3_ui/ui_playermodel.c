...
static void PlayerModel_UpdateModel( void )
{
	...
	UI_PlayerInfo_SetInfo( &s_playermodel.playerinfo, LEGS_IDLE, TORSO_STAND, viewangles, moveangles, WP_TRISHOTGUN, qfalse );
}
...
static void PlayerModel_DrawPlayer( void *self )
{
	...
	if( trap_MemoryRemaining() <= LOW_MEMORY ) {
		UI_DrawProportionalString( b->generic.x, b->generic.y + b->height / 2, "LOW MEMORY", UI_LEFT, tdc_text_color );
		return;
	}
...
static void PlayerModel_MenuInit( void )
{
	...
	s_playermodel.back.generic.x		= 12;
	s_playermodel.back.generic.y		= 480-PROP_HEIGHT;
	s_playermodel.back.string  		    = "back";
	s_playermodel.back.color  		    = color_red;
	s_playermodel.back.style         = UI_LEFT;
