...
sfxHandle_t menu_special_sound;
...
vec4_t menu_text_color	    = {0.8, 0.8, 0.8, 1.0};
vec4_t menu_dim_color       = {0.8, 0.8, 0.8, 0.75};
vec4_t color_black	        = {0.00, 0.00, 0.00, 1.00};
vec4_t color_white	        = {1.00, 1.00, 1.00, 1.00};
//CAMBIADOvec4_t color_yellow	        = {1.00, 1.00, 0.00, 1.00};
vec4_t color_yellow	        = {1.00, 1.00, 1.00, 1.00};
vec4_t color_blue	        = {0.00, 0.00, 1.00, 1.00};
vec4_t color_lightOrange    = {0.50, 0.00, 0.00, 1.00 };
vec4_t color_orange	        = {0.50, 0.00, 0.00, 1.00};
vec4_t color_red		    = {0.80, 0.80, 0.80, 1.00};
vec4_t color_dim		    = {0.80, 0.80, 0.80, 0.25};

// current color scheme
vec4_t pulse_color          = {1.00, 1.00, 1.00, 1.00};
vec4_t text_color_disabled  = {0.50, 0.50, 0.50, 1.00};	// light gray
vec4_t text_color_normal	= {0.80, 0.80, 0.80, 1.00};	// dark red
vec4_t text_color_highlight = {1.00, 1.00, 1.00, 1.00};	// red
vec4_t listbar_color        = {0.0, 0.00, 0.00, 0.01};	// transluscent very dark red
...
vec4_t color_white	        = {1.00, 1.00, 1.00, 1.00};
//CAMBIADOvec4_t color_yellow	        = {1.00, 1.00, 0.00, 1.00};
vec4_t color_yellow	        = {0.80, 0.80, 0.80, 1.00};
vec4_t color_blue	        = {0.00, 0.00, 1.00, 1.00};
vec4_t color_lightOrange    = {0.50, 0.00, 0.00, 1.00 };
vec4_t color_orange	        = {0.50, 0.00, 0.00, 1.00};
vec4_t color_red		    = {0.50, 0.00, 0.00, 1.00};
vec4_t color_dim		    = {0.00, 0.00, 0.00, 0.25};

// current color scheme
vec4_t pulse_color          = {1.00, 1.00, 1.00, 1.00};
vec4_t text_color_disabled  = {0.50, 0.50, 0.50, 1.00};	// light gray
vec4_t text_color_normal	= {0.50, 0.00, 0.00, 1.00};	// dark red
vec4_t text_color_highlight = {1.00, 0.00, 0.00, 1.00};	// red
vec4_t listbar_color        = {0.25, 0.00, 0.00, 0.30};	// transluscent very dark red
vec4_t text_color_status    = {1.00, 1.00, 1.00, 1.00};	// bright white	
*/
...
vec4_t menu_dim_color       = {0.0, 0.0, 0.0, 0.75};
vec4_t color_black	        = {0.00, 0.00, 0.00, 1.00};
vec4_t color_white	        = {0.80, 0.80, 0.80, 1.00};
...
// current color scheme
vec4_t pulse_color          = {1.00, 1.00, 1.00, 1.00};
vec4_t text_color_disabled  = {0.25, 0.25, 0.25, 1.00};	// light gray
vec4_t text_color_normal	= {0.60, 0.60, 0.60, 1.00};	// light orange
...
//ANITA
//tdc colors
vec4_t		tdc_text_color		= {0.60, 0.60, 0.60, 1.00};	//dark red //{0.80, 0.80, 0.80, 1.00}; //gris
vec4_t		tdc_highlight_color = {1.00, 1.00, 1.00, 1.00}; //red
vec4_t		tdc_listbar_color	= {0.25, 0.25, 0.25, 0.30}; //transluscent dark red
vec4_t		tdc_color_disabled  = {0.80, 0.80, 0.80, 1.00}; //gray
//vec4_t		tdc_dark_color		= 
//ANITA
...
static void Action_Draw( menuaction_s *a )
{
	...
	style = 0;
	color =	tdc_text_color;
	if ( a->generic.flags & QMF_GRAYED )
	{
		color = tdc_color_disabled; // color_red; //text_color_disabled;
	}
	else if (( a->generic.flags & QMF_PULSEIFFOCUS ) && ( a->generic.parent->cursor == a->generic.menuPosition ))
	{
		color = tdc_highlight_color;
		style = UI_PULSE;
	}
...
static void RadioButton_Draw( menuradiobutton_s *rb )
{
	...
	{
		// draw cursor
		UI_FillRect( rb->generic.left, rb->generic.top, rb->generic.right-rb->generic.left+1, rb->generic.bottom-rb->generic.top+1, tdc_listbar_color ); 
...
static void Slider_Draw( menuslider_s *s )
{
	...
	{
		// draw cursor
		UI_FillRect( s->generic.left, s->generic.top, s->generic.right-s->generic.left+1, s->generic.bottom-s->generic.top+1, tdc_listbar_color ); 
		UI_DrawChar( x, y, 13, UI_CENTER|UI_BLINK|UI_SMALLFONT, color);
	}
	...
	for( column = 0; column < l->columns; column++ ) {
		...
		for( i = base; i < base + l->height; i++) {
			...
			if (i == l->curvalue)
			{
				...
				UI_FillRect(u,y,l->width*SMALLCHAR_WIDTH,SMALLCHAR_HEIGHT+2,tdc_listbar_color);
				color = tdc_highlight_color;
...
void Menu_Cache( void )
{
	...
	uis.charsetPropGlow	= trap_R_RegisterShaderNoMip( "menu/art/font1_prop_glo.tga" );//"menu/art/font2_prop.tga" );
	uis.charsetPropB	= trap_R_RegisterShaderNoMip( "menu/art/font2_prop.tga" );//"menu/art/font2_prop.tga" );
	...
	menu_in_sound	= trap_S_RegisterSound( "sound/menu/menu_click.wav", qfalse );
	menu_special_sound = trap_S_RegisterSound( "sound/menu/menu_start_game.wav", qfalse );
	menu_move_sound	= -1;//Anita, para que no haga ruido //trap_S_RegisterSound( "sounds/menu/menu_mouse_over.wav", qfalse );
	menu_out_sound	= trap_S_RegisterSound( "sound/menu/menu_click.wav", qfalse );
	menu_buzz_sound	= trap_S_RegisterSound( "sound/menu/menu_click.wav", qfalse );
