...
#ifdef _DEBUG
	if(*(channel_t **) snd.freelist > &s_channels[MAX_CHANNELS] || *(channel_t **) snd.freelist < &s_channels[0])
	{							//DAJ    extra check
		Com_DPrintf("s_channel OUT OF BOUNDS\n");
		return NULL;
	}
...
//  dot = -vec[1];
	...
	if(dma.channels == 1 || noAttenuation)
...
void S_ThreadStartSoundEx(vec3_t origin, int entityNum, int entchannel, sfxHandle_t sfxHandle, int flags, int volume)
{
	...
	if(entityNum >= 0)
	{
		...
		{						// track 0 is music/cinematics
			...
			if((entchannel != CHAN_AUTO) && (streamingSounds[i].entnum >= 0) && (streamingSounds[i].channel == entchannel) &&
...
#ifdef _DEBUG
	if(ch > &s_channels[MAX_CHANNELS] || ch < &s_channels[0])
	{							//DAJ    extra check
		Com_DPrintf("s_channel OUT OF BOUNDS\n");
		Sys_LeaveCriticalSection(crit);
		return;
...
void S_AddLoopingSound(const vec3_t origin, const vec3_t velocity, const int range, sfxHandle_t sfxHandle, int volume,
					   int soundTime)
{
	sfx_t          *sfx;
	...
	snd.loopSounds[snd.numLoopSounds].startSample = sfx->soundLength
		? ((s_khz->integer * soundTime) - s_paintedtime) % sfx->soundLength : 0;

	snd.loopSounds[snd.numLoopSounds].startSample = sfx->soundLength
		? s_paintedtime - ((int)(dma.speed * (cl.serverTime - soundTime) / 1000.0f)) : 0;
...
==================
*/
void S_AddRealLoopingSound(const vec3_t origin, const vec3_t velocity, const int range, sfxHandle_t sfxHandle, int volume,
						   int soundTime)
{
	sfx_t          *sfx;
...
//      Com_DPrintf ("not started or muted\n");
	...
	for(i = 0; i < snd.numLoopSounds; i++)
	{
		...
		for(j = (i + 1); j < MAX_GENTITIES; j++)
		{
			...
			if(!loop2->active || loop2->doppler || loop2->sfx != loop->sfx || loop2->startSample != loop->startSample)
			{					// ydnar
				continue;
			}
...
*/
	...
	if(s_soundtime < snd.volTime2)
	{							// still has fading to do
		if(s_soundtime > snd.volTime1)
		{						// has started fading
...
//  char    name[MAX_QPATH];
	...
	if(entnum >= 0)
	{
		...
		{						// track 0 is music/cinematics
			...
			// check to see if this character currently has another sound streaming on the same channel
			if((channel != CHAN_AUTO) && (streamingSounds[i].entnum >= 0) && (streamingSounds[i].channel == channel) &&
...
//              memset( &s_rawsamples[i], 0, MAX_RAW_SAMPLES*sizeof(portable_samplepair_t) );
		...
		*rp = qfalse;
		...
		{						// RF, read a bit ahead of time to allow for talking animations
...
//      looped = qfalse;
		...
		while(*re < soundMixAheadTime + MAX_RAW_SAMPLES)
		{
			...
			if(s_mute->value)
			{					//----(SA)  sound is muted.  process to maintain timing, but play at 0 volume
				streamingVol = 0;
			}
			...
			// add to raw buffer
			S_RawSamples(fileSamples, ss->info.rate, ss->info.width, ss->info.channels, raw, lvol, rvol, i);
