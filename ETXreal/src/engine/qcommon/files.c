...
// XreaL END
					...
					if(!(pak->referenced & FS_CGAME_REF) &&
					   FS_ShiftedStrStr(filename, SYS_DLLNAME_CGAME, -SYS_DLLNAME_CGAME_SHIFT))
					{
		...
		else if(search->dir)
		{
			...
			if(fs_restrict->integer || fs_numServerPaks)
			{
				...
				if(Q_stricmp(filename + l - 4, ".cfg")	// for config files
				   ...
				   && Q_stricmp(filename + l - 8, "bots.txt") && Q_stricmp(filename + l - 8, ".botents")
#ifdef __MACOS__
			...
			if(Q_stricmp(filename + l - 4, ".cfg")	// for config files
			   ...
			   && Q_stricmp(filename + l - 5, ".game")	// menu files
			   && Q_stricmp(filename + l - strlen(demoExt), demoExt)	// menu files
			   && Q_stricmp(filename + l - 4, ".dat") && Q_stricmp(filename + l - 8, ".botents")
			   /*&& !strstr( filename, "botfiles" ) */ )
			{					// RF, need this for dev
...
//      }
	...
	// RB: moved this down so local files have a higher priority than files in .pk3 packages
			...
			else
			{
				...
				if(FS_SV_FileExists(va("%s.pk3", fs_serverReferencedPakNames[i])))
				{
					...
					if(CL_WWWBadChecksum(va("%s.pk3", fs_serverReferencedPakNames[i])))
					{
						...
						char           *rmv =
							FS_BuildOSPath(fs_homepath->string, va("%s.pk3", fs_serverReferencedPakNames[i]), "");
						rmv[strlen(rmv) - 1] = '\0';
						FS_Remove(rmv);
					}
#endif
...
#define SYS_PAKNAME_PAK0 "whr7"
			...
			if(FS_ShiftedStrStr(path->pack->pakBasename, SYS_PAKNAME_MPBIN, -SYS_PAKNAME_MPBIN_SHIFT))
			{
				...
				if((path->pack->checksum ^ 0x01042000u) != (DEMO_MPBIN_CHECKSUM ^ 0x01042000u))
				{
			...
			{
				// a tiny attempt to keep the checksum from being scannable from the exe
				if((path->pack->checksum ^ 0x04062001u) != (DEMO_PAK0_CHECKSUM ^ 0x04062001u))
				{
					Com_Error(ERR_FATAL, "Corrupted pakfile: %u", path->pack->checksum);
				}
			}
...
const char     *FS_LoadedPakPureChecksums(void)
{
	...
	for(search = fs_searchpaths; search; search = search->next)
	{
		...
		else
		{
			...
			Q_strcat(info, sizeof(info),
					 va("%s ",
						FS_ShiftStr(pak_purechecksums[lookup_randomized[feed_index]], -13 - lookup_randomized[feed_index])));
