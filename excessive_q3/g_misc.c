...
void TeleportPlayer( gentity_t *player, vec3_t origin, vec3_t angles ) {
	...
	//PANTS - disconnect this player's hook
	if(player->client && player->client->hook)
		Weapon_HookFree(player->client->hook);

	//PANTS - disconnects any hooks into this player
...
void Use_Shooter( gentity_t *ent, gentity_t *other, gentity_t *activator ) {
	...
	case WP_GRENADE_LAUNCHER:
		//PANTS - make it bounce
		fire_grenade( ent, ent->s.origin, dir, 2500, qtrue, qfalse);
		break;
