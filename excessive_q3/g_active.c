...
void ClientTimerActions( gentity_t *ent, int msec ) {
	...
	//PANTS - ammo regen
	times = ent->ammoRegenTime;
	ammo = ent->client->ps.ammo;
#ifdef MISSIONPACK
	powerup = (bg_itemlist[client->ps.stats[STAT_PERSISTANT_POWERUP]].giTag == PW_AMMOREGEN);
#endif
	for(i = WP_GAUNTLET ; i < WP_NUM_WEAPONS ; i++)
	{
		while((ammo[i] < weaponData[i].max) && (level.time >= times[i]))
		{
			times[i] += (weaponData[i].delay * AMMO_REGEN_SCALE);
#ifdef MISSIONPACK
			if(powerup)
			{
				ammo[i] += 3;
				if(ammo[i] > weaponData[i].max)
					ammo[i] = weaponData[i].max;
			}
			else
#endif
				ammo[i]++;
...
//		G_Printf("serverTime >>>>>\n" );
	...
	//PANTS - delay ammo regen when attacking
	if(ucmd->buttons & BUTTON_ATTACK)
		ent->ammoRegenTime[client->ps.weapon] = (level.time + (weaponData[client->ps.weapon].delay * AMMO_REGEN_SCALE));
