...
static void CL_MouseMove( usercmd_t* cmd )
{
	...
	{		
		...
		if (cl_mouseAccelLimit->value && (accelSensitivity > cl_mouseAccelLimit->value))
		accelSensitivity = cl_mouseAccelLimit->value;

		if ( cl_showMouseRate->integer )
		Com_Printf( "%f %f : %f\n", mx, my, accelSensitivity );
		}
		else
		{
			float rate = sqrt( mx * mx + my * my ) / (float) frame_msec;
