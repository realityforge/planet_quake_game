...
//	Swap_Init ();
  ...
  // get dedicated here for proper hunk megs initialization
	...
	//
	com_haxfps = Cvar_Get( "com_haxfps", "250", 0 ); // try to force that in some light way
	...
	com_introPlayed = Cvar_Get( "com_introplayed", "1", CVAR_ROM);

#ifndef DEDICATED
	com_skipIdLogo  = Cvar_Get( "com_skipIdLogo", "1", CVAR_ROM);
#endif
	...
	s = va("%s %s %s", "CNQ3 was cool ^zbut now ^8it's time for^0", Q3_VERSION, __DATE__ ); // PLATFORM_STRING
...
void Com_Frame( qboolean demoPlaying ) {
	...
	if ( com_dedicated->integer ) {
		...
		if ( !com_timedemo->integer || minMsec ) {
			...
			if ( com_haxfps->integer > 0 )
				minMsec = 1000 / com_haxfps->integer;
		}
	}
