...
FT_Bitmap* idRenderSystemFontLocal::RenderGlyph( FT_GlyphSlot glyph, glyphInfo_t* glyphOut )
{
    ...
    if ( glyph->format == ft_glyph_format_outline )
    {
        ...
        bit2 = ( FT_Bitmap* )clientMainSystem->RefMalloc( sizeof( FT_Bitmap ) );
        ...
        bit2->buffer = ( U8* )clientMainSystem->RefMalloc( pitch * height );
        bit2->num_grays = 256;
...
void idRenderSystemFontLocal::WriteTGA( UTF8* filename, U8* data, S32 width, S32 height )
{
    ...
    buffer = ( U8* )clientMainSystem->RefMalloc( width * height * 4 + 18 );
    ::memset( buffer, 0, 18 );
    // uncompressed type
    buffer[2] = 2;
    buffer[12] = width & 255;
    ...
    // flip upside down
    flip = ( U8* )clientMainSystem->RefMalloc( width * 4 );
...
void idRenderSystemLocal::RegisterFont( StringEntry fontName, S32 pointSize, fontInfo_t* font )
{
    ...
    out = ( U8* )clientMainSystem->RefMalloc( 256 * 256 );
    if ( out == NULL )
    {
        clientMainSystem->RefPrintf( PRINT_WARNING, "idRenderSystemLocal::RegisterFont: clientMainSystem->RefMalloc failure during output image creation.\n" );
        return;
    }
    ...
    while ( i <= GLYPH_END + 1 )
    {
        ...
        if ( xOut == -1 || yOut == -1 )
        {
            ...
            imageBuff = ( U8* )clientMainSystem->RefMalloc( newSize );
            left = 0;
            max = 0;
