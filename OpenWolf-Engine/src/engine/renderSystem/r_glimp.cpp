...
#define GLE(ret, name, ...) name##proc * qgl##name = &idRenderSystemDSALocal::name;
...
S32 idRenderSystemGlimpLocal::SetMode( S32 mode, bool fullscreen, bool noborder, bool fixedFunction )
{
    ...
    for ( i = 0; i < 16; i++ )
    {
        ...
        /* Create a Window */
        SDL_window = SDL_CreateWindow( CLIENT_WINDOW_TITLE, x, y, glConfig.vidWidth, glConfig.vidHeight, flags );
        if ( SDL_window == nullptr )
        {
            clientMainSystem->RefPrintf( PRINT_DEVELOPER, "idRenderSystemGlimpLocal::SetMode - SDL_CreateWindow failed: %s\n", SDL_GetError() );
            continue;
        }
