...
bool idRenderSystemModelLocal::LoadMD3( model_t* mod, S32 lod, void* buffer, S32 bufferSize, StringEntry modName )
{
    ...
    for ( i = 0; i < md3Model->numSurfaces; i++ )
    {
        ...
        // calc tangent spaces
        {
            vec3_t* sdirs = ( vec3_t* )clientMainSystem->RefMalloc( sizeof( *sdirs ) * surf->numVerts * mdvModel->numFrames );
            vec3_t* tdirs = ( vec3_t* )clientMainSystem->RefMalloc( sizeof( *tdirs ) * surf->numVerts * mdvModel->numFrames );
    ...
    
    {
        ...
        for ( i = 0; i < mdvModel->numSurfaces; i++, vaoSurf++, surf++ )
        {
            ...
            data = ( U8* )clientMainSystem->RefMalloc( dataSize );
            dataOfs = 0;
