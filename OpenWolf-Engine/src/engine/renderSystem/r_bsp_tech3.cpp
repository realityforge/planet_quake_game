...
void idRenderSystemBSPTechLocal::LoadLightmaps( lump_t* l, lump_t* surfs )
{
    ...
    image = ( U8* )clientMainSystem->RefMalloc( imageSize );
...
void idRenderSystemLocal::LoadWorld( StringEntry name )
{
    ...
    if ( 0 )
    {
        ...
        primaryLightGrid = ( U8* )clientMainSystem->RefMalloc( lightGridSize * sizeof( *primaryLightGrid ) );
        ...
        {
            S32 i;
            U8* buffer = ( U8* )clientMainSystem->RefMalloc( w->lightGridBounds[0] * w->lightGridBounds[1] * 3 + 18 );
            U8* out;
            U8* in;
            UTF8 fileName[MAX_QPATH];
