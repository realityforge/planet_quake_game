...
struct BufferedFile* idRenderSystemImagePNGLocal::ReadBufferedFile( StringEntry name )
{
    ...
    BF = ( struct BufferedFile* )clientMainSystem->RefMalloc( sizeof( struct BufferedFile ) );
    if ( !BF )
    {
        return( NULL );
    }
...
U32 idRenderSystemImagePNGLocal::DecompressIDATs( struct BufferedFile* BF, U8** Buffer )
{
    ...
    *Buffer = DecompressedData;
    ...
    CompressedData = ( U8* )clientMainSystem->RefMalloc( CompressedDataLength );
    if ( !CompressedData )
    {
        return( -1 );
    }
    ...
    DecompressedData = ( U8* )clientMainSystem->RefMalloc( puffDestLen );
    if ( !DecompressedData )
    {
        memorySystem->Free( CompressedData );
...
void idRenderSystemImagePNGLocal::LoadPNG( StringEntry name, U8** pic, S32* width, S32* height )
{
    ...
    *pic = NULL;
    ...
    OutBuffer = ( U8* )clientMainSystem->RefMalloc( IHDR_Width * IHDR_Height * Q3IMAGE_BYTESPERPIXEL );
    if ( !OutBuffer )
    {
        memorySystem->Free( DecompressedData );
        CloseBufferedFile( ThePNG );
