...
void idMemorySystemLocal::Free( void* ptr )
{
    ...
    {
        Com_Error( ERR_FATAL, "idMemorySystemLocal::Free memory block wrote past end" );
    }
...
{
    S32 zoneBytes, zoneBlocks, smallZoneBytes, smallZoneBlocks, rendererBytes, otherBytes;
    memblock_t*	block;
...
void idMemorySystemLocal::InitHunkMemory( void )
{
    ...
    // bk001205 - was malloc
    s_hunk.original = ( U8* )calloc( s_hunk.memSize + 63, 1 );
    if ( !s_hunk.original )
    {
    ...
    // cacheline align
    s_hunk.mem = ( U8* )( ( ( intptr_t )s_hunk.original + 63 ) & ~63 );
