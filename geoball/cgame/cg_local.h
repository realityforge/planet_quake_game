...
typedef struct {
	...
	float			yawAngle;		// Lanz: A test to get the ball to keep the angel when stopping
...
#define MAX_TIMER_CHECKPOINTS		32
...
typedef struct {
	...
	// Storage of the checkpoint times and scores for the race mode
	int			timerCurrentChecks[MAX_TIMER_CHECKPOINTS];
	int			timerCurrentCount;
	int			timerBestChecks[MAX_TIMER_CHECKPOINTS];
	int			timerBestCount;
	int			timerLast;
	int			timerBest;
...
typedef struct ownerDrawDef_s;

void CG_CenterPrint( const char *str, int y, int charWidth );
void CG_DrawActive( stereoFrame_t stereoView );
void CG_DrawTeamBackground( int x, int y, int w, int h, float alpha, int team );
void CG_OwnerDraw( struct ownerDrawDef_s *def );
