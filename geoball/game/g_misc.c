...
void SP_misc_model( gentity_t *ent ) {
	...
	ent->s.contents = CONTENTS_SOLID; // I added this line
	VectorSet (ent->mins, -4, -4, -4); //-16
	VectorSet (ent->maxs, 4, 4, 4); //16
	trap_LinkEntity (ent);
...
void G_BREAKWOOD(gentity_t *ent, vec3_t point, int mod) {
    ...
    // if the health of our glass brush is at 0 we'll move it
    if( ent->health <= 0 ) {
		G_FreeEntity( ent );
