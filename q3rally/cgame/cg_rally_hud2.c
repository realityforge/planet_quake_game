...
**  	cg_rally_hud2.c
**
**  	Components for the game mode dependant HUD elements.
**
**		Author: STONELANCE
*/
...
void CG_DrawHUD_Times(float x, float y){
	...
	// draw best time
	CG_DrawSmallDigitalStringColor(x + 12, y, "BEST:", colorWhite);
	if ( bestTime ){
		time = getStringForTime( bestTime );
		CG_DrawSmallDigitalStringColor(x + 102, y, va("%s (%i)", time, cent->bestLap), colorWhite);
	}
	else {
		CG_DrawSmallDigitalStringColor(x + 102, y, "N/A", colorWhite);
	}
...
*/
		if (cgs.clientinfo[other->currentState.clientNum].team == TEAM_RED){
			Vector4Copy(colorRed, color);
			color[3] = 0.5f;
		}
		else if (cgs.clientinfo[other->currentState.clientNum].team == TEAM_BLUE){
			Vector4Copy(colorBlue, color);
			color[3] = 0.5f;
		}
		else if (cgs.clientinfo[other->currentState.clientNum].team == TEAM_GREEN){
			Vector4Copy(colorGreen, color);
			color[3] = 0.5f;
		}
		else if (cgs.clientinfo[other->currentState.clientNum].team == TEAM_YELLOW){
		...
		Com_sprintf(s, sizeof(s), " %i: %s", cgs.clientinfo[other->currentState.clientNum].position, player);
