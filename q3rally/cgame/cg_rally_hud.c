...
static float CG_DrawArrowToCheckpoint( float y ) {
	centity_t	*cent;
	vec3_t		dir, forward, origin, angles;
	int			i;
...
static float CG_DrawTimes( float y ) {
	...
	if (cgs.gametype != GT_DERBY){
		...
		Com_sprintf(s, sizeof(s), "LAP TIME: %s", time);
...
static float CG_DrawSDKMessage( float y ) {
	...
	x = 4;
	w = (CG_DrawStrlen("For Development Purposes Only") * TINYCHAR_WIDTH);

	y -= 3*TINYCHAR_HEIGHT+2;

	CG_FillRect( x, y, w, 3*TINYCHAR_HEIGHT+2, bg_color );

	CG_DrawTinyStringColor( x, y, "Quake III Rally SDK", colorWhite);
	y += TINYCHAR_HEIGHT;
	CG_DrawTinyStringColor( x, y, "For Development Purposes Only", colorWhite);
	y += TINYCHAR_HEIGHT;
	CG_DrawTinyStringColor( x, y, "This is NOT the game", colorWhite);
	y += TINYCHAR_HEIGHT;
...
static float CG_DrawGear( float y ) {
	...
	y -= SMALLCHAR_HEIGHT;
	CG_DrawSmallDigitalStringColor( 560, y, va("RPM: %d", cg.predictedPlayerState.stats[STAT_RPM]), colors[0]);
	y -= SMALLCHAR_HEIGHT;
	return y;
}
