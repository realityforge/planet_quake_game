...
// dipper - instagib+ - // Willi - Offhand Grappling Hook
...
// dipper - instagib+ - no falling damage
...
// dipper - instagib+ - stop protecting
	if ( g_spawnProtect.integer ) { 	
		if ( ( ent->flags & FL_GODMODE ) && ( ( level.time - ent->client->startGod ) > g_protectTime.integer ) ) {
			client->ps.powerups[PW_BATTLESUIT] = 0;
			G_AddEvent( ent, EV_POWERUP_BATTLESUIT, 0 );
			ent->flags &= ~FL_GODMODE;
			if ( g_warnProtect.integer ) {
				trap_SendServerCommand( ent-g_entities, va("cp \"" S_COLOR_YELLOW "Protection Has Run Out!\n\"" ) );
			}
		}
	}
...
// dipper - instagib+ - dont use with offhand
...
// dipper - instagib+ - use with Willi's Offhand Grappling Hook
