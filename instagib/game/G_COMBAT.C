...
//	gentity_t	*drop;
	...
	if ( weapon > WP_MACHINEGUN && weapon != WP_GRAPPLING_HOOK && 
		...
		// spawn the item
// dipper - instagib+ - dont drop any weapons
		//Drop_Item( self, item, 0 );
	}

	// drop all the powerups if not in teamplay
// dipper - instagib+ - make sure no items get dropped
/*	if ( g_gametype.integer != GT_TEAM ) {
		angle = 45;
...
// dipper - instagib+ - promote the weapon
				ClientPromoteWeapon( attacker->client->ps.clientNum );
			}
...
// dipper - instagib+ - remove self damage
	if ( targ == attacker) {
		damage = 0; 
	}
	...
	// do the damage
	if (take) {
// dipper - instagib+ - no self damage
		if ( targ == attacker )
			return;
