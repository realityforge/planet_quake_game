...
void SaveRegisteredItems( void ) {
	...
	int		i;
	int		count;
// dipper - instagib+ - change from 19 to 18  to cache grapple
	int		maxNum = 18;

	count = 0;
// dipper - instagib+ - precache needed items only
// TODO - add options for enabling powerups.. have to cache them here
	if ( g_gametype.integer == GT_CTF ) {
		for ( i = 0 ; i < bg_numItems ; i++ ) {
			if (i > 7 && i < (bg_numItems - maxNum) ) {
				string[i] = '1';
				count++;
			} else if ( i > ( bg_numItems - 3 ) ) {
				string[i] = '1';
				count++;
			} else {
				string[i] = '0';
