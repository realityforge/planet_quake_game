...
static void CG_Obituary( entityState_t *ent ) {
	...
	if (attacker == target) {
		...
		case MOD_ANTIMATTER_SPLASH:
			message = "gets a physics lesson";
			break;
		case MOD_SHOTGUN_SPLASH:
			if ( gender == GENDER_FEMALE )
				message = "discovers her shotgun has a lot of kick";
			else if ( gender == GENDER_NEUTER )
				message = "discovers its shotgun has a lot of kick";
			else
				message = "discovers his shotgun has a lot of kick";
			break;
	...
	if ( attacker != ENTITYNUM_WORLD ) {
		...
		// CCH: New MODs
		case MOD_ANTIMATTER_SPLASH:
			message = "got an antimatter enema from";
			break;
		case MOD_SHOTGUN_SPLASH:
...
//		trap_S_StartSound (NULL, es->number, CHAN_AUTO, cgs.media.noAmmoSound );
	...
	// CCH: New event for chain lightning gun
	case EV_LIGHTNING:
		DEBUGNAME("EV_LIGHTNING");
		CG_Lightning( position, es->origin2 );
		break;
