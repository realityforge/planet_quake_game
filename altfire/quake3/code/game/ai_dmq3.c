...
bot_goal_t *BotEnemyFlag(bot_state_t *bs) {
	//NT
	if (BotTeam(bs) == TEAM_RED || BotTeam(bs) == TEAM_WATCHING_RED) {
		return &ctf_blueflag;
	}
	else {
		return &ctf_redflag;
...
void Bot1FCTFSeekGoals(bot_state_t *bs) {
	...
	if (rnd < l1 && ctf_neutralflag.areanum) {
		...
		bs->ordered = qfalse;
		//NT
		if (BotTeam(bs) == TEAM_RED || BotTeam(bs) == TEAM_WATCHING_RED) memcpy(&bs->teamgoal, &ctf_redflag, sizeof(bot_goal_t));
...
void BotObeliskSeekGoals(bot_state_t *bs) {
	...
	if (rnd < l1 && redobelisk.areanum && blueobelisk.areanum) {
		...
		bs->ordered = qfalse;
		//NT
		if (BotTeam(bs) == TEAM_RED || BotTeam(bs) == TEAM_WATCHING_RED) memcpy(&bs->teamgoal, &blueobelisk, sizeof(bot_goal_t));
		...
		bs->ordered = qfalse;
		//NT
		if (BotTeam(bs) == TEAM_RED || BotTeam(bs) == TEAM_WATCHING_RED) memcpy(&bs->teamgoal, &redobelisk, sizeof(bot_goal_t));
...
int BotSynonymContext(bot_state_t *bs) {
	...
	if (gametype == GT_CTF
		...
		//NT
		if (BotTeam(bs) == TEAM_RED || BotTeam(bs) == TEAM_WATCHING_RED) context |= CONTEXT_CTFREDTEAM;
...
void BotCheckForGrenades(bot_state_t *bs, entityState_t *state) {
	...
	//NT - making bots smarter about grenades
	// it's okay, since you can usually tell who created one because
	// they're illuminated now
	ent = &g_entities[state->number];

	// if it has no parent, avoid it
	if ( ent->parent ) {
		// if on the same team and friendly fire is off, don't worry about it at all
		if ( !g_friendlyFire.integer && BotSameTeam( bs, ent->parent->s.number ) ) {
			return;
		}

		// if on the same team and generic1 is 1 (it's an armed proximity grenade), don't worry too much
		if ( BotSameTeam( bs, ent->parent->s.number ) && ent->s.generic1 == 1 ) {
			trap_BotAddAvoidSpot(bs->ms, state->pos.trBase, 20, AVOID_ALWAYS);
			return;
		}
	}

	// try to avoid the grenade
	//NT - if it's a prox, stay further away
	if ( ent->s.generic1 == 1 ) {
...
int BotGetAlternateRouteGoal(bot_state_t *bs, int base) {
	...
	//NT
	if (base == TEAM_RED || base == TEAM_WATCHING_RED) {
