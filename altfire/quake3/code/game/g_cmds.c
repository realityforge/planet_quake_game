...
void Cmd_Give_f (gentity_t *ent)
{
	...
	if (give_all || Q_stricmp(name, "weapons") == 0)
	{
		...
		//NT - remove these so the client doesn't barf when there's
		// no model to load up
		ent->client->ps.stats[STAT_WEAPONS] &= ~(1 << WP_CHAFF);
		ent->client->ps.stats[STAT_WEAPONS] &= ~(1 << WP_PLASMAGUN2);
		ent->client->ps.stats[STAT_WEAPONS] &= ~(1 << WP_PROX_LAUNCHER);
		if (!give_all)
			return;
	}
...
void Cmd_Kill_f( gentity_t *ent ) {
	//NT - don't let watchers commit suicide
	if ( ent->client->sess.sessionTeam >= TEAM_SPECTATOR ) {
		return;
	}

	//NT - suicide while hologram ends hologram
	if ( ent->client->ps.powerups[PW_HOLOGRAM] ) {
		ent->client->ps.powerups[PW_HOLOGRAM] = 0;
		G_DestroyPlayerProxies( ent );
		return;
	}
...
void SetTeam( gentity_t *ent, char *s ) {
	...
	//NT - don't let a watcher change teams
...
void Cmd_Team_f( gentity_t *ent ) {
	...
	if ( trap_Argc() != 2 ) {
		...
		case TEAM_WATCHING_BLUE:
			...
			break;
		case TEAM_RED:
		case TEAM_WATCHING_RED:
			trap_SendServerCommand( ent-g_entities, "print \"Red team\n\"" );
			break;
		case TEAM_FREE:
		case TEAM_WATCHING_FREE:
...
static qboolean CheckValid( const char *cmd, const char *value, char **err) {
	...
	} else if ( !Q_stricmp( cmd, "fraglimit" ) || !Q_stricmp( cmd, "clientkick" ) || 
		!Q_stricmp( cmd, "timelimit" ) ) {
		i = atoi( value );
		if ( i < 0 ) {
	...
	} else if ( !Q_stricmp( cmd, "g_doWarmup" ) || !Q_stricmp( cmd, "g_fallingDamage" ) ||
		!Q_stricmp( cmd, "g_friendlyFire" ) || !Q_stricmp( cmd, "g_multiFlag" ) ||
		!Q_stricmp( cmd, "g_returnFlag" ) || !Q_stricmp( cmd, "g_explodingAmmo" ) ||
		!Q_stricmp( cmd, "g_teleportMissiles" ) ) {
	...
	} else if ( !Q_stricmp( cmd, "g_selfDamage" ) ) {
		if ( strlen( value ) == 0 || strlen( value ) > 1 ) {
			*err = "print \"Valid values are 0, 1, and 2.\n\"";
			return qfalse;
		}
