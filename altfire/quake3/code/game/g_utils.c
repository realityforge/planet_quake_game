...
void G_KillBox (gentity_t *ent) {
	...
	//NT - killbox can be done without a client now
	if ( ent->client ) {
	...
	for (i=0 ; i<num ; i++) {
		...
		if ( !hit->client && hit->s.eType != ET_PLAYER_PROXY ) {
			continue;
		}

		//NT - just to make sure
...
//NT - finds the next entity within the radius squared
...
//NT - returns qtrue if ent2 is visible to ent1
...
//NT - returns an integer unique to this game
