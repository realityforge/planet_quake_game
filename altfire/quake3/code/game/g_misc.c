...
=================================================================================
*/
//NT - added effect, spit, and telefrag parameters
void TeleportPlayer( gentity_t *player, vec3_t origin, vec3_t angles, qboolean effect, qboolean spit, qboolean telefrag ) {
	...
	//NT - don't do it for watchers
	if ( player->client->sess.sessionTeam < TEAM_SPECTATOR ) {
		//NT - check to see if an effect is called for
		if ( effect ) {
	...
	//NT - don't do it for watchers either, or things that shouldn't telefrag
	if ( telefrag && player->client->sess.sessionTeam < TEAM_SPECTATOR ) {
		G_KillBox (player);
	}
	...
	//NT - don't link watchers either
	if ( player->client->sess.sessionTeam < TEAM_SPECTATOR ) {
		trap_LinkEntity (player);
	}
}

//NT - teleport an entity that's not a player (for teleporting missiles mostly)
...
//	}
	...
	if( !destination ) {
		if( self->pos1[0] || self->pos1[1] || self->pos1[2] ) {
			TeleportPlayer( other, self->pos1, self->s.angles, qtrue, qtrue, qtrue );
		}
		G_Damage( other, other, other, NULL, NULL, 100000, DAMAGE_NO_PROTECTION, MOD_TELEFRAG );
		return;
	}

	TeleportPlayer( other, destination->s.pos.trBase, destination->s.angles, qtrue, qtrue, qtrue );
}
