	...
	for ( n = 0; n < g_numBots ; n++ ) {
		...
		if (i >= g_maxclients.integer) {
			...
			if (num <= 0) {
				...
				//NT
				if (team == TEAM_RED || team == TEAM_WATCHING_RED) teamstr = "red";
				else if (team == TEAM_BLUE || team == TEAM_WATCHING_BLUE) teamstr = "blue";
				else teamstr = "";
...
void G_CheckMinimumPlayers( void ) {
	...
	if (g_gametype.integer >= GT_TEAM) {
		...
		//NT
		humanplayers += G_CountHumanPlayers( TEAM_WATCHING_RED );
		botplayers = G_CountBotPlayers(	TEAM_RED );
		//NT
		botplayers += G_CountBotPlayers(	TEAM_WATCHING_RED );
		//
		...
		//NT
		humanplayers += G_CountHumanPlayers( TEAM_WATCHING_BLUE );
		botplayers = G_CountBotPlayers( TEAM_BLUE );
		//NT
		botplayers += G_CountBotPlayers( TEAM_WATCHING_BLUE );
		//
		if (humanplayers + botplayers < minplayers) {
