...
void P_WorldEffects( gentity_t *ent ) {
	qboolean	envirosuit, protection;	//NT - new protection
	int			waterlevel;
	...
	//NT - holograms can't drown, and the proxy will keep track of airOutTime
	...
	//NT - liquid metal
	protection = ent->client->ps.powerups[PW_PROTECTION] > level.time;
	...
	if (waterlevel && 
		...
		if (ent->health > 0
			...
			//NT - decided against protection from lava
//			} else if ( protection ) {
...
void	G_TouchTriggers( gentity_t *ent ) {
	...
	for ( i=0 ; i<num ; i++ ) {
		...
		//NT - There's a better way to do this (e.g. always fire the ->touch
		// func for all flags even if r.contents == 0) but I don't feel
		// like rewriting more CTF code
...
void SpectatorThink( gentity_t *ent, usercmd_t *ucmd ) {
	...
	//NT - attack2 does also
	if ( (( client->buttons & BUTTON_ATTACK ) && ! ( client->oldbuttons & BUTTON_ATTACK ))
		|| (( client->buttons & BUTTON_ATTACK2 ) && ! ( client->oldbuttons & BUTTON_ATTACK2 )) ) {
		Cmd_FollowCycle_f( ent, 1 );
	}
}

//NT - needed a think function for the temporary watcher
/*
=================
WatcherThink
=================
*/
void WatcherThink( gentity_t *ent, usercmd_t *ucmd ) {
	pmove_t	pm;
	gclient_t	*client;
...
qboolean ClientInactivityTimer( gclient_t *client ) {
	...
	//NT - attack2
		...
		(client->pers.cmd.buttons & BUTTON_ATTACK || client->pers.cmd.buttons & BUTTON_ATTACK2) ) {
...
void ClientTimerActions( gentity_t *ent, int msec ) {
	...
	while ( client->timeResidual >= 1000 ) {
		...
		if ( client->ps.powerups[PW_REGEN] ) {
			//NT - vitality powerup
			if ( client->ps.powerups[PW_VITALITY] ) {
				ent->health += 15;
				if ( ent->health > client->ps.stats[STAT_MAX_HEALTH] * 3 ) {
					ent->health = client->ps.stats[STAT_MAX_HEALTH] * 3;
				} else {
					G_AddEvent( ent, EV_POWERUP_REGEN, 0 );
				}
			} else {
		...
		} else if ( client->ps.powerups[PW_VITALITY] ) {
#endif
		} else {
		...
		//NT - only when the player doesn't have vitality
		if ( !client->ps.powerups[PW_VITALITY] ) {
...
//	gentity_t *drop;
	...
	for ( i = oldEventSequence ; i < client->ps.eventSequence ; i++ ) {
		...
		//NT - secondary attack event
		case EV_FIRE_WEAPON2:
			FireWeapon2( ent );
			break;

		//NT - consolidated dropping of flags
		...
		//NT - gives the player a PW_HOLOGRAM powerup
		case EV_USE_ITEM6:		// hologram generator
			Use_Hologram_Generator( ent );
			break;
...
//		G_Printf("serverTime >>>>>\n" );
	...
	//NT - watchers need a think function too
	if ( client->sess.sessionTeam > TEAM_SPECTATOR ) {
		WatcherThink( ent, ucmd );
		return;
	}
...
#endif
	//NT - speed powerup
	if ( client->ps.powerups[PW_SPEED] ) {
		int pivot = (client->ps.powerups[PW_SPEED] - client->ps.stats[STAT_SPEED_PICKUP]) * 2 / 3;
		client->ps.speed *= (float)(client->ps.powerups[PW_SPEED] - level.time) / pivot + 0.5;
	}
	...
	if ( client->ps.weapon == WP_GRAPPLING_HOOK &&
		client->hook && !( ucmd->buttons & BUTTON_ATTACK ) && !( ucmd->buttons & BUTTON_ATTACK2 )) {
