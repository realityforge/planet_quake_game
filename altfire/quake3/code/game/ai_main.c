...
void BotReportStatus(bot_state_t *bs) {
	...
	if (gametype == GT_CTF) {
		if (BotCTFCarryingFlag(bs)) {
			//NT
			if (BotTeam(bs) == TEAM_RED || BotTeam(bs) == TEAM_WATCHING_RED) strcpy(flagstatus, S_COLOR_RED"F ");
...
void BotSetInfoConfigString(bot_state_t *bs) {
	...
	if (gametype == GT_CTF) {
		...
		if (BotHarvesterCarryingCubes(bs)) {
			//NT
			if (BotTeam(bs) == TEAM_RED || BotTeam(bs) == TEAM_WATCHING_RED) Com_sprintf(carrying, sizeof(carrying), "%2d", bs->inventory[INVENTORY_REDCUBE]);
