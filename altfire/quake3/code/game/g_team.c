...
qboolean OnSameTeam( gentity_t *ent1, gentity_t *ent2 ) {
	...
	//NT - if either is a spectator, they can't be on the same team
	if ( ent1->client->sess.sessionTeam == TEAM_SPECTATOR ) {
		return qfalse;
	}
	...
	//NT - if difference is watching diff, they are
	if ( abs(ent1->client->sess.sessionTeam - ent2->client->sess.sessionTeam) == WATCHING_DIFF ) {
		return qtrue;
	}
...
void Team_CheckHurtCarrier(gentity_t *targ, gentity_t *attacker)
{
	...
	//NT
	if (targ->client->sess.sessionTeam == TEAM_RED || targ->client->sess.sessionTeam == TEAM_WATCHING_RED)
		flag_pw = PW_BLUEFLAG;
	else
		flag_pw = PW_REDFLAG;
...
void Team_ReturnFlagSound( gentity_t *ent, int team ) {
	...
	//NT
	if( team == TEAM_BLUE || team == TEAM_WATCHING_BLUE ) {
...
void Team_TakeFlagSound( gentity_t *ent, int team, qboolean enemy ) {
	...
	if ( g_multiFlag.integer == 1 ) {
		// only play when a flag has not been picked up for one second
		switch(team) {
			case TEAM_RED:
				if (teamgame.blueTakenTime > level.time - 1000)
					return;
			...
			case TEAM_BLUE:	// CTF
				if (teamgame.redTakenTime > level.time - 1000)
					return;
...
int Team_TouchEnemyFlag( gentity_t *ent, gentity_t *other, int team ) {
	...
	if ( cl->ps.powerups[enemy_flag] == 0 ) {
		...
		//NT - if it's multi and we take it from its place, respawn it
		if ( g_multiFlag.integer == 1 ) {
			if ( ent->flags & FL_DROPPED_ITEM )
				return -1;
			else 
	...
	if (level.time - level.lastTeamLocationTime > TEAM_LOCATION_UPDATE_TIME) {
		...
		for (i = 0; i < g_maxclients.integer; i++) {
			...
			if (ent->inuse && (ent->client->sess.sessionTeam == TEAM_RED ||
				...
				ent->client->sess.sessionTeam == TEAM_WATCHING_RED ||
				ent->client->sess.sessionTeam == TEAM_WATCHING_BLUE )) {
