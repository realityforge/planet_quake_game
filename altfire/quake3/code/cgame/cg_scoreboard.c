...
static void CG_DrawClientScore( int y, score_t *score, float *color, float fade, qboolean largeFormat ) {
	...
	//NT - altered a bunch for return-the-flag
	...
	if ( score->ping == -1 ) {
		Com_sprintf(string, sizeof(string),
			" connecting    %s", ci->filteredName); //NT - name filter
	} else if ( ci->team == TEAM_SPECTATOR ) {
		Com_sprintf(string, sizeof(string),
			" SPECT %3i %4i %s", score->ping, score->time, ci->filteredName); //NT - name filter
	} else {
		Com_sprintf(string, sizeof(string),
			"%5i %4i %4i %s", score->score, score->ping, score->time, ci->filteredName); //NT - name filter
	}
...
void CG_DrawOldTourneyScoreboard( void ) {
	...
	if ( cgs.gametype >= GT_TEAM ) {
		...
		for ( i = 0 ; i < MAX_CLIENTS ; i++ ) {
			...
			CG_DrawStringExt( 8, y, ci->filteredName, color, qtrue, qtrue, GIANT_WIDTH, GIANT_HEIGHT, 0 ); //NT - name filter
			s = va("%i", ci->score );
			CG_DrawStringExt( 632 - GIANT_WIDTH * strlen(s), y, s, color, qtrue, qtrue, GIANT_WIDTH, GIANT_HEIGHT, 0 );
			y += 64;
		}
