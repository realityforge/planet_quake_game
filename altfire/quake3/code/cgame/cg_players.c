...
static qboolean CG_RegisterClientModelname( clientInfo_t *ci, const char *modelName, const char *skinName, const char *headModelName, const char *headSkinName, const char *teamName ) {
	...
	if ( !CG_RegisterClientSkin( ci, teamName, modelName, skinName, headName, headSkinName ) ) {
		if ( teamName && *teamName) {
			...
			//NT
			if( ci->team == TEAM_BLUE || ci->team == TEAM_WATCHING_BLUE ) {
...
void CG_NewClientInfo( int clientNum ) {
	...
	//NT - name filter
	Q_strncpyz( newInfo.filteredName, v, sizeof( newInfo.filteredName ) );
	if ( cg_nameFilter.integer || ( cgs.forceNameFilter && cg_allowForceFilters.integer ) ) {
		CG_FilterName( newInfo.filteredName );
	}
...
static void CG_PlayerAnimation( centity_t *cent, int *legsOld, int *legs, float *legsBackLerp,
	...
	//NT - added speed
	if ( cent->currentState.powerups & ( 1 << PW_HASTE ) || cent->currentState.powerups & ( 1 << PW_SPEED ) ) {
		speedScale = 1.5;
	} else {
		speedScale = 1;
	}
...
===============
*/
//NT - added angles and updateCent for player proxy
static void CG_PlayerAngles( centity_t *cent, vec3_t legs[3], vec3_t torso[3], vec3_t head[3], vec3_t angles, qboolean updateCent ) {
	...
	int			dir, clientNum;
	clientInfo_t	*ci;
	qboolean	swinging; //NT

	VectorCopy( angles, headAngles );
	...
	if ( updateCent ) {
		...
		swinging = cent->pe.torso.yawing;
		CG_SwingAngles( torsoAngles[YAW], 25, 90, cg_swingSpeed.value, &torsoAngles[YAW], &swinging );

		swinging = cent->pe.legs.yawing;
		CG_SwingAngles( legsAngles[YAW], 40, 90, cg_swingSpeed.value, &legsAngles[YAW], &swinging );
	}
	...
	if ( updateCent ) {
		...
		swinging = cent->pe.torso.pitching;
		CG_SwingAngles( dest, 15, 30, 0.1f, &torsoAngles[PITCH], &swinging );
	}
...
//NT - moved #ifdef MISSIONPACK lower
/*
===============
CG_BreathPuffs
===============
...
//	}
	...
	//NT - changed this from 2000
	ci->breathPuffTime = cg.time + 500;
}
...
//NT - changed this a bit to account for one player having two flags
/*
===============
CG_TrailItem
===============
...
===============
*/
//NT - added head for breath puffs
static void CG_PlayerPowerups( centity_t *cent, refEntity_t *torso, refEntity_t *head ) {
	...
	//NT - add the hologram effect
	if ( ( powerups & ( 1 << PW_HOLOGRAM ) ) && cg.holoFlashTime < 0 ) {
		// add sounds if not the current player
		if ( cent->currentState.number != cg.snap->ps.clientNum || cg_thirdPerson.integer ) {
			int r = random() * 24;
			if ( r == 0 ) {
				trap_S_StartSound( cent->lerpOrigin, ENTITYNUM_WORLD, CHAN_WEAPON, cgs.media.sfx_lghit1 );
			} else if ( r == 1 ) {
				trap_S_StartSound( cent->lerpOrigin, ENTITYNUM_WORLD, CHAN_WEAPON, cgs.media.sfx_lghit2 );
			} else if ( r == 2 ) {
				trap_S_StartSound( cent->lerpOrigin, ENTITYNUM_WORLD, CHAN_WEAPON, cgs.media.sfx_lghit3 );
			}
		}

		trap_R_AddLightToScene( cent->lerpOrigin, 96 + (rand()&63), 0.5f, 0.5f, 0.5f );
	}
	...
	//NT - speed might, or it might make breath puffs
	else if ( powerups & ( 1 << PW_SPEED ) ) {
...
static qboolean CG_PlayerShadow( centity_t *cent, float *shadowPlane ) {
	...
	*shadowPlane = 0;
	...
	//NT - no shadows for holograms
	if ( cent->currentState.powerups & ( 1 << PW_HOLOGRAM ) ) {
		return qfalse;
	}
...
void CG_AddRefEntityWithPowerups( refEntity_t *ent, entityState_t *state, int team ) {
	...
	if ( state->powerups & ( 1 << PW_INVIS ) ) {
		...
		//NT - accuracy powerup
		if ( state->powerups & ( 1 << PW_ACCURACY ) ) {
			ent->customShader = cgs.media.accuracyShader;
			trap_R_AddRefEntityToScene( ent );
		}
		//NT - vitality powerup
		if ( state->powerups & ( 1 << PW_VITALITY ) ) {
			ent->customShader = cgs.media.vitalityShader;
			trap_R_AddRefEntityToScene( ent );
		}
		//NT - liquid metal skin
		if ( state->powerups & ( 1 << PW_PROTECTION ) ) {
			if ( team == TEAM_RED || team == TEAM_WATCHING_RED )
				ent->customShader = cgs.media.protectionShader_red;
			else if ( team == TEAM_BLUE || team == TEAM_WATCHING_BLUE )
				ent->customShader = cgs.media.protectionShader_blue;
			else
...
void CG_Player( centity_t *cent ) {
	...
	//NT - hologram glitch
	if ( ( cent->currentState.powerups & ( 1 << PW_HOLOGRAM ) ) && cg.holoGlitchTime < 0 )
		linkPlayer = qfalse;
	...
	//NT - added update flag for player proxy
	CG_PlayerAngles( cent, legs.axis, torso.axis, head.axis, cent->lerpAngles, qtrue );
...
Add's a player's proxy doing the player's actions
Most of this was shamelessly ripped off from CG_Player
===================
*/
void CG_PlayerProxy( centity_t *proxy ) {
	centity_t		*cent;
	int				entNum;
	clientInfo_t	*ci;
	refEntity_t		legs;
	...
	int				renderfx;
	weaponInfo_t	*weapon;
	vec3_t			barrelangles, origin, angles;

	entNum = proxy->currentState.otherEntityNum;
	...
	VectorCopy( proxy->currentState.apos.trBase, angles );
	angles[PITCH] = cent->lerpAngles[PITCH];

	// get the rotation information
	CG_PlayerAngles( cent, legs.axis, torso.axis, head.axis, angles, qfalse );
	
	// get the animation state (after rotation, to allow feet shuffle)
	CG_PlayerAnimation( cent, &legs.oldframe, &legs.frame, &legs.backlerp,
		 &torso.oldframe, &torso.frame, &torso.backlerp );

	VectorCopy( proxy->lerpOrigin, origin );

	// add the joystick balloon
	memset( &balloon, 0, sizeof( balloon ) );
	VectorCopy( origin, balloon.origin );
	balloon.origin[2] += 48;
	balloon.reType = RT_SPRITE;
	balloon.customShader = cgs.media.holoBalloonShader;
	balloon.radius = 10;
	balloon.shaderRGBA[0] = 255;
	balloon.shaderRGBA[1] = 255;
	balloon.shaderRGBA[2] = 255;
	balloon.shaderRGBA[3] = 255;
	trap_R_AddRefEntityToScene( &balloon );
	...
	if ( cg.holoFlashTime > 0 ) {
		...
		effect.origin[2] -= 24;
		effect.reType = RT_MODEL;
		effect.hModel = cgs.media.teleportEffectModel;
		effect.customShader = cgs.media.holoEffectShader;
		AxisClear( effect.axis );

		trap_R_AddRefEntityToScene( &effect );
		trap_R_AddLightToScene( origin, 64, 1.0f, 0.9f, 0.9f );
	}
	...
	if ( weapon->barrelModel ) {
		...
		barrelangles[YAW] = 0;
		barrelangles[PITCH] = 0;
		barrelangles[ROLL] = CG_MachinegunSpinAngle( cent );
