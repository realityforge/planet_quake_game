...
//NT
vmCvar_t	cg_profanity;
vmCvar_t	cg_chatFilter;
vmCvar_t	cg_nameFilter;
vmCvar_t	cg_allowForceFilters;
vmCvar_t	cg_showHints;
vmCvar_t	cg_hintNum;
...
static cvarTable_t cvarTable[] = { // bk001129
	...
	//NT
	{ &cg_gibs, "com_gibs", "1", CVAR_ARCHIVE  },
	...
	//NT
	{ &cg_profanity, "cg_profanity", "1", CVAR_ARCHIVE},
	{ &cg_chatFilter, "cg_chatFilter", "0", CVAR_ARCHIVE},
	{ &cg_nameFilter, "cg_nameFilter", "0", CVAR_ARCHIVE},
	{ &cg_allowForceFilters, "cg_allowForceFilters", "1", CVAR_ARCHIVE},
	{ &cg_showHints, "cg_showHints", "1", CVAR_ARCHIVE},
	{ &cg_hintNum, "cg_hintNum", "0", CVAR_ARCHIVE},
...
qboolean CG_FilterString( char *str, char **words, int numWords, char *customWords, int numCustomWords ) {
...
qboolean CG_FilterChat( char *str ) {
	int ret = CG_FilterString( str, filteredWords, nFilteredWords, cgs.filteredWords, cgs.numFilteredWords );
...
qboolean CG_FilterName( char *str ) {
	int ret = CG_FilterString( str, filteredNames, nFilteredNames, cgs.filteredNames, cgs.numFilteredNames );
	// These are used to simulate VM conditions (slow)
	// Comment them out for production!
/*
	CG_FilterString( str, filteredNames, nFilteredNames, cgs.filteredNames, cgs.numFilteredNames );
	CG_FilterString( str, filteredNames, nFilteredNames, cgs.filteredNames, cgs.numFilteredNames );
	CG_FilterString( str, filteredNames, nFilteredNames, cgs.filteredNames, cgs.numFilteredNames );
	CG_FilterString( str, filteredNames, nFilteredNames, cgs.filteredNames, cgs.numFilteredNames );
	CG_FilterString( str, filteredNames, nFilteredNames, cgs.filteredNames, cgs.numFilteredNames );
...
void QDECL CG_Printf( const char *msg, ... ) {
	...
	if ( cg_nameFilter.integer || ( cgs.forceNameFilter && cg_allowForceFilters.integer ) ) {
		int i, j;
		clientInfo_t	*ci;
		// loop through the string to look for client names
		for ( i = 0; text[i] != '\0'; i++) {
			// loop through all the clients to match names
			for ( j = 0, ci = cgs.clientinfo; j < cgs.maxclients; j++, ci++ ) {
				if ( ci->infoValid ) {
					int len = strlen( ci->name );
					if ( Q_strncmp( &text[i], ci->name, len ) == 0 ) {
						// isolate and filter
						char tmp = text[i + len];
						text[i + len] = '\0';
						CG_FilterName( &text[i] );
						text[i + len] = tmp;
					}
				}
			}
	...
	//NT - chat filter
	if ( cg_chatFilter.integer || ( cgs.forceChatFilter && cg_allowForceFilters.integer ) ) {
...
//		}
	...
	//NT - radius rail prefire sound
	cgs.media.sfx_railgpf = trap_S_RegisterSound ("sound/weapons/railgun/railgpf.wav", qfalse);
...
#endif
	//NT - need these sounds for BFG overload
	...
	//NT - hologram powerup sounds
	cgs.media.holoStartSound = trap_S_RegisterSound("sound/items/holostart.wav", qfalse);
...
static void CG_RegisterGraphics( void ) {
	...
	//NT - liquid metal shaders
	cgs.media.protectionShader = trap_R_RegisterShader("powerups/protection" );
	cgs.media.protectionShader_blue = trap_R_RegisterShader("powerups/protection_blue" );
	cgs.media.protectionShader_red = trap_R_RegisterShader("powerups/protection_red" );
	//NT - alternate powerup shaders
	cgs.media.accuracyShader = trap_R_RegisterShader("powerups/accuracy" );
	cgs.media.accuracyWeaponShader = trap_R_RegisterShader("powerups/accuracyWeapon" );
	cgs.media.vitalityShader = trap_R_RegisterShader("powerups/vitality" );
	cgs.media.vitalityWeaponShader = trap_R_RegisterShader("powerups/vitalityWeapon" );
	...
	//NT - logo
	cgs.media.logoShader = trap_R_RegisterShader( "gfx/2d/altfire_logo_medium" );
	...
	//NT - hologram indicator shader
	cgs.media.holoBalloonShader = trap_R_RegisterShader( "sprites/holoballoon" );
	cgs.media.holoEffectShader = trap_R_RegisterShader( "models/mapobjects/teleporter/energy" );
	cgs.media.holoViewShader = trap_R_RegisterShader( "sprites/holoview" );
	...
	//NT - for the BFG overload shockwave
	cgs.media.blueSphereModel = trap_R_RegisterModel( "models/alternatefire/blue_sphere.md3" );
	cgs.media.bfgShockwaveShader = trap_R_RegisterShader( "models/alternatefire/bfg_shockwave" );
	cgs.media.bfgRedSphereShader = trap_R_RegisterShader( "models/alternatefire/red_sphere" );
	cgs.media.bfgRedShockwaveShader = trap_R_RegisterShader( "models/alternatefire/bfg_shockwave_red" );
	//NT - accuracy powerup
	cgs.media.accuracyCrosshairShader = trap_R_RegisterShader( "textures/effects/crosshair_big" );
	...
	for ( i = 1 ; i < bg_numItems ; i++ ) {
		if ( items[ i ] == '1' || cg_buildScript.integer ) {
			...
			//NT - also register alternate powerups
			if ( bg_itemlist[ i ].giType == IT_POWERUP ) {
				if ( bg_itemlist[ i ].giTag == PW_QUAD ) {
					int j = BG_FindItemForPowerup( PW_ACCURACY ) - bg_itemlist;
					CG_LoadingItem( j );
					CG_RegisterItemVisuals( j );
				} else if ( bg_itemlist[ i ].giTag == PW_HASTE ) {
					int j = BG_FindItemForPowerup( PW_SPEED ) - bg_itemlist;
					CG_LoadingItem( j );
					CG_RegisterItemVisuals( j );
				} else if ( bg_itemlist[ i ].giTag == PW_REGEN ) {
					int j = BG_FindItemForPowerup( PW_VITALITY ) - bg_itemlist;
					CG_LoadingItem( j );
					CG_RegisterItemVisuals( j );
				} else if ( bg_itemlist[ i ].giTag == PW_INVIS ) {
					int j = BG_FindItemForHoldable( HI_HOLOGRAM ) - bg_itemlist;
					CG_LoadingItem( j );
					CG_RegisterItemVisuals( j );
				}
			} else if ( bg_itemlist[ i ].giType == IT_HOLDABLE ) {
				if ( bg_itemlist[ i ].giTag == HI_TELEPORTER ) {
					int j = BG_FindItemForHoldable( HI_HOLOGRAM ) - bg_itemlist;
					// keep this unknown to the user (it's an easter egg, after all)
					//CG_LoadingItem( j );
					CG_RegisterItemVisuals( j );
				}
			}
...
void CG_BuildSpectatorString() {
	...
	for (i = 0; i < MAX_CLIENTS; i++) {
		if (cgs.clientinfo[i].infoValid && cgs.clientinfo[i].team == TEAM_SPECTATOR ) {
			//NT - name filter
			Q_strcat(cg.spectatorList, sizeof(cg.spectatorList), va("%s     ", cgs.clientinfo[i].filteredName));
		}
	}
...
static int CG_FeederCount(float feederID) {
	...
	if (feederID == FEEDER_REDTEAM_LIST) {
		for (i = 0; i < cg.numScores; i++) {
			//NT
			if (cg.scores[i].team == TEAM_RED || cg.scores[i].team == TEAM_WATCHING_RED) {
				count++;
			}
		}
		...
		for (i = 0; i < cg.numScores; i++) {
			//NT
			if (cg.scores[i].team == TEAM_BLUE || cg.scores[i].team == TEAM_WATCHING_BLUE) {
				count++;
			}
		}
...
void CG_SetScoreSelection(void *p) {
	...
	for (i = 0; i < cg.numScores; i++) {
		...
		if (cg.scores[i].team == TEAM_RED || cg.scores[i].team == TEAM_WATCHING_RED) {
			red++;
		//NT
		} else if (cg.scores[i].team == TEAM_BLUE || cg.scores[i].team == TEAM_WATCHING_BLUE) {
			blue++;
		}
	...
	if ( cgs.gametype >= GT_TEAM ) {
		...
		i = red;
		//NT
		if (cg.scores[cg.selectedScore].team == TEAM_BLUE || cg.scores[cg.selectedScore].team == TEAM_WATCHING_BLUE) {
