...
void CG_ParseServerinfo( void ) {
	...
	cgs.forceChatFilter = atoi( Info_ValueForKey( info, "g_forceChatFilter" ) );
	trap_Cvar_Set("g_forceChatFilter", va("%i", cgs.forceChatFilter));

	cgs.forceNameFilter = atoi( Info_ValueForKey( info, "g_forceNameFilter" ) );
	trap_Cvar_Set("g_forceNameFilter", va("%i", cgs.forceNameFilter));

	Q_strncpyz( cgs.filteredWords, Info_ValueForKey( info, "g_filteredWords" ), BIG_INFO_VALUE );
	trap_Cvar_Set("g_filteredWords", cgs.filteredWords);
	cgs.numFilteredWords = 0;
	for ( i = 0; cgs.filteredWords[i] != '\0'; i++ ) {
		if ( cgs.filteredWords[i] == ' ' || cgs.filteredWords[i] == '\t' ) {
			cgs.filteredWords[i] = '\0';
			cgs.numFilteredWords++;
		}
	}
	if ( cgs.filteredWords[0] != '\0' )
		cgs.numFilteredWords++;

	Q_strncpyz( cgs.filteredNames, Info_ValueForKey( info, "g_filteredNames" ), BIG_INFO_VALUE );
	trap_Cvar_Set("g_filteredNames", cgs.filteredNames);
	cgs.numFilteredNames = 0;
	for ( i = 0; cgs.filteredNames[i] != '\0'; i++ ) {
		if ( cgs.filteredNames[i] == ' ' || cgs.filteredNames[i] == '\t' ) {
			cgs.filteredNames[i] = '\0';
			cgs.numFilteredNames++;
		}
	}
	if ( cgs.filteredNames[0] != '\0' )
		cgs.numFilteredNames++;

	//NT - version check
	Q_strncpyz( cgs.alternateFireVersion, Info_ValueForKey( info, "g_alternateFireVersion" ), MAX_QPATH );
	trap_Cvar_Set("g_alternateFireVersion", cgs.alternateFireVersion);
}
