...
#include "g_local.h"
   ...
   //BJ
      ...
      //count number of redflags
         for (ent = NULL, count = 0; (ent = G_Find (ent, FOFS(classname), "team_CTF_redflag")) != NULL; count++);
      
         if (count > 0) {
         
         //find random red flag
            choice = (random() * count) + 1;
         
            G_Printf("********* %i %i\n", count, choice);
            for (ent = NULL, count = 0; count < choice; ent = G_Find (ent, FOFS(classname), "team_CTF_redflag"), count++);
      ...
      //rinse and repeat for blue flag
         for (ent = NULL, count = 0; (ent = G_Find (ent, FOFS(classname), "team_CTF_blueflag")) != NULL; count++);
      
         if (count > 0) {
         
            choice = (random() * count) + 1;
            for (ent = NULL, count = 0; count < choice; ent = G_Find (ent, FOFS(classname), "team_CTF_blueflag"), count++);
...
*/
   ...
   //BJ
   //select random flag ent
      choice = (random() * count) + 1;
      for (count = 0, ent = NULL; count < choice; ent = G_Find (ent, FOFS(classname), c), count++);
