...
void CG_CheckLocalSounds( playerState_t *ps, playerState_t *ops ) {
	...
	//Hal9000 play win sound ??
	if ( ps->persistant[PERS_SCORE] >= cgs.fraglimit ) {
		trap_S_StartLocalSound( cgs.media.redLeadsSound, CHAN_ANNOUNCER );
	}

	//Hal9000 play lose sound ??
	if ( cg.intermissionStarted && (ps->persistant[PERS_SCORE] < cgs.fraglimit) ) {
...
/* Hal9000 using my own damn sounds
...
/*
		if ( !( cg.timelimitWarnings & 2 ) && msec > (cgs.timelimit - 1) * 60 * 1000 ) {
			cg.timelimitWarnings |= 2;
			trap_S_StartLocalSound( cgs.media.sixtysecSound, CHAN_ANNOUNCER );
		}
		if ( !( cg.timelimitWarnings & 4 ) && msec > (cgs.timelimit - 0.5) * 60 * 1000 ) {
			cg.timelimitWarnings |= 2;
			trap_S_StartLocalSound( cgs.media.thirtysecSound, CHAN_ANNOUNCER );
		}
*/
	}
