...
int		w = 0; //Hal9000 global var for win or lose
...
void MoveClientToIntermission( gentity_t *ent ) {
	...
	//Hal9000 check to see if this isn't a bot and has the fraglimit
	if ( (!(ent->r.svFlags & SVF_BOT)) && (ent->client->ps.persistant[PERS_SCORE] >= g_fraglimit.integer ) ) {
		w = 1;
	}
...
void CheckIntermissionExit( void ) {
	...
	//Hal9000 check if player has won (w==1), if not, kick their sorry ass out
	if ( w == 0 ) {
		for (i=0 ; i< g_maxclients.integer ; i++) {
			cl = level.clients + i;
			if ( !(g_entities[cl->ps.clientNum].r.svFlags & SVF_BOT) ) {
				trap_DropClient( i, NULL );
			}
		}
	}
