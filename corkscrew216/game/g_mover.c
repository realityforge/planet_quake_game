...
qboolean	G_TryPushingEntity( gentity_t *check, gentity_t *pusher, vec3_t move, vec3_t amove ) {
	...
	if ( check->s.groundEntityNum != pusher->s.number && check->s.eType != ET_GRAPPLE) {
		check->s.groundEntityNum = -1;
	}

	block = G_TestEntityPosition( check );
	if (!block || check->s.eType == ET_GRAPPLE) {
		// pushed ok
		if ( check->client ) {
...
qboolean G_MoverPush( gentity_t *pusher, vec3_t move, vec3_t amove, gentity_t **obstacle ) {
	...
	*obstacle = NULL;
	...
	for ( e = 0 ; e < listedEntities ; e++ ) {
		...
		if ( check->s.eType != ET_ITEM && check->s.eType != ET_PLAYER && check->s.eType != ET_GRAPPLE && !check->physicsObject ) {
			continue;
		}
		
		if ( check->s.eType == ET_GRAPPLE &&
			!strcmp(pusher->classname, "func_rotating") ) {
			continue;
		}
		...
		if ( G_TryPushingEntity( check, pusher, move, amove ) ) {
			if ( check->s.eType == ET_GRAPPLE )
				VectorCopy( check->r.currentOrigin, check->parent->client->ps.grapplePoint );
			continue;
		}
