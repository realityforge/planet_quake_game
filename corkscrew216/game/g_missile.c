...
void G_MissileImpact( gentity_t *ent, trace_t *trace ) {
	...
	if (!strcmp(ent->classname, "hook")) {
		...
		nent->s.weapon = WP_GRAPPLING_HOOK;
...
gentity_t *fire_grapple (gentity_t *self, vec3_t start, vec3_t dir) {
	...
	if ( level.accuracyChallenge ) {
		...
		if ( self->client->ps.powerups[PW_HASTE] )
			VectorScale( dir, g_grappleSpeed.integer * 1.3, hook->s.pos.trDelta );
			else VectorScale( dir, g_grappleSpeed.integer, hook->s.pos.trDelta );
	}
