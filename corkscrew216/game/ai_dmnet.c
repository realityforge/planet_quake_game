...
int BotNearbyGoal(bot_state_t *bs, int tfl, bot_goal_t *ltg, float range) {
	...
	if( g_headHunt.integer ) {
		// make range huge when hunting for heads
		range = 1000;

		// testing.... try and see if we can make the bot go for the heads more...
		for( i = 0; i < 4; i++ ) {
			ret = trap_BotChooseNBGItem(bs->gs, bs->origin, bs->inventory, tfl, ltg, range);

			// if no nbg at all, don't bother
			if( !ret ) break;
			...
			if( i < 3 ) {
				if( ent->item->giTag == PW_HEAD ) {
					if( ent->s.otherEntityNum == bs->client || BotTeam(bs) == ent->spawnflags ) {
						trap_BotPopGoal(bs->gs);
					} else {
						break;
					}
		...
		// check if we should ignore our current enemy and just go straight for the head
/*
		if( bs->enemy != -1 ) {
			if( trap_BotGetTopGoal(bs->gs, &goal) ) {
				ent = &g_entities[ goal.entitynum ];
				if( ent && ent->item->giTag == PW_HEAD ) {
					VectorSubtract( goal.origin, bs->origin, distance );
					if( VectorLengthSquared( distance ) < Square( 250 ) ) {
						BotAI_Print(PRT_MESSAGE, "ignoring my enemy\n" );
						bs->enemy = -1;
					}
				}
			}
...
int AINode_Seek_ActivateEntity(bot_state_t *bs) {
	...
	// CorkScrew: shoot bodies to get heads
	BotHeadHunt(bs);
