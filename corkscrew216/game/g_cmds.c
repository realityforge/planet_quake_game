...
void DeathmatchScoreboardMessage( gentity_t *ent ) {
	...
	for (i=0 ; i < numSorted ; i++) {
		...
		time2 = ( (float)(level.time - cl->pers.enterTime)/60000.0 - time ) * 60;

		Com_sprintf (entry, sizeof(entry),
			" %i %i %i %i %i %i %i", level.sortedClients[i],
			cl->ps.persistant[PERS_SCORE], ping, time, time2,
...
void ClientCommand( int clientNum ) {
	...
	if (Q_stricmp (cmd, "give") == 0)
		...
		Cmd_Stats_f( ent );
	// CORKSCREW add item command
	else if (Q_stricmp (cmd, "additem") == 0)
		Cmd_AddItem_f( ent );
	else if (Q_stricmp (cmd, "skirmish") == 0);
	else if (Q_stricmp (cmd, "randomskirmish") == 0);
	else
