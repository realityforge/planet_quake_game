...
/*
	...
	*/
	...
	return 100;	// instagib == aggression
}
...
int BotCanAndWantsToRocketJump(bot_state_t *bs) {
	...
	if (!g_railgunExplosions.integer) return qfalse;
	/*
...
==================
BotHeadHunt
CorkScrew: headhunt, find the nearest head and go get it
also make the bot shoot bodies to get heads
search for a body that ain't friendly and has a head
if nearby, shoot it to get the head
==================
*/
#define HEADHUNT_INTERVAL 2		// seconds between searching for a head

void BotHeadHunt(bot_state_t *bs) {
	int			i, j, areanum;
	gentity_t	*ent;
	vec3_t		dir;
	bsp_trace_t trace;
	...
	// no heat hunting, dûh!
	if( !g_headHunt.integer ) return;

// not working any better than making the head an item	:-/
/*
	// find the nearest enemy head and go get it!
	if( bs->headhunt_time < FloatTime() - HEADHUNT_INTERVAL ) {
		bs->headhunt_time = FloatTime();

		trap_BotGetTopGoal(bs->gs, &goal);

		if( goal.iteminfo != ET_HEAD ) {
			distance = 999999999;

			for( i = MAX_CLIENTS; i < MAX_GENTITIES; i++ ) {
				ent = &g_entities[i];
				if( ent->s.eType == ET_HEAD && ent->s.otherEntityNum != bs->client && !BotSameTeam( bs, ent->s.otherEntityNum ) ) {
					VectorSubtract( bs->origin, ent->s.pos.trBase, dir );
					temp = VectorLengthSquared( dir );
					if( temp < distance ) {
						distance = temp;
						j = i;
					}
			...
			if( distance < Square(2000) ) {		// if we found a head not too far away, go for it!
				ent = &g_entities[j];
				areanum = BotPointAreaNum( ent->s.pos.trBase );
				// it should be reachable by the bot, aka have a areanum
				if ( areanum ) {
					VectorCopy( ent->s.pos.trBase, goal.origin );
					goal.areanum = areanum;
					VectorSet (goal.mins, -ITEM_RADIUS, -ITEM_RADIUS, -ITEM_RADIUS);
					VectorSet (goal.maxs, ITEM_RADIUS, ITEM_RADIUS, ITEM_RADIUS);
					goal.number = 0;
					goal.iteminfo = ET_HEAD;
					goal.entitynum = j;
					// go get it!
...
*/
	// don't shoot bodies if there is no gain in shooting bodies
	if( !g_headFromCorpse.integer ) return;

	for( i = 0; i < BODY_QUEUE_SIZE; i++ ) {
		ent = level.bodyQue[i];

		// body must still be there, and it must not be friendly
		if( ent->takedamage && ent->r.linked && !( ent->s.eFlags & EF_HEADSHOT ) && !BotSameTeam( bs, ent->s.clientNum ) && ent->s.clientNum != bs->client ) {
			VectorSubtract( ent->s.pos.trBase, bs->eye, dir );
			dir[2] -= 24;

			if( VectorLengthSquared( dir ) < Square(500) ) {	// only shoot nearby bodies
				BotAI_Trace(&trace, bs->eye, mins, maxs, ent->s.pos.trBase, bs->entitynum, MASK_SHOT);

				if( trace.ent == ( ent - g_entities ) || trace.fraction == 1 ) {	// don't shoot walls dammit!
					bs->flags |= BFL_IDEALVIEWSET;
					vectoangles(dir, bs->ideal_viewangles);

					if (InFieldOfVision(bs->viewangles, 5, bs->ideal_viewangles)) {
						trap_EA_Attack(bs->client);
						return;
					}
				}
	...
	for( i = 0; i < MAX_CLIENTS; i++ ) {
		...
		// search for corpses of players who haven't respawned yet, this makes for the realistic
		// behaviour of shooting the players body just after killing them
		if( ent->r.contents == CONTENTS_CORPSE && !( ent->s.eFlags & EF_HEADSHOT ) && !BotSameTeam( bs, ent->s.clientNum ) && ent->s.clientNum != bs->client ) {
			VectorSubtract( ent->s.pos.trBase, bs->eye, dir );
			dir[2] -= 24;
