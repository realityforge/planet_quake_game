...
void ClearRegisteredItems( void ) {
	...
	itemRegistered[ BG_FindItemForWeapon( WP_RAILGUN ) - bg_itemlist ] = qtrue;
	itemRegistered[ BG_FindItemForWeapon( WP_GRAPPLING_HOOK ) - bg_itemlist ] = qtrue;
	if ( g_allowGrapple.integer ) 	itemRegistered[ BG_FindItemForWeapon( WP_GRAPPLING_HOOK ) - bg_itemlist ] = qtrue;
}
...
void RegisterItem( gitem_t *item, qboolean always ) {
	...
	if ( item->giType != IT_TEAM && item->giType != IT_POWERUP && !always) {
		return;
	}
...
void G_RunItem( gentity_t *ent ) {
	...
	if ( ent->s.pos.trType == TR_STATIONARY || !Q_stricmp( ent->classname, "bodyque" ) ) {
