...
//	ent->s.pos.trBase[2] -= 0.5;
	if ( level.time - ent->timestamp < 5100) {
		ent->s.pos.trDelta[0] = 0;
		ent->s.pos.trDelta[1] = 0;
		ent->s.pos.trDelta[2] = -15;
...
void CopyToBodyQue( gentity_t *ent ) {
	...
	body->s = ent->s;
	if ( body->s.eFlags & EF_HEADSHOT )
		body->s.eFlags = EF_DEAD | EF_HEADSHOT;
		else body->s.eFlags = EF_DEAD;		// clear EF_TALK, etc
	...
	body->physicsBounce = 0.1;		// don't bounce (ok then, bounce a little)
	...
	body->s.generic1 = ent->client->sess.sessionTeam;
