...
draws player head, spinning like items, enlarged
===========
*/
void CG_Head( centity_t *cent ) {
	refEntity_t	ent;
	clientInfo_t	*ci;

	memset( &ent, 0, sizeof( ent ) );

	ci = &cgs.clientinfo[ cent->currentState.otherEntityNum ];

	VectorCopy( cent->lerpOrigin, ent.origin );
	ent.origin[2] += 4 + cos( ( cg.time ) *  0.005 ) * 4;
	VectorCopy( ent.origin, ent.oldorigin );

	ent.hModel = ci->headModel;
	ent.customSkin = ci->headSkin;

	AxisCopy( cg.autoAxis, ent.axis );
	VectorScale( ent.axis[0], 1.7, ent.axis[0] );
	VectorScale( ent.axis[1], 1.7, ent.axis[1] );
	VectorScale( ent.axis[2], 1.7, ent.axis[2] );
	ent.nonNormalizedAxes = qtrue;

	trap_R_AddRefEntityToScene( &ent );

	ent.shaderRGBA[0] = sin( cg.time * 0.01 ) * 127 + 127;
	ent.shaderRGBA[1] = ent.shaderRGBA[0];
	ent.shaderRGBA[2] = ent.shaderRGBA[0];
	ent.shaderRGBA[3] = 1;
	if( cent->currentState.generic1 == TEAM_RED ) ent.customShader = cgs.media.quadboomRed;
		else ent.customShader = cgs.media.quadboomBlue;
...
//	int	col;
	...
	if ( s1->weapon == WP_GRAPPLING_HOOK ) return; // added with the trail
...
static void CG_Grapple( centity_t *cent ) {
	...
	// no more missile trail, so call it here
...
static void CG_AddCEntity( centity_t *cent ) {
	...
	case ET_PLAYER:
		...
		break;
	case ET_ITEM:
		if ( bg_itemlist[ cent->currentState.modelindex ].giType == IT_POWERUP &&
				bg_itemlist[ cent->currentState.modelindex ].giTag == PW_HEAD ) {
			CG_Head( cent );
		} else {
			CG_Item( cent );
		}
