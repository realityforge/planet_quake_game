...
void CG_NewClientInfo( int clientNum ) {
	...
	CG_ColorFromString( va( "%c", v[0] ), newInfo.color );
	newInfo.c1 = atoi( va( "%c", v[0] ) );

	if( newInfo.c1 < 1 || newInfo.c1 > 7 ) newInfo.c1 = 7;

	if ( strlen( v ) > 2 ) {
		CG_ColorFromString( va( "%c", v[2] ), newInfo.color1 );
		CG_ColorFromString( va( "%c", v[4] ), newInfo.color2 );
		newInfo.c2 = atoi( va( "%c", v[2] ) );
		newInfo.c3 = atoi( va( "%c", v[4] ) );
		if( newInfo.c2 < 1 || newInfo.c2 > 7 ) newInfo.c2 = 7;
		if( newInfo.c3 < 1 || newInfo.c3 > 7 ) newInfo.c3 = 7;
	} else {
		VectorCopy( newInfo.color, newInfo.color1);
		VectorCopy( newInfo.color, newInfo.color2);
		newInfo.c2 = newInfo.c1;
		newInfo.c3 = newInfo.c1;
	}
	...
	if ( newInfo.botSkill ) {
		int temp;
		temp = clientNum % 7 + 1; // dude, colors range from 1 to 7 inclusive... >:(
		CG_ColorFromString( va("%i", temp), newInfo.color );
		VectorCopy( newInfo.color, newInfo.color1);
		VectorCopy( newInfo.color, newInfo.color2);
		newInfo.c3 = newInfo.c2 = newInfo.c1 = temp;
	}

	if ( cgs.gametype >= GT_TEAM ) {
		if ( newInfo.team == TEAM_RED ) {
			VectorCopy( colorRed, newInfo.color );
			VectorCopy( colorRed, newInfo.color2 );
			newInfo.c1 = 4;
			// c2 can still be set by user
			newInfo.c3 = 4;
		} else {
			VectorCopy( colorBlue, newInfo.color );
			VectorCopy( colorBlue, newInfo.color2 );
			newInfo.c1 = 1;
			// c2 can still be set by user
			newInfo.c3 = 1;
		}
...
static void CG_PlayerSprites( centity_t *cent ) {
	...
	if ( cent->currentState.eFlags & EF_AWARD_HEADSHOT ) {
		CG_PlayerFloatSprite( cent, cgs.media.medalFrags );
		return;
	}
...
void CG_Player( centity_t *cent ) {
	...
	// switching teams doesn't update clientinfo, so do it here
	if ( cgs.gametype >= GT_TEAM ) {
		if ( ci->team == TEAM_RED && ci->c1 != 4 ) {
			ci->c1 = 4;
			ci->c3 = 4;
			VectorCopy( colorRed, ci->color );
			VectorCopy( colorRed, ci->color2 );
		} else if ( ci->team == TEAM_BLUE && ci->c1 != 1 ) {
			ci->c1 = 1;
			ci->c3 = 1;
			VectorCopy( colorBlue, ci->color );
			VectorCopy( colorBlue, ci->color2 );
		}
	}
	...
	// add the head
	//
	if ( cent->currentState.eFlags & EF_HEADSHOT ) return;
