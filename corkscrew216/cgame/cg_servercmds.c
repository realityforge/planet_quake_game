...
void CG_ParseServerinfo( void ) {
	...
	// CORKSCREW
	cgs.accuracyChallenge = atoi( Info_ValueForKey( info, "g_accuracyChallenge" ) );
	cgs.headhunt = atoi( Info_ValueForKey( info, "g_headHunt" ) );
	cgs.valkyrMode = atoi( Info_ValueForKey( info, "g_valkyrMode" ) ) && !cgs.accuracyChallenge;
}
...
static void CG_MapRestart( void ) {
	...
	if ( cg.warmup == 0 && !cgs.accuracyChallenge /* && cgs.gametype == GT_TOURNAMENT */) {
