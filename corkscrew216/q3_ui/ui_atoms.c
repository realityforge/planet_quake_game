...
=================
*/
void UI_GetTrippyColor( int offset, vec4_t color ) {
	int time;

	if( VectorCompare( color, text_color_disabled ) ) return ;

	time = uis.realtime + offset;
	if ( time % 6000 < 2000 ) {
		color[2] = ( time % 6000 ) / 2000.0;
		color[1] = 0;
		color[0] = 1 - ( time % 6000 ) / 2000.0;
	} else if ( time % 6000 < 4000 ) {
		color[1] = ( ( time % 6000 ) - 2000 ) / 2000.0;
		color[0] = 0;
		color[2] = 1 - ( ( time % 6000 ) - 2000 ) / 2000.0;
	} else {
		color[0] = ( ( time % 6000 ) - 4000 ) / 2000.0;
		color[2] = 0;
		color[1] = 1 - ( ( time % 6000 ) - 4000 ) / 2000.0;
	}
...
don't allow sounds if soundtime > uis.realtime
=================
*/
int				soundTime;
...
void UI_DrawProportionalString( int x, int y, const char* str, int style, vec4_t color ) {
	...
	VectorCopy( color, trippyColor )
	if( trippyMenu ) {
		UI_GetTrippyColor( x * 10 + y * 10, trippyColor );
		trippyColor[3] = color[3];
	} else {
		trippyColor[3] = color[3];
	}
	...
	if ( style & UI_DROPSHADOW ) {
		drawcolor[0] = drawcolor[1] = drawcolor[2] = 0;
		drawcolor[3] = trippyColor[3];
		UI_DrawProportionalString2( x+2, y+2, str, drawcolor, sizeScale, uis.charsetProp );
	}

	if ( style & UI_INVERSE ) {
		drawcolor[0] = trippyColor[0] * 0.7;
		drawcolor[1] = trippyColor[1] * 0.7;
		drawcolor[2] = trippyColor[2] * 0.7;
		drawcolor[3] = trippyColor[3];
	...
	if ( style & UI_PULSE ) {
		...
		drawcolor[2] = trippyColor[2] * 0.7;
		drawcolor[3] = trippyColor[3];
		UI_DrawProportionalString2( x, y, str, trippyColor, sizeScale, uis.charsetProp );

		drawcolor[0] = trippyColor[0];
		drawcolor[1] = trippyColor[1];
		drawcolor[2] = trippyColor[2];
...
void UI_DrawString( int x, int y, const char* str, int style, vec4_t color )
{
	...
	float	*drawcolor;
	vec4_t	dropcolor;
	char	cleanString[MAX_STRING_CHARS];
	...
	if (style & UI_PULSE)
	{
		lowlight[0] = 0.8*trippyColor[0];
		lowlight[1] = 0.8*trippyColor[1];
		lowlight[2] = 0.8*trippyColor[2];
		lowlight[3] = 0.8*trippyColor[3];
		UI_LerpColor(trippyColor,lowlight,newcolor,0.5+0.5*sin(uis.realtime/PULSE_DIVISOR));
		drawcolor = newcolor;
	}
	else
		drawcolor = trippyColor;



	Q_strncpyz( cleanString, str, sizeof(cleanString) );
	Q_CleanStr( cleanString );
...
qboolean UI_ConsoleCommand( int realTime ) {
	...
	// allow randomskirmish to be executed from the console or from the QAGAME
	if ( Q_stricmp (cmd, "randomskirmish") == 0 ) {
		UI_RandomSkirmish( trap_Cvar_VariableValue( "sv_running" ) );
	}

	if ( Q_stricmp (cmd, "skirmish") == 0 ) {
		buf = UI_Argv( 1 );
		if( buf[0] ) {
			UI_SkirmishByMap( buf, trap_Cvar_VariableValue( "sv_running" ) );
		}
	}

	if ( Q_stricmp (cmd, "accuracychallenge") == 0 ) {
	...
	/** CORKSCREW trippy menu >:) **/

	if ( Q_stricmp (cmd, "iamfirestarter") == 0 ) {
		trippyMenu = (trippyMenu == 0);
		return qtrue;
	}

	/** i don't fucking believe it, but what the fuck... :/
	if ( Q_stricmp (cmd, "thelongestyard") == 0 ) {
		trap_Cvar_SetValue( "fraglimit", 150 );
		trap_Cvar_SetValue( "timelimit", 0 );
		trap_Cvar_SetValue( "g_gametype", 0 );
		trap_Cvar_SetValue( "g_spawnProtection", 0 );
...
void UI_Refresh( int realtime )
{
	...
	if ( trap_Cvar_VariableValue( "trippymenu" ) ) trippyMenu = qtrue;
