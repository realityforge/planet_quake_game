...
/*
=================
InGame_SkirmishAction
=================
*/
static void InGame_SkirmishAction( qboolean result ) {
	if( !result ) {
		return;
	}

	UI_PopMenu();
	UI_RandomSkirmish( qtrue );	// qtrue -> we are in a game, so kick all the bots etc.
}
...
static void InGame_RestartAction( qboolean result ) {
	...
	UI_PopMenu();
	if( trap_Cvar_VariableValue( "g_accuracyChallenge" ) == 1 ) {
...
static void InGame_LeaveArenaAction( qboolean result ) {
	if( !result ) {
		return;
	}
	UI_PopMenu();
...
void InGame_Event( void *ptr, int notification ) {
	...
	// CORKSCREW function in ui_main.c
	case ID_RANDOMSKIRMISH:
		if ( trap_Cvar_VariableValue( "ui_randomskirmish" ) ) {	// don't ask if already in a random skirmish
			UI_PopMenu();
			UI_RandomSkirmish( qtrue );
		} else UI_ConfirmMenu( "ARE YOU SURE?", NULL, InGame_SkirmishAction );
		break;
	...
	case ID_LEAVEARENA:
		UI_ConfirmMenu( "LEAVE ARENA?", NULL, InGame_LeaveArenaAction );
		break;
...
void InGame_MenuInit( void ) {
	...
	s_ingame.banner.generic.flags	= QMF_CENTER_JUSTIFY;
	s_ingame.banner.generic.x		= 320;
	s_ingame.banner.generic.y		= 16;
	s_ingame.banner.string			= "THE CORKSCREW MOD";
	...
	y += INGAME_MENU_VERTICAL_SPACING;
	s_ingame.randomskirmish.generic.type		= MTYPE_PTEXT;
	s_ingame.randomskirmish.generic.flags		= QMF_CENTER_JUSTIFY|QMF_PULSEIFFOCUS;
	s_ingame.randomskirmish.generic.x			= 320;
	s_ingame.randomskirmish.generic.y			= y;
	s_ingame.randomskirmish.generic.id			= ID_RANDOMSKIRMISH;
	s_ingame.randomskirmish.generic.callback	= InGame_Event;
	s_ingame.randomskirmish.string				= "RANDOM SKIRMISH";
	s_ingame.randomskirmish.color				= color_yellow;
	s_ingame.randomskirmish.style				= UI_CENTER|UI_SMALLFONT;
	if( !trap_Cvar_VariableValue( "sv_running" ) || !trap_Cvar_VariableValue( "bot_enable" ) || (trap_Cvar_VariableValue( "g_gametype" ) == GT_SINGLE_PLAYER)) {
		s_ingame.randomskirmish.generic.flags |= QMF_GRAYED;
	}
	...
	Menu_AddItem( &s_ingame.menu, &s_ingame.randomskirmish );
