...
#define ID_RAILSCREW		139
#define ID_EXPLOSIONS		140
#define ID_DRAWHUD			141
#define ID_DRAWKILLS		142
...
typedef struct {
	...
	menuradiobutton_s	railscrew;
	menuradiobutton_s	explosions;
	menuradiobutton_s	drawhud;
	menuradiobutton_s	drawkills;
	menufield_s			particles;
...
static void Preferences_SetMenuItems( void ) {
	s_preferences.railscrew.curvalue		= trap_Cvar_VariableValue( "cg_railscrew" ) != 0;
	s_preferences.explosions.curvalue		= trap_Cvar_VariableValue( "cg_railgunExplosions" ) != 0;
	s_preferences.drawhud.curvalue			= trap_Cvar_VariableValue( "cg_drawHUD" ) != 0;
	s_preferences.drawkills.curvalue		= trap_Cvar_VariableValue( "cg_drawKill" ) != 0;
	Com_sprintf( s_preferences.particles.field.buffer, 4, "%i", (int)Com_Clamp( 0, 999, trap_Cvar_VariableValue( "cg_explosionParticles" ) ) );
...
static void Preferences_Event( void* ptr, int notification ) {
	...
	case ID_RAILSCREW:
		trap_Cvar_SetValue( "cg_railscrew", s_preferences.railscrew.curvalue );
		break;

	case ID_EXPLOSIONS:
		trap_Cvar_SetValue( "cg_railgunExplosions", s_preferences.explosions.curvalue );
		break;

	case ID_DRAWHUD:
		trap_Cvar_SetValue( "cg_drawHUD", s_preferences.drawhud.curvalue );
		break;

	case ID_DRAWKILLS:
		trap_Cvar_SetValue( "cg_drawKill", s_preferences.drawkills.curvalue );
		break;
	...
	case ID_BACK:
		trap_Cvar_SetValue( "cg_explosionParticles", Com_Clamp( 0, 999, atof( s_preferences.particles.field.buffer ) ) );
		UI_PopMenu();
		break;
	}
}
...
static void Preferences_MenuInit( void ) {
	...
	y += 2*BIGCHAR_HEIGHT+4;
	s_preferences.drawkills.generic.type        = MTYPE_RADIOBUTTON;
	s_preferences.drawkills.generic.name	      = "Draw kill messages:";
	s_preferences.drawkills.generic.flags	      = QMF_PULSEIFFOCUS|QMF_SMALLFONT;
	s_preferences.drawkills.generic.callback    = Preferences_Event;
	s_preferences.drawkills.generic.id          = ID_DRAWKILLS;
	s_preferences.drawkills.generic.x	          = PREFERENCES_X_POS;
	s_preferences.drawkills.generic.y	          = y;

	y += BIGCHAR_HEIGHT+2;
	s_preferences.particles.generic.type		= MTYPE_FIELD;
	s_preferences.particles.generic.name		= "Explosion particles:";
	s_preferences.particles.generic.flags		= QMF_NUMBERSONLY|QMF_PULSEIFFOCUS|QMF_SMALLFONT;
	s_preferences.particles.generic.x			= PREFERENCES_X_POS;
	s_preferences.particles.generic.y			= y;
	s_preferences.particles.field.widthInChars	= 3;
	s_preferences.particles.field.maxchars		= 3;

	y += BIGCHAR_HEIGHT+2;
	s_preferences.railscrew.generic.type        = MTYPE_RADIOBUTTON;
	s_preferences.railscrew.generic.name	      = "3D railtrail:";
	s_preferences.railscrew.generic.flags	      = QMF_PULSEIFFOCUS|QMF_SMALLFONT;
	s_preferences.railscrew.generic.callback    = Preferences_Event;
	s_preferences.railscrew.generic.id          = ID_RAILSCREW;
	s_preferences.railscrew.generic.x	          = PREFERENCES_X_POS;
	s_preferences.railscrew.generic.y	          = y;

	y += BIGCHAR_HEIGHT+2;
	s_preferences.explosions.generic.type        = MTYPE_RADIOBUTTON;
	s_preferences.explosions.generic.name	      = "Explosions:";
	s_preferences.explosions.generic.flags	      = QMF_PULSEIFFOCUS|QMF_SMALLFONT;
	s_preferences.explosions.generic.callback    = Preferences_Event;
	s_preferences.explosions.generic.id          = ID_EXPLOSIONS;
	s_preferences.explosions.generic.x	          = PREFERENCES_X_POS;
	s_preferences.explosions.generic.y	          = y;
	...
	Menu_AddItem( &s_preferences.menu, &s_preferences.drawkills );
	Menu_AddItem( &s_preferences.menu, &s_preferences.particles );
	Menu_AddItem( &s_preferences.menu, &s_preferences.railscrew );
	Menu_AddItem( &s_preferences.menu, &s_preferences.explosions );
