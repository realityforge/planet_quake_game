...
void P_WorldEffects( gentity_t *ent ) {
	...
	// check for drowning
	//
	if ( waterlevel == 3 && !g_aqua.integer) {
		// envirosuit give air
		if ( envirosuit ) {
...
int qpong_PointContents( const vec3_t point, int passEntityNum ) {
	int cont = trap_PointContents(point, passEntityNum);

	if (g_aqua.integer){
		return (cont | CONTENTS_WATER);
	}else{
		return cont;
	}
}
...
//		G_Printf("serverTime >>>>>\n" );
	...
	if (g_speedy.integer){
		client->ps.weaponTime *= 0.9f;
		client->ps.speed *= 1.5;
	}

	if (g_aqua.integer){
		// haq haq! increase speed to counteract water friction
		client->ps.speed *= 1.3;
	}
	...
	pm.trace = trap_Trace;
	pm.pointcontents = qpong_PointContents;
