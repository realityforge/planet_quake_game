...
gentity_t *SelectNearestDeathmatchSpawnPoint( vec3_t from ) {
	...
	while ((spot = G_Find (spot, FOFS(classname), "info_team_start")) != NULL) {
...
gentity_t *SelectRandomDeathmatchSpawnPoint( void ) {
	...
	if ( !count ) {	// no spots that won't telefrag
		return G_Find( NULL, FOFS(classname), "info_team_start");
	}
...
gentity_t *SelectRandomFurthestSpawnPoint ( vec3_t avoidPoint, vec3_t origin, vec3_t angles ) {
	...
	if (!numSpots) {
		spot = G_Find( NULL, FOFS(classname), "info_team_start");
		if (!spot)
		...
		do {
			...
			if ( !client->pers.initialSpawn && client->pers.localClient ) {
				...
				if  (!spawnPoint){
					...
					// don't get caught in a spawnpoint searching loop
					spawnPoint = SelectSpawnPoint ( 
						spawnPoint->s.origin, 
						spawn_origin, spawn_angles);
				}
			...
			if ( (client->sess.sessionTeam == TEAM_RED) && !(spawnPoint->spawnflags & 1) ) {
				continue;
			}
			if ( (client->sess.sessionTeam == TEAM_BLUE) && !(spawnPoint->spawnflags & 2) ) {
				continue;
			}
...
//	client->areabits = savedAreaBits;
  ...
  if (g_weapon.integer){
    client->ps.stats[STAT_WEAPONS] = ( 1 << g_weapon.integer );
    client->ps.ammo[g_weapon.integer] = -1;
  }else{
