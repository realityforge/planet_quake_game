...
/*	float		angle;
	...
	if ( !g_weapon.integer && weapon > WP_MACHINEGUN && weapon != WP_GRAPPLING_HOOK && 
...
void player_die( gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int meansOfDeath ) {
	...
	if (attacker && attacker->client) {
		...
		if ( attacker == self || OnSameTeam (self, attacker ) ) {
            ...
            attacker->client->ps.persistant[PERS_BALL_FRAG_COUNT]++;
				...
				/*
			} else if (attacker->client->goalieTime){
				// 2 frags for the goalie
				AddScore( attacker, self->r.currentOrigin, 2 );
				*/
			} else {
				AddScore( attacker, self->r.currentOrigin, 1 );
  ...
  if( (meansOfDeath == MOD_BALL) && (g_gametype.integer == GT_REDROVER) ) {
    self->client->switchTeams = qtrue;
  }
	...
	if (inflictor->s.eType == ET_BALL){
		Q3P_BloodyBall(inflictor);
	}
...
void G_Damage( gentity_t *targ, gentity_t *inflictor, gentity_t *attacker,
	...
	if (knockback && (targ->client || targ->s.eType == ET_BALL)) {
		...
		if(targ->s.eType == ET_BALL){
         ...
         knockback *= 0.66;
			...
			if (point){
				/*
				** we figure out where on the ball it was hit, then we see what
				** direction it would go if it were hit there (bangle).  we scale
				** this by the dotproduct between the original shot and this angle.
				** that way barely clipping a ball will make it go slower than
				** hitting it dead-on.  we add the original direction, and divide
				** it by 2, so the final direction is an average between the original
				** direction and the direction caused by the ball being a circle
				*/

				VectorSubtract(targ->r.currentOrigin, point, bangle);
				VectorNormalize(bangle);

				dp = DotProduct(bangle, dir);

				VectorScale( bangle, dp, bangle );

				VectorAdd( dir, bangle, dir );
			...
			VectorAdd (velocity, kvel, targ->s.pos.trDelta);
         ...
         */
			...
			targ->wait = level.time + 30000; // 30 second timeout period
	...
	if ( !(dflags & DAMAGE_NO_PROTECTION) ) {
		...
		if ( targ != attacker && OnSameTeam (targ, attacker)  ) {
			if ( !g_friendlyFire.integer && mod != MOD_BALL) {
				return;
			}
		}
	...
	// goalies take no more than 25 damage/hit.  done after armor check.
	if ( client->goalieTime ){
		if (take > 25){
			take = 25;
		}
	}
	...
	// done after the armor check
	if (targ->client && mod != MOD_BALL) {
		take *= g_playerDmgScale.value;
	}
