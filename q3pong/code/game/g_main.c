...
cvarTable_t		gameCvarTable[] = {
	...
	// latched vars
	{ &g_gametype, "g_gametype", "", CVAR_SERVERINFO | CVAR_LATCH, 0, qfalse  },
...
void G_RegisterCvars( void ) {
	...
	// check some things
	if ( g_gametype.integer < 0 || g_gametype.integer >= GT_MAX_GAME_TYPE || g_gametype.integer == 1 || g_gametype.integer == 2 ) {
...
void CalculateRanks( void ) {
	...
	*/
	...
	level.teamScores[TEAM_RED] = q3p_levelData.redPoints;
	level.teamScores[TEAM_BLUE] = q3p_levelData.bluePoints;
...
void CheckIntermissionExit( void ) {
	...
	// if most people want to go, go now
	if ( ready > notReady ) {
		ExitLevel();
		return;
	}
