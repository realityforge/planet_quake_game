...
void CG_Draw3DCrosshair(void){
	...
	VectorSubtract(start, trace.endpos, delta);

	re.customShader = hShader;
	VectorCopy(trace.endpos, re.origin);
	re.radius = VectorLength(delta) * w / 1024;
	re.reType = RT_SPRITE;
	re.shaderRGBA[0] = hcolor[0] * 255;
	re.shaderRGBA[1] = hcolor[1] * 255;
	re.shaderRGBA[2] = hcolor[2] * 255;
	re.shaderRGBA[3] = 255;
