...
void CG_Trace( trace_t *result, vec3_t start, vec3_t mins, vec3_t maxs, vec3_t end, int skipNumber, int mask ) {
	trace_t	t;
	trap_CM_BoxTrace ( &t, start, end, mins, maxs, 0, mask);
	t.entityNum = t.fraction != 1.0 ? ENTITYNUM_WORLD : ENTITYNUM_NONE;
	CG_ClipMoveToEntities (start, mins, maxs, end, skipNumber, mask, &t); // check all other solid models
	*result = t;
}
...
static void CG_InterpolatePlayerState( qboolean grabAngles ) {
	...
	*out = cg.snap->ps;
	...
	if ( cg.nextFrameTeleport ) { // if the next frame is a teleport, we can't lerp to it
		return;
	}
...
static void CG_TouchItem( centity_t *cent ) {
	...
	if ( cent->miscTime == cg.time ) { // never pick an item up twice in a prediction
		return;
	}
	...
	BG_AddPredictableEventToPlayerstate( EV_ITEM_PICKUP, cent->currentState.modelindex , &cg.predictedPlayerState); // grab it
	cent->currentState.eFlags |= EF_NODRAW; // remove it from the frame so it won't be drawn
	cent->miscTime             = cg.time;   // don't touch it again this prediction
	if ( item->giType == IT_WEAPON ) { // if it's a weapon, give them some predicted ammo so the autoswitch will work
...
static void CG_TouchTriggerPrediction() {
	...
	centity_t	*cent;
	qboolean	spectator;
	if ( cg.predictedPlayerState.stats[STAT_HEALTH] <= 0 ) { // dead clients don't activate triggers
		return;
	}
...
void CG_PredictPlayerState( void ) {
	...
	if ( cg_pmove_msec.integer < 8 ) {
		Cvar_SetSafe("cg_pmove_msec", "8");
	}
	else if (cg_pmove_msec.integer > 33) {
		Cvar_SetSafe("cg_pmove_msec", "33");
	}
