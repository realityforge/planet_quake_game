...
int (*callback_CG_ServerCommand)() = NULL;
CCALL void set_callback_CG_ServerCommand(int (*cb)()) {
	callback_CG_ServerCommand = cb;
}
...
static void CG_ServerCommand( void ) {
	...
	if (callback_CG_ServerCommand && callback_CG_ServerCommand()) {
		return;
	}
	...
	if ( !strcmp( cmd, "cp" ) ) {
		char *text = (char *)CG_Argv(1);
		//CG_CenterPrint(text, SCREEN_HEIGHT * 0.30, BIGCHAR_WIDTH );

#if EMSCRIPTEN
			EM_ASM_({
				cg_servercommand_centerprint($0);
			}, text);
#endif
