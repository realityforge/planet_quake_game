...
CCALL void SV_SendServerCommand( client_t *cl, const char *fmt, ...) __attribute__ ((format (printf, 2, 3)));
CCALL void SV_AddOperatorCommands (void);
CCALL void SV_RemoveOperatorCommands (void);
CCALL void SV_MasterShutdown (void);
CCALL int SV_RateMsec(client_t *client);
...
CCALL void SV_SpawnServer( char *server, qboolean killBots );
...
#ifndef STANDALONE
CCALL void SV_AuthorizeIpPacket( netadr_t from );
#endif
...
CCALL void SV_FreeClient(client_t *client);
...
CCALL int SV_WriteDownloadToClient(client_t *cl , msg_t *msg);
CCALL int SV_SendDownloadMessages(void);
CCALL int SV_SendQueuedMessages(void);
...
CCALL void		SV_ShutdownGameProgs ( void );
CCALL void		SV_RestartGameProgs( void );
...
CCALL int			SV_BotGetSnapshotEntity( int client, int ent );
...
CCALL void SV_Trace( trace_t *results, const vec3_t start, vec3_t mins, vec3_t maxs, const vec3_t end, int passEntityNum, int contentmask, int capsule );
...
// sv_net_chan.c
//
CCALL void SV_Netchan_Transmit( client_t *client, msg_t *msg);
CCALL int SV_Netchan_TransmitNextFragment(client_t *client);
CCALL qboolean SV_Netchan_Process( client_t *client, msg_t *msg );
CCALL void SV_Netchan_FreeQueue(client_t *client);
...
CCALL qboolean	SV_EntityContact(vec3_t mins, vec3_t maxs, const sharedEntity_t* gEnt, int capsule);
...
CCALL void SV_ShutdownGameProgs();
CCALL void SV_InitGameVM(qboolean restart);
CCALL void SV_RestartGameProgs();
CCALL void SV_InitGameProgs();
CCALL qboolean ConsoleCommand();
CCALL qboolean SV_GameCommand();
