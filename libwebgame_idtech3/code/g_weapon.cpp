...
void Bullet_Fire (gentity_t *ent, float spread, int damage, int mod ) {
	...
	float distance = 8192*16;

	// TODO: GENERALIZE: weapon->maxDistance or something
	if (mod == MOD_CROWBAR || mod == MOD_SWORD)
		distance = 64;

	VectorMA (muzzle, distance, forward, end);
	if (mod != MOD_GAUNTLET) {
	...
	//for (i = 0; i < 10; i++) // only for bounced rails, not needed and should be g_weapon_bla.c function 
	{

		trap_Trace (&tr, muzzle, NULL, NULL, end, passent, MASK_SHOT);

		//Com_Printf("Bullet_Fire: tr.fraction=%f\n", tr.fraction);
		if ( tr.surfaceFlags & SURF_NOIMPACT ) {
			return;
		}
		if (tr.fraction == 1.0)
...
void (*callback_rocket)(int clientid, int rocketid);
CCALL void set_callback_rocket(void (*cb)(int, int)) {
	callback_rocket = cb;
}
...
void Weapon_RocketLauncher_Fire (gentity_t *ent) {
	...
	if (callback_rocket)
		callback_rocket(ent - g_entities, m - g_entities);
//	VectorAdd( m->s.pos.trDelta, ent->client->ps.velocity, m->s.pos.trDelta );	// "real" physics
}
...
serverweapon_t serverweapons[32];
...
//	}
	...
	serverweapon_t *serverweapon = serverweapons + ent->s.weapon;

	static int first = 1;
	if (first) {
		serverweapons[WP_BAZOOKA].fire = bazooka_fire;
		serverweapons[WP_BAZOOKA].pm_weapon = bazooka_pm_weapon;
		first = 0;
	}

	if (serverweapon->fire)
	{
		serverweapon->fire(ent);
		return;
	}

	switch( ent->s.weapon ) {
		case WP_KAR98           : Bullet_Fire( ent, 200, 40, MOD_KAR98 ); break;
		case WP_MP44            : Bullet_Fire( ent, 200, 5, MOD_MP44 ); break;
		case WP_SWORD           : Bullet_Fire( ent, 200, 50, MOD_SWORD); break;
		case WP_CROWBAR         : Bullet_Fire( ent, 200, 10, MOD_CROWBAR ); break;
