...
//	}
	...
	// dont send any shit to bots
	// meh, on maprestart gentity is NULL, so there is still shit send
...
CCALL void jl_SV_SendServerCommand(int clientid, char *cmd) {
	SV_SendServerCommand(svs.clients + clientid, cmd);
}
...
void SV_PacketEvent( netadr_t from, msg_t *msg ) {
	...
	for (i=0, cl=svs.clients ; i < sv_maxclients->integer ; i++,cl++) {
		...
		if (cl->netchan.remoteAddress.port!= from.port) { // only care about the real port, netchan.qport is some client-induced q3 bullshit via net_qport client cvar
...
static void SV_CheckTimeouts( void ) {
	...
	for (i=0,cl=svs.clients ; i < sv_maxclients->integer ; i++,cl++) {
		...
		if ( cl->state >= CS_CONNECTED && cl->lastPacketTime < droppoint) {
			...
			// cause a timeout
			if ( ! (cl->gentity->r.svFlags & SVF_BOT)) // dont kick our bots
