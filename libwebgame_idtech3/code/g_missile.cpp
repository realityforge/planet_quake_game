...
void G_MissileImpact( gentity_t *ent, trace_t *trace ) {
	...
	//Com_Printf("missile hit: %.2f %.2f %.2f\n", trace->endpos[0], trace->endpos[1], trace->endpos[2]);
	G_SetOrigin( ent, trace->endpos );
...
void G_RunMissile( gentity_t *ent ) {
	...
	// get the position where the rocket is moving to
	// I wanna keep the bullettrace short, only scan for the next 100ms... thats 2 frames at 20fps... should be fine?
	BG_EvaluateTrajectory( &ent->s.pos, level.time + 50, to );
	...
	//SV_Trace( &tr, ent->r.currentOrigin, NULL, NULL, origin, passent, ent->clipmask, qfalse );
	SV_Trace( &tr, origin, NULL, NULL, to, passent, ent->clipmask, qfalse );
	

	//vec3_t *a = &ent->r.currentOrigin;
	//vec3_t *b = &origin;
	//Com_Printf("tr.allsolid=%d tr.startsolid=%d tr.fraction=%f distance(r.currentOrigin, origin)=%f\n", tr.allsolid, tr.startsolid, tr.fraction, vec3_distance(a, b));
	//int trTime = ent->s.pos.trTime;
	//Com_Printf("tr.allsolid=%d tr.startsolid=%d tr.fraction=%f cg.time-trTime=%d\n", tr.allsolid, tr.startsolid, tr.fraction, cg.time-trTime);

	if ( tr.startsolid || tr.allsolid ) {
		// make sure the tr.entityNum is set to the entity we're stuck in
		//trap_Trace( &tr, ent->r.currentOrigin, ent->r.mins, ent->r.maxs, ent->r.currentOrigin, passent, ent->clipmask );
		SV_Trace( &tr, ent->r.currentOrigin, NULL, NULL, ent->r.currentOrigin, passent, ent->clipmask, qfalse);
		tr.fraction = 0;
	}
	else {
...
gentity_t *fire_rocket (gentity_t *self, vec3_t start, vec3_t dir) {
	...
	// rocket explodes on all walls with a value of around 15, but why the fuck this is so buggy... and how did this even work in q3 with values of mins/maxs=0,0,0?
	// in order to have working rockets, i disable mins/maxs by setting NULL... raycasting only
	//float size = test_x->value;
