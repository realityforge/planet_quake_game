...
CCALL int jl_entity_moveto(int id, vec3_t to, float durationSeconds) {
...
CCALL float *jl_entity_get_pos(int id) {
	gentity_t *ent = g_entities + id;
	if (ent->client == NULL) {
		return ent->r.currentOrigin;
	}
	return ent->client->ps.origin;
}

CCALL float *entity_get_position(int id) {
	gentity_t *ent = g_entities + id;
	if (ent->client == NULL) {
		return ent->r.currentOrigin;
	}
	//return ent->client->ps.origin;
	return cg_entities->lerpOrigin;
}

CCALL void player_forward(int playerid, float *out) {
	gentity_t *ent = g_entities + playerid;
	AngleVectors(ent->client->ps.viewangles, out, NULL, NULL);
}
...
#if DEBUG_MOVETO
	Com_Printf("id=%d to=%d durationSeconds=%f\n", ent-g_entities, to, durationSeconds);
	Com_Printf("vec: %f %f %f\n", to[0], to[1], to[2]);	
#endif
	if (durationSeconds <= 0.0) {
		G_Printf("durationMilliseconds must be > 0.0\n");
		return 0;
	}
...
int getEye(gentity_t *player, vec3_t out_eye) {
	...
	if (player->client == NULL) {
		G_Printf("call getEye only on player entity\n");
		return 0;
	}
...
CCALL int jl_entity_send_hud(int playerid, int left, int top, int width, int height, float r, float g, float b, float a, char *text) {
	...
	if (clienthudelem == NULL) {
		Com_Printf("No clienthudelems left... clean some or increase limits\n");
