...
CCALL void SV_DropClientByID( int clientNum, const char *reason ) {
	SV_DropClient(svs.clients + clientNum, reason);
}

CCALL int SV_QuakeIDByPort(int port) {
	client_t *cl = NULL;
	for (int i=0; i < sv_maxclients->integer ; i++) {
		...
		if (cl->netchan.remoteAddress.port == port) {
			return i;
		}
	}
	return -1;
...
gotnewcl:	
	...
	*newcl = temp;
	...
	denied = (intptr_t)ClientConnect(clientNum, qtrue, qfalse);
	if ( denied ) {
		char *str = (char *)denied;
		NET_OutOfBandPrint( NS_SERVER, from, "print\n%s\n", str );
...
int (*callback_SV_DropClient)(int *player) = NULL;
CCALL void set_callback_SV_DropClient(int (*cb)(int *player)) {
	callback_SV_DropClient = cb;
}
...
void SV_DropClient( client_t *drop, const char *reason ) {
	...
	if (callback_SV_DropClient && callback_SV_DropClient((int *)player)) {
		return;
	}
