...
CCALL int G_AddBot() {
	...
	userinfo[0] = '\0';
	Info_SetValueForKey( userinfo, "name", "bot" );
	Info_SetValueForKey( userinfo, "rate", "25000" );
	Info_SetValueForKey( userinfo, "snaps", "20" );
	Info_SetValueForKey( userinfo, "skill", "5" );
	Info_SetValueForKey( userinfo, "handicap", "50" );
	Info_SetValueForKey( userinfo, "model", "visor/default" );
	Info_SetValueForKey( userinfo, "team_model", "visor/default" );
	...
	trap_SetUserinfo( clientNum, userinfo ); // register the userinfo
	if ( ClientConnect( clientNum, qtrue, qtrue ) ) { // have it connect to the game as a normal client
		return -1;
	}
	ClientBegin( clientNum );
	return clientNum;
...
static void G_SpawnBots( char *botList, int baseDelay ) {
	...
	skill = trap_Cvar_VariableValue( "g_spSkill" );
	if( skill < 1 ) {
		Cvar_SetSafe( "g_spSkill", "1" );
		skill = 1;
	}
	else if ( skill > 5 ) {
		Cvar_SetSafe( "g_spSkill", "5" );
		skill = 5;
	}
...
void G_InitBots( qboolean restart ) {
	...
	if( g_gametype.integer == GT_SINGLE_PLAYER ) {
		...
		if ( fragLimit ) {
			Cvar_SetSafe( "fraglimit", strValue );
		}
		else {
			Cvar_SetSafe( "fraglimit", "0" );
		}

		strValue = Info_ValueForKey( arenainfo, "timelimit" );
		timeLimit = atoi( strValue );
		if ( timeLimit ) {
			Cvar_SetSafe( "timelimit", strValue );
		}
		else {
