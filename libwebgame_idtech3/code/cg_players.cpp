...
static qboolean CG_RegisterClientModelname( clientInfo_t *ci, const char *modelName, const char *skinName, const char *headModelName, const char *headSkinName, const char *teamName ) {
	char	filename_upper[MAX_QPATH];
	char	filename_lower[MAX_QPATH];
	char	filename_head[MAX_QPATH];
	char	filename[MAX_QPATH];
	const char		*headName;
	char newTeamName[MAX_QPATH];

	//ci->legsModel = trap_R_RegisterModel( filename_lower );
	ci->legsModel = R_CustomMD3("rektman_lower", 363, 363, 191, 1);
	//ci->torsoModel = trap_R_RegisterModel( filename_upper );

	ci->torsoModel = R_CustomMD3("rektman_upper", 400, 1100, 152, 3);
	//ci->headModel = trap_R_RegisterModel( filename_head );
	ci->headModel = R_CustomMD3("rektman_head", 100, 30, 1, 1); // todo: remove numTags etc. for static shit
	return qtrue;
}
	...
	if ( !CG_RegisterClientModelname( ci, ci->modelName, ci->skinName, ci->headModelName, ci->headSkinName, teamname ) ) {
		...
		if( cgs.gametype >= GT_TEAM) {
			...
			if ( !CG_RegisterClientModelname( ci, DEFAULT_MODEL, "default", DEFAULT_MODEL, "default", teamname ) ) {
				...
				CG_Printf( "DEFAULT_MODEL (%s) failed to register", DEFAULT_MODEL );
			}
		}
		modelloaded = qfalse;
	}
...
CCALL const char *anim_to_string(animation_t *anim);
...
static void CG_RunLerpFrame( clientInfo_t *ci, lerpFrame_t *lf, int newAnimation, float speedScale ) {
	...
	#ifdef ENABLE_IMGUI
	if (imgui_cg_runlerpframe->integer) {
		ImGui::Begin("CG_RunLerpFrame");
		ImGui::PushID(lf);
		ImGui::Text("animationTime=%d backlerp=%f frame=%d frameTime=%d", lf->animationTime, lf->backlerp, lf->frame, lf->frameTime);
	}
	#endif
	...
	int toggleAnimation = (newAnimation & ANIM_TOGGLEBIT);
	animation_t	*old_anim = lf->animation;
	animation_t	*new_anim = ci->animations + (newAnimation & ~ANIM_TOGGLEBIT);
	
	animNumber_t old_anim_id = (animNumber_t)(old_anim - ci->animations);
	animNumber_t new_anim_id = (animNumber_t)(new_anim - ci->animations);

	int switchAnim = newAnimation != lf->animationNumber || !lf->animation;


	// rip q3 anim system, just wanna have the information which anim is played... saving played "anim" in "frame"
	lf->frame = new_anim_id;
	return;
	...
	int frameTimeDelta = lf->frameTime - lf->oldFrameTime;
	int cgtimeMinusFrameTime = cg.time - lf->frameTime;
	int cgtimeMinusOldFrameTime = cg.time - lf->oldFrameTime;

	//if (imgui_cg_runlerpframe->integer) {
	//	ImGui::Text("old_anim=%s new_anim=%s", anim_id_to_string(old_anim_id), anim_id_to_string(new_anim_id));
	//	ImGui::DragInt("frame", &lf->frame);
	//	ImGui::DragInt("frameTime", &lf->frameTime);
	//	ImGui::DragInt("oldFrame", &lf->oldFrame);
	//	ImGui::DragInt("oldFrameTime", &lf->oldFrameTime);
	//	ImGui::DragInt("animationTime", &lf->animationTime);
	//	ImGui::DragFloat("backlerp", &lf->backlerp);
	//	ImGui::DragInt("frameTimeDelta", &frameTimeDelta);
	//	ImGui::DragInt("cgtimeMinusFrameTime", &cgtimeMinusFrameTime);
	//	ImGui::DragInt("cgtimeMinusOldFrameTime", &cgtimeMinusOldFrameTime);
	//}

	int differentAnim = old_anim != new_anim; // e.g. going from TORSO_STAND to TORSO_ATTACK
	int switchToTorsoAttack = differentAnim && new_anim_id == TORSO_ATTACK;

	if (switchToTorsoAttack) {
		#ifdef ENABLE_IMGUI
		if (imgui_cg_runlerpframe->integer) {
			ImGui::Button("switchToTorsoAttack");
		}
		#endif

		lf->frame = new_anim->firstFrame + 1;
		lf->oldFrame = new_anim->firstFrame;
		lf->backlerp = 0;
		
		lf->frameTime = cg.time + 66;
...
static void CG_PlayerFlag( centity_t *cent, qhandle_t hSkin, refEntity_t *torso ) {
	...
	// dont execute, but leave for later study
	return;
...
// todo: turn cvars into handle references.. like assign model once via string and then just cvar->model for the model
cvar_t *cg_player_legs = NULL;
cvar_t *cg_player_body = NULL;
cvar_t *cg_player_head = NULL;
cvar_t *cg_player_weapon = NULL;
...
===============
*/
CCALL cvar_t *Cvar_Get( const char *var_name, const char *var_value, int flags );

// these are reset to 0 when map restarts, because the shader ids change
EXTERNC qhandle_t torsoshader_a = 0;
EXTERNC qhandle_t torsoshader_b = 0;
EXTERNC qhandle_t legshader_a = 0;
EXTERNC qhandle_t legshader_b = 0;

void CG_Player( centity_t *cent ) {

	if (cg_player_legs == NULL) {
	
		cg_player_legs = Cvar_Get( "cg_player_legs", "0", 0);
		cg_player_body = Cvar_Get( "cg_player_body", "1", 0);
		cg_player_head = Cvar_Get( "cg_player_head", "0", 0);
		cg_player_weapon = Cvar_Get( "cg_player_weapon", "0", 0);
	}
	...
	//legs.shaderRGBA[0] = 10;
	//legs.shaderRGBA[1] = 64;
	//legs.shaderRGBA[2] = 128;
	//legs.shaderRGBA[3] = 255;

	qhandle_t legshader = 0;

	if (ci->team == TEAM_RED) {
		if (legshader_a == 0)
			legshader_a = trap_R_RegisterShader("rektman_lower_team_a");
		legshader = legshader_a;
	} else if (ci->team == TEAM_BLUE) {
		if (legshader_b == 0)
			legshader_b = trap_R_RegisterShader("rektman_lower_team_b");
		legshader = legshader_b;
	}
	legs.customShader = legshader;

	if (cg_player_legs->integer)
	...
	//RotateAroundDirection(torso.axis, 1.5f);
	//torso.axis
	if (cg_player_body->integer) {

		qhandle_t torsoshader = 0;

		if (ci->team == TEAM_RED) {
			if (torsoshader_a == 0)
				torsoshader_a = trap_R_RegisterShader("rektman_upper_team_a");
			torsoshader = torsoshader_a;
		} else if (ci->team == TEAM_BLUE) {
			if (torsoshader_b == 0)
				torsoshader_b = trap_R_RegisterShader("rektman_upper_team_b");
			torsoshader = torsoshader_b;
		}
		torso.customShader = torsoshader;
	...
	if ( cent->currentState.powerups & ( 1 << PW_GUARD ) ) {
		...
		RE_AddRefEntityToScene( &powerup );
	}
	...
	if (cg_player_head->integer) {
		//cent->currentState.scale or something
