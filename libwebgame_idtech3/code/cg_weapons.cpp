...
//typedef struct kungweapon_s {
//	int id;
//	FxHandle shotsounds;
//	void (*precache)(weapon_t *weapon);
//	void (*shot)(weapon_t *weapon);
//	void (*reload)(weapon_t *weapon);
//	void (*activate)(weapon_t *weapon);
//	void (*deactivate)(weapon_t *weapon);
//} kungweapon_t;
//
//kungweapon_t weapons[20];
#include "cg_weapon.h"
...
void CG_RegisterWeapon( int weaponNum ) {
	...
	//if (first)
	{
		...
		cg_weapons[WP_CROWBAR].precache = crowbar_precache;
		cg_weapons[WP_CROWBAR].activate = crowbar_activate;
		cg_weapons[WP_SWORD  ].precache =   sword_precache;
		cg_weapons[WP_SWORD  ].activate =   sword_activate;
		cg_weapons[WP_KAR98  ].precache =   kar98_precache;
		cg_weapons[WP_KAR98  ].activate =   kar98_activate;
		cg_weapons[WP_MP44   ].precache =    mp44_precache;
		cg_weapons[WP_MP44   ].activate =    mp44_activate;
		//cg_weapons[WP_ROCKET_LAUNCHER].precache = panzerschreck_precache;
		//cg_weapons[WP_ROCKET_LAUNCHER].activate = panzerschreck_activate;
		cg_weapons[WP_BAZOOKA   ].precache =    bazooka_precache;
		cg_weapons[WP_BAZOOKA   ].activate =    bazooka_activate;
	}
	...
	for ( i = 0 ; i < 3 ; i++ ) {
		weaponInfo->weaponMidpoint[i] = 0.0f;
	}
	...
	//weaponInfo->flashModel = trap_R_RegisterModel( "kungmodels/crowbar.kung1" );
	...
	COM_StripExtension( item->world_model[0], path, sizeof(path) );
	Q_strcat( path, sizeof(path), "_hand.kung1" );
	...
	if (weaponInfo->precache) {
		weaponInfo->precache(weaponInfo);
		return;
	}
...
static void CG_CalculateWeaponPosition( vec3_t origin, vec3_t angles ) {
	...
	// disabled this, bobbing fucks up sniper weapons etc., makes it feel very fuzzy
	// wrong thingy, dont see any difference actually
#if 1
...
void CG_AddPlayerWeapon( refEntity_t *parent, playerState_t *ps, centity_t *cent, int team ) {
	...
	if ( !ps ) {
		...
		if ( ( cent->currentState.eFlags & EF_FIRING ) && weapon->firingSound ) {
			...
			//	trap_S_AddLoopingSound( cent->currentState.number, cent->lerpOrigin, vec3_origin, FxHandleGetID(&cgs.media.whoosh) );
			//} else {
	...
	lerped.origin[0] = cent->lerpOrigin[0];
	lerped.origin[1] = cent->lerpOrigin[1];
	lerped.origin[2] = cent->lerpOrigin[2];
	AnglesToAxis(cent->lerpAngles, lerped.axis);
	

	//R_LerpTag(&lerped, parent->hModel, parent->oldframe, parent->frame, 1.0 - parent->backlerp, "tag_weapon");
	//float cvaraxis[3] = {0,0,0};
	//if (cent->per
	//AnglesToAxis(cvaraxis, lerped.axis);
	...
	memcpy(gun.axis, parent->axis, sizeof(gun.axis));
	...
	VectorCopy(flash.origin, gun.origin);
...
void CG_AddViewWeapon( playerState_t *ps ) {
	...
	VectorMA( hand.origin, cvar_indexes[cg_gun_x.handle].value, cg.refdef.viewaxis[0], hand.origin );
	VectorMA( hand.origin, cvar_indexes[cg_gun_y.handle].value, cg.refdef.viewaxis[1], hand.origin );
	VectorMA( hand.origin, (cvar_indexes[cg_gun_z.handle].value+fovOffset), cg.refdef.viewaxis[2], hand.origin );
	...
	if ( cg_gun_frame.integer ) {
		...
		hand.frame = cent->pe.torso.frame; // fuck it, the mapping makes it just more complex
...
void CG_FireWeapon( centity_t *cent ) {
	...
	int weaponNum = cent->currentState.weapon;
	

	weaponInfo_t *weapon = cg_weapons + weaponNum;

	if (weapon->activate) {
		weapon->activate(weapon, cent);
		//if (weap->flashSound[0])
...
void CG_MissileHitWall( int weapon, int clientNum, vec3_t origin, vec3_t dir, impactSound_t soundType ) {
	...
	radius = 64;
	sfx = FxHandleGetID(&cgs.media.rocketexplosion);

	switch ( weapon ) {
	default:

	case WP_CROWBAR: break;
	...
	case WP_MP44:
		mod = cgs.media.bulletFlashModel;
		shader = cgs.media.bulletExplosionShader;
		mark = FxHandleGetID(&cgs.media.bulletmark);
...
void CG_Bullet( vec3_t end, int sourceEntityNum, vec3_t normal, qboolean flesh, int fleshEntityNum ) {
	...
	if ( sourceEntityNum >= 0 && cg_tracerChance.value > 0 ) {
		if ( CG_CalcMuzzlePoint( sourceEntityNum, start ) ) {
			...
			// bubble trail from air into water
				...
				// todo: make water splash on water
				CG_BubbleTrail( trace.endpos, end, 32 );
			}

			// todo: tracers are nice, but they need to at least persist like 0.1seconds, otherwise they look like graphic glitches
	...
	if ( flesh ) {
		...
		CG_MissileHitWall( WP_MP44, 0, end, normal, IMPACTSOUND_DEFAULT );
	}
