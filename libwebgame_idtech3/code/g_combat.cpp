...
void TossClientItems( gentity_t *self ) {
	...
	if (/* weapon > WP_MACHINEGUN && weapon != WP_GRAPPLING_HOOK && */ self->client->ps.ammo[ weapon ] ) {
		...
		if (item) {
			...
			Com_Printf("TossClientItems(playerid=%d)> Couldn't find item for weapon %i\n", self-g_entities, weapon);
		}
	}
...
int (*callback_player_killed)(int *self, int *inflictor, int *attacker, int damage, int meansOfDeath) = NULL;
CCALL void set_callback_player_killed(int (*cb)(int *self, int *inflictor, int *attacker, int damage, int meansOfDeath)) {
	callback_player_killed = cb;
}
...
void player_die( gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int meansOfDeath ) {
	...
	if (callback_player_killed && callback_player_killed((int *)self, (int *)inflictor, (int *)attacker, (int)damage, (int)meansOfDeath)) {
		return;
	}
	...
	// ignore gib shit... probably just lags with many players and cant test anims
...
void G_Damage( gentity_t *targ, gentity_t *inflictor, gentity_t *attacker,
	...
	// need some kind of entity in case of NULL
	gentity_t *world = g_entities + 1022;
	vec3_t zero = {0,0,0};

	if (inflictor == NULL) inflictor = world;
	if (attacker == NULL) attacker = world;
	if (dir == NULL) dir = zero;
	if (point == NULL) point = zero;
