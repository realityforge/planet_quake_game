...
void CG_PainEvent( centity_t *cent, int health ) {
	...
	if (CG_WaterLevel(cent) == 3) {
		...
		trap_S_StartSound(NULL, cent->currentState.number, CHAN_VOICE, FxHandleGetID(&cgs.media.pain));
	}
...
void CG_EntityEvent( centity_t *cent, vec3_t position ) {
	...
	//if (event >= ((entity_event_t)52) && event <= 54) {
 	//	//Com_Printf("debug rocket event...\n");
	//}
	//
	//Com_Printf("CG_EntityEvent: event=%d pos: %.2f %.2f %.2f\n", event, position[0], position[1], position[2]);
	...
	case EV_FOOTSTEP:
		...
		if (cg_footsteps.integer) {
			trap_S_StartSound (NULL, es->number, CHAN_BODY, FxHandleGetID(&cgs.media.footstep_default) );
		}
		break;
	case EV_FOOTSTEP_METAL:
		DEBUGNAME("EV_FOOTSTEP_METAL");
		if (cg_footsteps.integer) {
			trap_S_StartSound (NULL, es->number, CHAN_BODY, FxHandleGetID(&cgs.media.footstep_metal) );
		}
		break;
	case EV_FOOTSPLASH:
		DEBUGNAME("EV_FOOTSPLASH");
		if (cg_footsteps.integer) {
			trap_S_StartSound (NULL, es->number, CHAN_BODY, FxHandleGetID(&cgs.media.footstep_splash) );
		}
		break;
	case EV_FOOTWADE:
		DEBUGNAME("EV_FOOTWADE");
		if (cg_footsteps.integer) {
			trap_S_StartSound (NULL, es->number, CHAN_BODY, FxHandleGetID(&cgs.media.footwade) );
		}
		break;
	case EV_SWIM:
		DEBUGNAME("EV_SWIM");
		if (cg_footsteps.integer) {
			trap_S_StartSound (NULL, es->number, CHAN_BODY, FxHandleGetID(&cgs.media.swim) );
		}
		break;
	case EV_FALL_SHORT:
	...
	case EV_FALL_MEDIUM:
		...
		trap_S_StartSound( NULL, es->number, CHAN_VOICE, FxHandleGetID(&cgs.media.pain) );
		if ( clientNum == cg.predictedPlayerState.clientNum ) {
			// smooth landing z changes
			cg.landChange = -16;
			cg.landTime = cg.time;
	...
	case EV_FALL_FAR:
		...
		trap_S_StartSound (NULL, es->number, CHAN_AUTO, FxHandleGetID(&cgs.media.pain) );
...
//		CG_Printf( "EV_JUMP_PAD w/effect #%i\n", es->eventParm );
	...
	case EV_JUMP:
		DEBUGNAME("EV_JUMP");
		trap_S_StartSound (NULL, es->number, CHAN_VOICE, FxHandleGetID(&cgs.media.jump) );
		break;
	case EV_TAUNT:
		DEBUGNAME("EV_TAUNT");
	...
	case EV_WATER_TOUCH:
		...
		trap_S_StartSound (NULL, es->number, CHAN_AUTO, FxHandleGetID(&cgs.media.waterIn) );
		break;
	case EV_WATER_LEAVE:
		DEBUGNAME("EV_WATER_LEAVE");
		trap_S_StartSound (NULL, es->number, CHAN_AUTO, FxHandleGetID(&cgs.media.waterOut) );
		break;
	case EV_WATER_UNDER:
		DEBUGNAME("EV_WATER_UNDER");
		trap_S_StartSound (NULL, es->number, CHAN_AUTO, FxHandleGetID(&cgs.media.waterSlowIn) );
		break;
	case EV_WATER_CLEAR:
		DEBUGNAME("EV_WATER_CLEAR");
		trap_S_StartSound (NULL, es->number, CHAN_AUTO, FxHandleGetID(&cgs.media.waterAppear) );
		break;
...
//		trap_S_StartSound (NULL, es->number, CHAN_AUTO, cgs.media.noAmmoSound );
...
#ifdef EMSCRIPTEN
		float* base = (float*)& es->pos.trBase;
		EM_ASM_({
		CG_Bullet($0, $1, $2, $3, $4, $5, $6);
			}, base[0], base[1], base[2], es->otherEntityNum, dir[0], dir[1], dir[2]);
#else
		CG_Bullet(es->pos.trBase, es->otherEntityNum, dir, qfalse, ENTITYNUM_WORLD);
#endif

		trap_S_StartSound(es->pos.trBase, ENTITYNUM_WORLD, CHAN_AUTO, FxHandleGetID(&cgs.media.ev_bullet_hit_wall));
	...
	case EV_BULLET_HIT_FLESH:
		...
		trap_S_StartSound( es->pos.trBase, ENTITYNUM_WORLD, CHAN_AUTO, FxHandleGetID(&cgs.media.ev_bullet_hit_flesh) );
		break;
	...
	case EV_DEATH3:
		...
		if (CG_WaterLevel(cent) == 3) {
			...
			trap_S_StartSound(NULL, es->number, CHAN_VOICE, FxHandleGetID(&cgs.media.death)/*CG_CustomSound(es->number, va("*death%i.wav", event - EV_DEATH1 + 1))*/);
		}
