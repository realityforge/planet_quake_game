...
// "locals" of SV_SpawnServer, need them in SV_SpawnServer_Resume
int			i;
int			checksum;
qboolean	isBot;
...
void SV_SpawnServer( char *server_, qboolean killBots_ ) {
	memcpy(server, server_, strlen(server_) + 1);
	killBots = killBots_;
	...
	//Com_Printf("before CL_ShutdownAll(qfalse);\n");
...
//	Cvar_Set( "nextmap", va("map %s", server) );
	...
	char fullpath[256];
	snprintf(fullpath, sizeof(fullpath), "maps/%s.bsp", server);
	CM_LoadMap( fullpath, qfalse, &checksum );
}

CCALL void SV_SpawnServer_Resume() {

	if (Cvar_VariableValue("sv_running") == 0) {
		Com_Printf("server was started but ended already before asynchron map download finished\n");
		return;
	}

	// set serverinfo visible name
	Cvar_Set( "mapname", (const char *)server );
	...
	//Com_Printf("SV_SpawnServer 2\n");

	// create a baseline for more efficient communications
	SV_CreateBaseline ();

	//Com_Printf("SV_SpawnServer 3\n");
	...
	Cvar_Set( "sv_referencedPaks", "abc" );
	Cvar_Set( "sv_referencedPakNames", "abc" );


	
	//Com_Printf("SV_SpawnServer 5\n");
