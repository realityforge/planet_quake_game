...
qboolean	NET_GetLoopPacket (netsrc_t sock, netadr_t *net_from, msg_t *net_message)
{
	...
	//Com_Printf("NET_GetLoopPacket....\n");
	
	loop = &loopbacks[sock];
	
	//Com_Printf("NET_GetLoopPacket....  1\n");
	if (loop->send - loop->get > MAX_LOOPBACK)
		loop->get = loop->send - MAX_LOOPBACK;
	
	
	//Com_Printf("NET_GetLoopPacket.... 2\n");
	
	if (loop->get >= loop->send)
		return qfalse;
	
	
	//Com_Printf("NET_GetLoopPacket.... 3\n");
	
	i = loop->get & (MAX_LOOPBACK-1);
	loop->get++;
	
	 
	//Com_Printf("NET_GetLoopPacket.... 4 loop->msgs[i].datalen=%d net_message->maxsize=%d\n", loop->msgs[i].datalen, net_message->maxsize);
	
	memcpy (net_message->data, loop->msgs[i].data, loop->msgs[i].datalen);
	//
	//Com_Printf("NET_GetLoopPacket....5\n");
	//
	net_message->cursize = loop->msgs[i].datalen;
	//
	//Com_Printf("NET_GetLoopPacket....6 sizeof(*net_from)=%d\n", sizeof(*net_from));
	memset (net_from, 0, sizeof(*net_from));
	//
	//Com_Printf("NET_GetLoopPacket....7\n");
	net_from->type = NA_LOOPBACK;
	
	//Com_Printf("NET_GetLoopPacket....8\n");
	return qtrue;
...
//typedef void (*type_callback_sendpacket)(int, int, int, int, int, int *, int);
//type_callback_sendpacket callback_sendpacket = NULL;
//
//CCALL void set_callback_sendpacket(type_callback_sendpacket cb) {
//	callback_sendpacket = cb;
//}
...
#ifdef EMSCRIPTEN
	EM_ASM({
		callback_sendpacket($0, $1, $2, $3, $4, $5, $6, $7);
	}, to.ip[0], to.ip[1], to.ip[2], to.ip[3], to.port, data, length, sock);
	return;
#endif

	//if (callback_sendpacket) {
	//	callback_sendpacket(to.ip[0], to.ip[1], to.ip[2], to.ip[3], to.port, data, length);
	//	return;
	//}
