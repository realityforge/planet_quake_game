...
void CG_NewClientInfo( int clientNum ) {
	...
	if ( cg_forceModel.integer ) {
		...
		if (cgs.gametype >= GT_TEAM ) {
 			...
 			//KILDEREAN : models for red and blue team
 			char Blue_modelStr[MAX_QPATH];
 			char Red_modelStr[MAX_QPATH];
 
 			trap_Cvar_VariableStringBuffer( "cg_PureBluemodel", Blue_modelStr, sizeof( Blue_modelStr ) );
 			trap_Cvar_VariableStringBuffer( "cg_PureRedmodel", Red_modelStr, sizeof( Red_modelStr ) );
 			//END KILDEREAN
			...
			//KILDEREAN : models for red and blue team
 			if(newInfo.team==TEAM_BLUE || (ci->oldTeam==TEAM_BLUE && newInfo.team == TEAM_SPECTATOR)){
 				Q_strncpyz( newInfo.modelName, Blue_modelStr, sizeof( newInfo.modelName ) );
 			}
			else{
 				if(newInfo.team==TEAM_RED || (ci->oldTeam==TEAM_RED && newInfo.team == TEAM_SPECTATOR)){					
					Q_strncpyz( newInfo.modelName, Red_modelStr, sizeof( newInfo.modelName ) );					
 				}
 				else{
	...
	if ( cg_forceModel.integer ) {
		...
		if ( cgs.gametype >= GT_TEAM ) {
 			...
 			trap_Cvar_VariableStringBuffer( "cg_PureBlueHeadmodel", Blue_modelStr, sizeof( Blue_modelStr ) );
 			trap_Cvar_VariableStringBuffer( "cg_PureRedHeadmodel", Red_modelStr, sizeof( Red_modelStr ) );
 			//END KILDEREAN
			...
			//KILDEREAN : models for red and blue team
 			if(newInfo.team==TEAM_BLUE || (ci->oldTeam==TEAM_BLUE && newInfo.team == TEAM_SPECTATOR)){
 				Q_strncpyz( newInfo.headModelName, Blue_modelStr, sizeof( newInfo.headModelName ) );
 			}
			else{
 				if(newInfo.team==TEAM_RED || (ci->oldTeam==TEAM_RED && newInfo.team == TEAM_SPECTATOR)){					
					Q_strncpyz( newInfo.headModelName, Red_modelStr, sizeof( newInfo.headModelName ) );					
 				}
 				else{
 					Q_strncpyz( newInfo.headModelName, "sarge", sizeof( newInfo.headModelName ) );
 				}
 			}
 			//END KILDEREAN
		}
...
static void CG_PlayerAnimation( centity_t *cent, int *legsOld, int *legs, float *legsBackLerp,
	...
	// KILDEREAN
	if ( cent->currentState.powerups & ( 1 << PW_HASTE ) && cent->currentState.powerups & ( 1 << PW_HASTE_PURE )) {
		speedScale = 1.8f;
...
// KILDEREAN : function for flag trail when player has the enemy flag
static void CG_FlagTrail( centity_t *cent ) {
	localEntity_t	*smoke;
 	vec3_t			origin;
...
static void CG_TrailItem( centity_t *cent, qhandle_t hModel ) {
	...
	// KILDEREAN : make flag actually appear to be carried by fc
	VectorMA( cent->lerpOrigin, -8, axis[0], ent.origin );
	ent.origin[2] += 16;
	angles[YAW] += 90;
	angles[ROLL] -=10;
	angles[PITCH] -= 45;
...
static void CG_PlayerPowerups( centity_t *cent ) {
	...
	if ( powerups & ( 1 << PW_REDFLAG ) ) {
		if (ci->newAnims) {
 		...
 		// KILDEREAN
 			if(cg_PureFlagTrail.integer==1)
				CG_FlagTrail(cent);
		}

		//trap_R_AddLightToScene( cent->lerpOrigin, 200 + (rand()&31), 1.0, 0.2f, 0.2f );
		trap_R_AddLightToScene( cent->lerpOrigin, 300 + (rand()&31), 1, 0.2f, 0.2f );		
		// END KILDEREAN
	}
	...
	if ( powerups & ( 1 << PW_BLUEFLAG ) ) {
		...
		trap_R_AddLightToScene( cent->lerpOrigin, 300 + (rand()&31), 0.2f, 0.2f, 1 );
		// END KILDEREAN
	}
...
// END KILDEREAN
	...
	if ( state->powerups & ( 1 << PW_INVIS ) ) {
		...
		// KILDEREAN : add rune shells
 		if( powerups & ( 1 << PW_STRENGTH_PURE ) )
 		{
 			ent->customShader=cgs.media.strengthShader;
 			trap_R_AddRefEntityToScene( ent );
 		}
 		if( powerups & ( 1 << PW_HASTE_PURE ) )
 		{
 			ent->customShader=cgs.media.hastepureShader;
 			trap_R_AddRefEntityToScene( ent );
 		}
 		if ( powerups & ( 1 << PW_REGEN ) || powerups & ( 1 << PW_REGEN_PURE ) || teamMateIsDoctor(team)) {
			if ( ( ( cg.time / 100 ) % 10 ) == 1 ) {
		...
		if((powerups & ( 1 << PW_BATTLESUIT )) && (powerups & ( 1 << PW_RESISTANCE_PURE ))){
			if(team == TEAM_RED)
				ent->customShader = cgs.media.redShield;
			else
				ent->customShader = cgs.media.blueShield;
			trap_R_AddRefEntityToScene( ent );		
		}
		else {
			...
			if ( (cg.time - cent->pe.painTime < PAIN_TWITCH_TIME*2) && (powerups & ( 1 << PW_RESISTANCE_PURE )) ) {
 				ent->customShader = cgs.media.resistanceShader1;
 				trap_R_AddRefEntityToScene( ent );
 			}
 			if( !(cg.time - cent->pe.painTime < PAIN_TWITCH_TIME*2) && (powerups & ( 1 << PW_RESISTANCE_PURE )) ){
 				ent->customShader = cgs.media.resistanceShader2;
 				trap_R_AddRefEntityToScene( ent );
 			}
		}

 		if( cg.inOvertimeWarmup == qtrue){
 			if (team == TEAM_RED)
 				ent->customShader = cgs.media.redQuadShader;
 			else
 				ent->customShader = cgs.media.quadShader;
...
// KILDEREAN
qboolean teamMateIsDoctor(int team){
	int i;
	clientInfo_t *ci;
	...
	for (i=0 ; i < MAX_CLIENTS ; i++) {
		ci = &cgs.clientinfo[i];
		if(ci){
			if(ci->team == team && ci->powerups & (1<<PW_REGEN) && ci->powerups & (1<<PW_REGEN_PURE)){
				return qtrue;
			}
		}
	}
