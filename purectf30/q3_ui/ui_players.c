...
tryagain:
	...
	if( pi->weaponModel == 0 ) {
 	...
 	// KILDEREAN : add grapple
 	if ( weaponNum == WP_MACHINEGUN || weaponNum == WP_GAUNTLET || weaponNum == WP_BFG || weaponNum == WP_GRAPPLING_HOOK ) {
 	// END KILDEREAN
...
void UI_DrawPlayer( float x, float y, float w, float h, playerInfo_t *pi, int time ) {
	...
	int				renderfx;
	// KILDEREAN : all +8
	vec3_t			mins = {-24, -24, -32};
	vec3_t			maxs = {24, 24, 40};
	// END KILDEREAN
	float			len;
	float			xx;
	...
	//
 	// KILDEREAN : add grapple
 	if ( pi->realWeapon == WP_MACHINEGUN || pi->realWeapon == WP_GAUNTLET || pi->realWeapon == WP_BFG || pi->realWeapon == WP_GRAPPLING_HOOK ) {
	// END KILDEREAN
		vec3_t	angles;
		...
		angles[ROLL] = UI_MachinegunSpinAngle( pi );
 		...
 		if( pi->realWeapon == WP_GRAPPLING_HOOK){
	...
	//if ( dp_realtime <= pi->muzzleFlashTime ) {
 	if ( dp_realtime <= pi->muzzleFlashTime || pi->realWeapon == WP_GRAPPLING_HOOK ) {
 	// END KILDEREAN
		if ( pi->flashModel ) {
