...
//KILDEREAN : add options for team models
#define ID_BLUEMODEL			136
#define ID_REDMODEL				137
//END KILDEREAN
#define ID_DRAWTEAMOVERLAY		138
...
static void Teammodel_draw( void *self ) {
	...
	if(s->generic.id == ID_BLUEMODEL){
		trap_Cvar_VariableStringBuffer( "cg_PureBluemodel", bmodel, 64 );
		UI_DrawString(x + SMALLCHAR_WIDTH, y ,bmodel,style,color);
	}
	if(s->generic.id == ID_REDMODEL){
		trap_Cvar_VariableStringBuffer( "cg_PureRedmodel", rmodel, 64 );
