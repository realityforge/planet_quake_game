...
void G_RunMissile( gentity_t *ent ) {
	...
	// KILDEREAN : prevent hook from continuing to exist if we switch weapon from the grapple while holding fire
	if((ent->parent->client->ps.weaponstate == WEAPON_DROPPING) && (ent->parent && ent->parent->client->hook == ent)){
...
gentity_t *fire_bfg (gentity_t *self, vec3_t start, vec3_t dir) {
	...
	VectorNormalize (dir);
 	...
 	if(g_PureMortarBFG.integer==1){
 		...
 		bolt->parent = self;
 		bolt->damage = 200;
 		bolt->splashDamage = 150;//KILDER was 100
 		bolt->splashRadius = 200;//KILDER was 120
 		bolt->methodOfDeath = MOD_BFG;
 		bolt->splashMethodOfDeath = MOD_BFG_SPLASH;
 		bolt->clipmask = MASK_SHOT;
 
 		bolt->s.pos.trType = TR_GRAVITY; // KILDER : changed from TR_LINEAR to make bfg a mortar gun
 		bolt->s.pos.trTime = level.time - MISSILE_PRESTEP_TIME;		// move a bit on the very first frame
 		VectorCopy( start, bolt->s.pos.trBase );
...
gentity_t *fire_grapple (gentity_t *self, vec3_t start, vec3_t dir) {
	...
	VectorCopy( start, hook->s.pos.trBase );
 	...
 	// KILDEREAN : new speed was 800
	//VectorScale( dir, 800, hook->s.pos.trDelta );
 	if(g_PureFastHook.integer == 1)
 		VectorScale( dir, PURE_HOOKFAST, hook->s.pos.trDelta );
 	else
 		VectorScale( dir, PURE_HOOKNORMAL, hook->s.pos.trDelta );
 	// END KILDEREAN
