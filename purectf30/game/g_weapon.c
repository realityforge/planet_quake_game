...
qboolean CheckGauntletAttack( gentity_t *ent ) {
	...
	// KILDEREAN : prevent gauntlet hits in warmup
	if(level.getReadyForOT || level.inOvertimeWarmup)
		return qfalse;
	// END KILDEREAN
	...
	if ( !traceEnt->takedamage) {
 	...
 	if (ent->client->ps.powerups[PW_QUAD] ) {
 		if (ent->client->ps.powerups[PW_STRENGTH_PURE] ) 
 			G_AddEvent( ent, EV_POWERUP_QUAD_STRENGTH, 0 );
 		else
 	...
 	if ( (ent->client->ps.powerups[PW_HASTE_PURE]))
		G_AddEvent( ent, EV_POWERUP_HASTE_PURE, 0 );
	// END KILDEREAN
	...
	// KILDEREAN
 	if (ent->client->ps.powerups[PW_STRENGTH_PURE] ) {
 		if (!ent->client->ps.powerups[PW_QUAD] )
                         G_AddEvent( ent, EV_POWERUP_STRENGTH, 0 );
 		damage *= 2;
 	}
 	// KILDEREAN END
...
void BFG_Fire ( gentity_t *ent ) {
	...
	m->splashDamage *= s_quadFactor;
 	...
 	// KILDEREAN: make bfg hurt the player ; so that it is usable but at a cost
    if (ent->client->ps.powerups[PW_STRENGTH_PURE] ) {
 		m->damage *= 2;
 		m->splashDamage *= 2;
 		if(g_PureHardBFG.integer==1)
 			G_Damage(ent,NULL,ent,NULL,NULL,250,DAMAGE_RADIUS,MOD_BFG_SPLASH);
 	}
     else{
 		if(g_PureHardBFG.integer==1)
 			G_Damage(ent,NULL,ent,NULL,NULL,125,DAMAGE_RADIUS,MOD_BFG_SPLASH);
     }
 	// END KILDEREAN
...
void FireWeapon( gentity_t *ent ) {
	...
	// KILDEREAN : removed these? i dont remember why... probably events sent elsewhere
 	if (ent->client->ps.powerups[PW_QUAD] ) {
 		/*
 		if (ent->client->ps.powerups[PW_STRENGTH_PURE])
 			G_AddEvent( ent, EV_POWERUP_QUAD_STRENGTH, 0 );
