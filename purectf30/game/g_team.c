...
void Team_FragBonuses(gentity_t *targ, gentity_t *inflictor, gentity_t *attacker)
{
	...
	if (targ->client->ps.powerups[enemy_flag_pw]) {
		...
		//AddScore(attacker, targ->r.currentOrigin, CTF_FRAG_CARRIER_BONUS);
 		AddScore(attacker,targ->r.currentOrigin, ( (g_PureScore.integer == 1) ? PURE_CTF_FRAG_CARRIER_BONUS : CTF_FRAG_CARRIER_BONUS )); 
 		// END KILDEREAN
	...
	if (targ->client->pers.teamState.lasthurtcarrier &&
		...
		//AddScore(attacker, targ->r.currentOrigin, CTF_CARRIER_DANGER_PROTECT_BONUS);
 		AddScore(attacker, targ->r.currentOrigin,( (g_PureScore.integer == 1) ? PURE_CTF_CARRIER_DANGER_PROTECT_BONUS : CTF_CARRIER_DANGER_PROTECT_BONUS ));
 		// END KILDEREAN
	...
	if ( ( ( VectorLength(v1) < CTF_TARGET_PROTECT_RADIUS &&
		...
		AddScore(attacker,targ->r.currentOrigin, ( (g_PureScore.integer == 1) ? PURE_CTF_FLAG_DEFENSE_BONUS : CTF_FLAG_DEFENSE_BONUS ));
		// END KILDEREAN
	...
	if (carrier && carrier != attacker) {
		...
		if ( ( ( VectorLength(v1) < CTF_ATTACKER_PROTECT_RADIUS &&
			...
			//AddScore(attacker, targ->r.currentOrigin, CTF_CARRIER_PROTECT_BONUS);
 			AddScore(attacker,targ->r.currentOrigin, ( (g_PureScore.integer == 1) ? PURE_CTF_CARRIER_PROTECT_BONUS : CTF_CARRIER_PROTECT_BONUS ));
 			// END KILDEREAN
...
int Team_TouchOurFlag( gentity_t *ent, gentity_t *other, int team ) {
	...
	if ( ent->flags & FL_DROPPED_ITEM ) {
		...
		AddScore(other, ent->r.currentOrigin,( (g_PureScore.integer == 1) ? PURE_CTF_RECOVERY_BONUS : CTF_RECOVERY_BONUS ));
 		// END KILDEREAN
	...
	//AddScore(other, ent->r.currentOrigin, CTF_CAPTURE_BONUS);
 	AddScore(other, ent->r.currentOrigin,( (g_PureScore.integer == 1) ? PURE_CTF_CAPTURE_BONUS : CTF_CAPTURE_BONUS ));
 	// END KILDEREAN
		...
		if (player->client->sess.sessionTeam !=
			...
			if (player != other)				
				...
				//AddScore(player, ent->r.currentOrigin, CTF_TEAM_BONUS);
 				AddScore(player,ent->r.currentOrigin,( (g_PureScore.integer == 1) ? PURE_CTF_TEAM_BONUS : CTF_TEAM_BONUS ));
 				// END KILDEREAN
			...
			if (player->client->pers.teamState.lastreturnedflag + 
				...
				//AddScore (player, ent->r.currentOrigin, CTF_RETURN_FLAG_ASSIST_BONUS);
 				AddScore(player,ent->r.currentOrigin, ( (g_PureScore.integer == 1) ? PURE_CTF_RETURN_FLAG_ASSIST_BONUS : CTF_RETURN_FLAG_ASSIST_BONUS ));
				// END KILDEREAN
			...
			if (player->client->pers.teamState.lastfraggedcarrier + 
				...
				//AddScore(player, ent->r.currentOrigin, CTF_FRAG_CARRIER_ASSIST_BONUS);
 				AddScore(player, ent->r.currentOrigin,( (g_PureScore.integer == 1) ? PURE_CTF_FRAG_CARRIER_ASSIST_BONUS : CTF_FRAG_CARRIER_ASSIST_BONUS ));
 				// END KILDEREAN
...
int Team_TouchEnemyFlag( gentity_t *ent, gentity_t *other, int team ) {
	...
	//AddScore(other, ent->r.currentOrigin, CTF_FLAG_BONUS);
 	AddScore(other, ent->r.currentOrigin,( (g_PureScore.integer == 1) ? PURE_CTF_FLAG_BONUS : CTF_FLAG_BONUS ));
	// END KILDEREAN
...
gentity_t *SelectRandomTeamSpawnPoint( int teamstate, team_t team ) {
	...
	// KILDEREAN : use any freakin spot to spawn a player
	if(count == 0){
		if(teamstate != TEAM_BEGIN){
			if(team == TEAM_RED){
				classname = "team_CTF_redplayer";
		...
		// KILDEREAN : in OT use any freakin spot to spawn a player
		else{
			if(team == TEAM_RED){
				classname = "team_CTF_redspawn";
				while ((spot = G_Find (spot, FOFS(classname), classname)) != NULL) {
