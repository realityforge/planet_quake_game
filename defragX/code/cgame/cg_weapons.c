...
vec3_t flashOrigin; //used to draw the grappling hook chain
...
static void CG_RocketTrail( centity_t *ent, const weaponInfo_t *wi ) {
	...
	for ( ; t <= ent->trailTime ; t += step ) {
		...
		// SLK: colorize the smoke based on railcolor?
		smoke = CG_SmokePuff( lastPos, up,
					  wi->trailRadius,
					  1.0f, 0.8f, 0.9f, 0.33f,
					  wi->wiTrailTime,
					  t,
					  0,
					  0,
...
void CG_GrappleTrail( centity_t *ent, const weaponInfo_t *wi )
{
    ...
    {
		if ( cg.predictedPlayerState.weapon == WP_GRAPPLING_HOOK && !cg.renderingThirdPerson && cg_drawGun.integer)
		{
			VectorAdd( flashOrigin, cg.predictedPlayerState.origin, beam.origin );
		}
        else
        {
			VectorCopy( cg.predictedPlayerState.origin, beam.origin );
			beam.origin[2] += 26;
...
void CG_RegisterWeapon( int weaponNum )
{
	...
	if ( weaponNum == 0 || weaponInfo->registered )
    {
		return;
	}
	...
	case WP_GRAPPLING_HOOK:
		...
		weaponInfo->missileSound = trap_S_RegisterSound( "sound/weapons/grapple/chain.wav", qfalse );
		cgs.media.sfx_hookhit = trap_S_RegisterSound( "sound/weapons/grapple/hit.wav", qfalse );
...
#if 0
    // SLK drop the weapon when stair climbing?
...
static void CG_LightningBolt( centity_t *cent, vec3_t origin ) {
	...
	if ( !pm_interference ) {
		CG_Trace( &trace, muzzlePoint, vec3_origin, vec3_origin, endPoint, cent->currentState.number, CONTENTS_SOLID );
	} else {
...
void CG_AddPlayerWeapon( refEntity_t *parent, playerState_t *ps, centity_t *cent, int team )
{
	...
	if ( ps )
    {
		VectorSubtract( flash.origin, ps->origin, flashOrigin ); // save it for the grappling hook
	}
	...
	if ( ps || cg.renderingThirdPerson || cent->currentState.number != cg.predictedPlayerState.clientNum )
    {
		...
		if ( ( weapon->flashDlightColor[0] || weapon->flashDlightColor[1] || weapon->flashDlightColor[2] ) && cgs.gametype != GT_ADVENTURE )
...
void CG_AddViewWeapon( playerState_t *ps ) {
	...
	VectorMA( hand.origin, cg_gun_x.value + (ps->weapon == WP_GRAPPLING_HOOK ? 5 : 0), cg.refdef.viewaxis[0], hand.origin );
    ...
    */
	...
	for ( i = WP_GAUNTLET ; i < MAX_WEAPONS ; i++ ) {
		...
		if ( i == cg.weaponSelect )
			CG_DrawPic( x-4, y-4, ICON_SIZE+8, ICON_SIZE+8, cgs.media.selectShader );
        */

		if ( cg_drawWeaponSelect.integer > 1 /*&& cg.snap->ps.ammo[ i ] > 0*/ ) {
			// ammo counter
			BG_sprintf( buf, "%i", cg.snap->ps.ammo[ i ] );
			if ( cg_drawWeaponSelect.integer == 2 ) {
				// horizontal ammo counters
				if ( i != cg.weaponSelect ) {
                    CG_FillRect( x-4, y, ICON_SIZE+8, ICON_SIZE+AMMO_FONT_SIZE+6, colorBlackTrans);
                    CG_DrawString( x+ICON_SIZE/2, y + ICON_SIZE+4, buf, colorWhite, AMMO_FONT_SIZE, AMMO_FONT_SIZE, 0, DS_CENTER | DS_PROPORTIONAL | DS_SHADOW );
                    CG_DrawPic( x, y, ICON_SIZE, ICON_SIZE, cg_weapons[i].weaponIcon );
                } else {
                    //CG_FillRect( x, y, ICON_SIZE, ICON_SIZE + 6, colorLtGreyTrans);
                    CG_DrawPic( x-4, y-4, ICON_SIZE+8, ICON_SIZE+8, cgs.media.selectShader );
                    CG_DrawPic( x, y, ICON_SIZE, ICON_SIZE, cg_weapons[i].weaponIcon );
                }
			} else {
				...
				if ( i != cg.weaponSelect ) {
                    CG_FillRect( x-6 , y, ICON_SIZE + (6*AMMO_FONT_SIZE), ICON_SIZE, colorBlackTrans);
                    CG_DrawString( x+ 28+(3*AMMO_FONT_SIZE), y+(ICON_SIZE-AMMO_FONT_SIZE)/2, buf, colorWhite, AMMO_FONT_SIZE, AMMO_FONT_SIZE, 0, DS_RIGHT | DS_SHADOW );
                    CG_DrawPic( x, y, ICON_SIZE, ICON_SIZE, cg_weapons[i].weaponIcon );
                }/* else {
                    CG_FillRect( x-6 , y, ICON_SIZE + (6*AMMO_FONT_SIZE), ICON_SIZE, colorLtGreyTrans);
                }*/
		...
		if ( !cg.snap->ps.ammo[ i ] )
			CG_DrawPic( x, y, ICON_SIZE, ICON_SIZE, cgs.media.noammoShader );
	...
	if ( cg_weapons[ cg.weaponSelect ].item && cg_drawWeaponSelect.integer == 1 ) {
		...
		if ( name )
			CG_DrawString( 320-((ARRAY_LEN(name)*SMALLCHAR_WIDTH)/2), y - 22, name, colorWhite, SMALLCHAR_WIDTH, SMALLCHAR_HEIGHT, 0, DS_SHADOW /*| DS_PROPORTIONAL*/ | DS_CENTER | DS_FORCE_COLOR );
	}
...
static void CG_ShotgunPellet( vec3_t start, vec3_t end, int skipNum ) {
	...
	if ( !pm_interference ) {
		CG_Trace( &tr, start, NULL, NULL, end, skipNum, CONTENTS_SOLID );
	} else {
