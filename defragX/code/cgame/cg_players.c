...
==========================
*/
static qbool	CG_FileExists( const char *filename )
{
	int len;
	fileHandle_t	f;
...
==========================
*/
static qbool	CG_FindClientModelFile( char *filename, int length, clientInfo_t *ci, const char *teamName, const char *modelName, const char *skinName, const char *base, const char *ext )
{
...
==========================
*/
static qbool	CG_FindClientHeadFile( char *filename, int length, clientInfo_t *ci, const char *teamName, const char *headModelName, const char *headSkinName, const char *base, const char *ext ) {
...
==========================
*/
static qbool	CG_RegisterClientSkin( clientInfo_t *ci, const char *teamName, const char *modelName, const char *skinName, const char *headModelName, const char *headSkinName ) {
...
==========================
*/
static qbool CG_RegisterClientModelname( clientInfo_t *ci, const char *modelName, const char *skinName, const char *headModelName, const char *headSkinName, const char *teamName ) {
	char	filename[MAX_QPATH];
	const char		*headName;
...
static qbool CG_IsKnownModel( const char *modelName )
{
...
static qbool CG_ScanForExistingClientInfo( clientInfo_t *ci ) {
	...
	for ( i = 0 ; i < cgs.maxclients ; i++ ) {
		...
		if ( !Q_stricmp( ci->modelName, match->modelName )
			...
			&& (cgs.gametype < GT_TEAMRUN || ci->team == match->team) ) {
...
static void CG_SetSkinAndModel( clientInfo_t *newInfo,
		...
		qbool allowNativeModel,
		int clientNum, int myClientNum,
		team_t myTeam, qbool setColor,
		...
		char *skinName, int skinNameSize )
{
	...
	if ( cg_forceModel.integer || cg_enemyModel.string[0] || cg_teamModel.string[0] )
	{
		if ( cgs.gametype == GT_TEAMRUN )
		{
			...
			if ( pm_model && myClientNum != clientNum && cgs.gametype != GT_DEFRAG ) {
				Q_strncpyz( modelName, infomodel, modelNameSize );
			...
			} else if ( cg_enemyModel.string[0] && myClientNum != clientNum && !allowNativeModel && cgs.gametype != GT_DEFRAG ) {
...
static qbool CG_PlayerShadow( centity_t *cent, float *shadowPlane ) {
...
void CG_AddBoundingBox(centity_t * cent)
{
	...
	int i;
	vec3_t mins, maxs;
	float extx, exty, extz, minZ;
	vec3_t corners[8];
	qhandle_t bboxShader, bboxFloorShader;

	if (!hud_bbox.integer || cgs.gametype == GT_ADVENTURE)
        return;

	// if we want firstperson drawed but we are in thirdperson
	if ( !hud_bboxFPS.integer && !cg_thirdPerson.integer )
        return;
	...
	bboxFloorShader = trap_R_RegisterShader("bbox_bottom");
	minZ = hud_bboxMinsZ.value;

	// if they don't exist, forget it
	if (!bboxShader || !bboxFloorShader)
		return;

	// get the player's client info
	ci = &cgs.clientinfo[cent->currentState.clientNum];
	// if it's us calculate it based on whether we're crouching
	...
	else
    {
		...
		// at really small scales this might be wrong
		if( fabs(zu - crouch) < fabs(zu - stand) )
        {
			maxs[2] = crouch;
		}
		else
	...
	switch (hud_bboxColor.integer) {
    case 0: //dense grey
        for (i = 0; i < 4; i++)
        {
			verts[i].modulate[0] = 10;
			verts[i].modulate[1] = 10;
			verts[i].modulate[2] = 10;
			verts[i].modulate[3] = 255;
		}
		break;
    ...
    default: // white
        for (i = 0; i < 4; i++)
        {
			verts[i].modulate[0] = 192;
			verts[i].modulate[1] = 192;
			verts[i].modulate[2] = 192;
			verts[i].modulate[3] = 255;
		}
	}
    ...
    // top
    if (hud_bbox.integer == 1 || hud_bbox.integer == 3  )
    {
	...
	if (hud_bbox.integer >= 1 )
	{
        ...
        if (hud_bbox.integer >= 3)
        {
            trap_R_AddPolyToScene(bboxFloorShader, 4, verts);
        }
        else
        {
            trap_R_AddPolyToScene(bboxShader, 4, verts);
...
void CG_Spawnpoint( centity_t *cent ) {
    ...
    if ( cg_showSpawns.integer == 1 ) {
        cent->currentState.powerups = 1 << PW_INVIS;
    } else if ( cg_showSpawns.integer == 2 ) {
		cent->currentState.powerups = 1 << PW_QUAD;
    } else if ( cg_showSpawns.integer == 3 ) {
		cent->currentState.powerups = 1 << PW_BATTLESUIT;
    } else if ( cg_showSpawns.integer == 4 ) {
		cent->currentState.powerups = 1 << PW_REGEN;
	}
