...
void CG_BuildSolidList( void ) {
	...
	for ( i = 0 ; i < snap->numEntities ; i++ ) {
		...
		if ( ent->eType == ET_ITEM || ent->eType == ET_PUSH_TRIGGER || ent->eType == ET_TELEPORT_TRIGGER  || ent->eType == ET_PHYSICS_TRIGGER ) {
...
static void CG_AddWeapon( int weapon, int quantity, qbool dropped )
{
	// dropped items and teamplay weapons always have full ammo
	if ( !dropped && cgs.gametype != GT_TEAMRUN ) {
...
static void CG_PickupPrediction( centity_t *cent, const gitem_t *item ) {
	...
	// key Item Prediction
	if ( item->giType == IT_KEY && cent->currentState.time2 > 0 ) {
		//
		return;
	}
...
static void CG_TouchItem( centity_t *cent ) {
	...
	// SLK check item availability
    if (cent->currentState.eType == ET_ITEM )
    {
        if ( !IsItemAvailable( &cent->currentState, cg.predictedPlayerState.clientNum ) )
        {
            return;
        }
        else
        {
            // prevent multiple predicted pickups
            SetItemAvailable ( &cent->currentState, cg.predictedPlayerState.clientNum, qfalse );
        }
    }
...
static void CG_TouchTriggerPrediction( void ) {
	...
	for ( i = 0 ; i < cg_numTriggerEntities ; i++ ) {
        ...
        } else if ( ent->eType == ET_PHYSICS_TRIGGER ) {
			BG_TouchPhysicsTrigger( &cg.predictedPlayerState, ent );
		}
	}
...
static int CG_IsUnacceptableError( playerState_t *ps, playerState_t *pps, qbool *forceMove ) {
	vec3_t delta;
	int i, n, v0, v1;
...
void CG_PredictPlayerState( void ) {
	...
	if ( !(cg_pmove.ps->pm_flags & PMF_GRAPPLE_PULL) )
    {
        cg.chainLength = -1;
    }
