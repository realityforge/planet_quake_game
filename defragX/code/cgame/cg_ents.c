...
static void CG_Item( centity_t *cent ) {
	...
	if ( !cg_alwaysDrawItems.integer && !IsItemAvailable( es, cg.predictedPlayerState.clientNum ) )
		return;
	...
	if ( cg_simpleItems.integer && item->giType != IT_TEAM ) {
		...
		trap_R_AddRefEntityToScene(&ent);
		if ( !IsItemAvailable( es, cg.predictedPlayerState.clientNum ) ) // SLK: ugly, move upper inside condition, also the trap_r_add...
        {
			ent.customShader = cgs.media.unavailableItem;
			trap_R_AddRefEntityToScene(&ent);
		}
		return;
	}
	...
	if ( ( !cg_alwaysDrawItems.integer )&& msec >= 0 && msec < ITEM_SCALEUP_TIME )
    {
	...
	// pickup color from spectator/own client
    ...
    if ( !IsItemAvailable( es, cg.predictedPlayerState.clientNum ) )
...
static void CG_Missile( centity_t *cent ) {
    ...
    col = 1; // SLK experimental color stuff
	...
	// add missile sound
	// SLK cvar here for muting other clients missiles?
...
static void CG_Grapple( centity_t *cent )
{
	...
	s1 = &cent->currentState;
	shooter = &cg_entities[s1->otherEntityNum];

	if ( shooter->currentState.clientNum == cg.predictedPlayerState.clientNum ) {
		cg.chainLength = s1->angles2[0];
	}
	...
	// play the chain sound if moving
	if ( weapon->missileSound ) {
		vec3_t chainVec;

		VectorSubtract( s1->pos.trBase, shooter->lerpOrigin, chainVec );
		VectorNormalize( chainVec );
		if ( fabs(DotProduct( shooter->currentState.pos.trDelta, chainVec )) > 200 ) {
			trap_S_AddLoopingSound( cent->currentState.number, shooter->lerpOrigin, vec3_origin, weapon->missileSound );
		}
	}
