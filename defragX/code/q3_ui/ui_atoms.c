...
qbool UI_ConsoleCommand( int realTime ) {
	...
	if ( Q_stricmp (cmd, "silent") == 0 ) {
		static char buf[MAX_STRING_CHARS];
		int old_noprint, i, argc;

		trap_Cvar_VariableStringBuffer( "cl_noprint", buf, sizeof(buf) );
		old_noprint = atoi( buf );

		argc = trap_Argc();
		buf[0] = '\0';
		for ( i = 1; i < argc; i++ ) {
			Q_strcat( buf, sizeof(buf), UI_Argv(i) );
			Q_strcat( buf, sizeof(buf), " " );
		}
		Q_strcat( buf, sizeof(buf), "\n" );

        trap_Cmd_ExecuteText( EXEC_INSERT, va("set cl_noprint 1\n%s\nset cl_noprint %d\n", buf, old_noprint) );
		return qtrue;
	}

	if ( Q_stricmp (cmd, "q") == 0 ) {
