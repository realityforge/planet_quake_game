...
#define ART_REFRESH0		"gfx/ui/refresh_0"
#define ART_REFRESH1		"gfx/ui/refresh_1"
#define ART_CONNECT0		"gfx/ui/play_0"
#define ART_CONNECT1		"gfx/ui/play_1"
#define ART_ARROWS0			"gfx/ui/arrows_vert_0"
#define ART_ARROWS_UP		"gfx/ui/arrows_vert_top"
#define ART_ARROWS_DOWN		"gfx/ui/arrows_vert_bot"
...
static char quake3worldMessage[] = "Visit ws.q3df.org - Maps, Models, Tutorials...";
...
static void ArenaServers_UpdateList( void )
{
	...
	for( i = 0, j = 0; i < count; i++, servernodeptr++ ) {
		...
		case GAMES_FFA:
			if( servernodeptr->gametype != GT_FASTCAP ) {
				continue;
			}
			break;

		case GAMES_TEAMPLAY:
			if( servernodeptr->gametype != GT_TEAMRUN ) {
				continue;
			}
			break;

		case GAMES_TOURNEY:
			if( servernodeptr->gametype != GT_ADVENTURE ) {
				continue;
			}
			break;

		case GAMES_CTF:
			if ( servernodeptr->gametype != GT_FREESTYLE ) {
				continue;
			}
			break;
		}
...
static qbool UI_CleanStr( char *dst, int size, const char *src )
{
	char	*max = dst + size - 1;
	int		c;
	qbool filtered = qfalse;
...
void ArenaServers_LoadFavorites( void )
{
	...
	// no results were found, reset server list, list will be automatically refreshed when selected
	if (!found)
		g_numfavoriteservers = 0;
...
static void ArenaServers_StopRefresh( void ) {
	if (!g_arenaservers.refreshservers) // not currently refreshing
		return;
...
{
	int		i, j, time, maxPing;
	char	adrstr[MAX_ADDRESSLENGTH], info[MAX_INFO_STRING];
...
static void ArenaServers_MenuInit( void ) {
	...
	g_arenaservers.frame.generic.type	= MTYPE_BITMAP;
	g_arenaservers.frame.generic.flags	= QMF_INACTIVE;
	g_arenaservers.frame.generic.name	= ART_FRAME;
	g_arenaservers.frame.width			= 640;
	g_arenaservers.frame.height			= 480;
	g_arenaservers.frame.generic.x		= 0;
	g_arenaservers.frame.generic.y		= 0;
	...
	Menu_AddItem( &g_arenaservers.menu, (void*) &g_arenaservers.frame );
