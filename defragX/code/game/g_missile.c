...
void G_MissileImpact( gentity_t *ent, trace_t *trace )
{
	...
	if (!strcmp(ent->classname, "hook"))
    {
        ...
        // make sure the hook is facing the wall
		VectorNegate( trace->plane.normal, normal );
		vectoangles( normal, ent->s.angles );
...
void G_RunMissile( gentity_t *ent )
{
   ...
   if ( client && client->respawnTime > ent->firedTime ) {
        G_FreeEntity( ent );
        return;
    }
	...
	//don't hit other players when interference is off and missile was shot by a client
	oldclipmask = ent->clipmask;
	if ( !dfx_interference.integer && g_entities[ent->r.ownerNum].client != NULL  )
    {
		ent->clipmask &= ~CONTENTS_BODY & ~CONTENTS_CORPSE;
	}

	// trace a line from the previous position to the current position
	trap_Trace( &tr, ent->r.currentOrigin, ent->r.mins, ent->r.maxs, origin, passent, ent->clipmask );

	ent->clipmask = oldclipmask;
	...
	if ( tr.fraction != 1 )
    {
		...
		// SLK: check for GT_ADVENTURE and SURF_GRAPPLE here
		if ( tr.surfaceFlags & SURF_NOIMPACT || ( (dfx_gametype.integer == GT_ADVENTURE && !(tr.surfaceFlags & SURF_WOOD) ) && (ent->parent && ent->parent->client && ent->parent->client->hook == ent) ) )
		{
...
gentity_t *fire_rocket (gentity_t *self, vec3_t start, vec3_t dir) {
	...
	VectorCopy( start, bolt->s.pos.trBase );
	SnapVector( bolt->s.pos.trBase );			// save net bandwidth
	VectorScale( dir, pm_rocketspeed, bolt->s.pos.trDelta );
	SnapVector( bolt->s.pos.trDelta );			// save net bandwidth
	VectorCopy (start, bolt->r.currentOrigin);
...
gentity_t *fire_grapple (gentity_t *self, vec3_t start, vec3_t dir) {
	...
	hook = G_Spawn();
	hook->classname = "hook";
	hook->nextthink = level.time + GRAPPLE_LENGTH;
	...
	VectorScale( dir, 800, hook->s.pos.trDelta );   // grapple speed
	SnapVector( hook->s.pos.trDelta );			    // save net bandwidth
	VectorCopy (start, hook->r.currentOrigin);

	self->client->hook = hook;
	hook->firedTime = level.time;
