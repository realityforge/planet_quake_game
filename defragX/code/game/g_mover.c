...
gentity_t	*G_TestEntityPosition( gentity_t *ent ) {
	...
	if ( ent->client ) {
        if ( !dfx_interference.integer ) { //SLK Fix movers killing you when interference is off and you touch another player
			mask &= ~CONTENTS_BODY;
		}
...
==================
*/
qbool G_CheckProxMinePosition( gentity_t *check ) {
	vec3_t		start, end;
	trace_t	tr;
...
==================
*/
qbool G_TryPushingProxMine( gentity_t *check, gentity_t *pusher, vec3_t move, vec3_t amove ) {
...
qbool G_MoverPush( gentity_t *pusher, vec3_t move, vec3_t amove, gentity_t **obstacle ) {
	...
	*obstacle = NULL;
	...
	for ( e = 0 ; e < listedEntities ; e++ ) {
		...
		// hook always moves
		if ( check->s.eType == ET_GRAPPLE && check->enemy == pusher )
        {
			G_TryPushingEntity( check, pusher, move, amove );
			continue;
		}
...
void Reached_BinaryMover( gentity_t *ent ) {
	...
	if ( ent->moverState == MOVER_1TO2 ) {
		...
		if ( ent->wait != -2000)
			ent->nextthink = level.time + ent->wait; // SLK keep it open if wait -2 until it gets triggered again
...
void Use_BinaryMover( gentity_t *ent, gentity_t *other, gentity_t *activator )
{
	...
	ent->activator = activator;
	 ...
	 // SLK: check for locked door and if have a key
    if ( ent->needKey && !ent->keyUsed ) {
        hasKey = activator->client->ps.persistant[PERS_KEYS];
        if ( hasKey ) {
            ent->keyUsed = qtrue;
            activator->client->ps.persistant[PERS_KEYS] -= 1;
            // SLK add an event here for unlock
        } else {
            // SLK add an event here when need the key
            return;
        }
    }
...
static void Touch_DoorTriggerSpectator( gentity_t *ent, gentity_t *other, trace_t *trace )
{
	...
	TeleportPlayer( other, origin, NULL, qfalse );
}
...
void SP_func_door (gentity_t *ent)
{
    ...
    // SLK: need holdable key to open the first time
...
/*QUAKED func_door_rotating (0 .5 .8) START_OPEN CRUSHER REVERSE TOGGLE X_AXIS Y_AXIS LOCKED
...
void SP_func_plat (gentity_t *ent)
{
	...
	startsound = endsound = NULL;
	G_SpawnString("startsound", "sound/movers/plats/pt1_strt.wav", &startsound);
	G_SpawnString("endsound", "sound/movers/plats/pt1_end.wav", &endsound);
...
void SP_func_button( gentity_t *ent )
{
    ...
    G_SpawnString("noise", "sound/movers/switches/butn2.wav", &sound);
...
void Reached_Train( gentity_t *ent ) {
	...
	if ( next->speed )
		speed = next->speed;    // if the path_corner has a speed, use that
	else
...
void Use_Rotating (gentity_t *ent, gentity_t *other, gentity_t *activator) {
	...
	if ( ent->s.apos.trTime > 0 ) {
		...
		ent->s.loopSound = ent->oldSoundLoop;
	} else {
		ent->oldSoundLoop = ent->s.loopSound;
		ent->s.loopSound = 0;
		if ( level.time == 0 )
			ent->s.apos.trTime = 1;
		else
