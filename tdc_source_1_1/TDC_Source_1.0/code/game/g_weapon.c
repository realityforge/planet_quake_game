...
//
// JPL (jpl@ing.puc.cl) - Added new weapons - ACE Team
...
void Weapon_Bone( gentity_t *ent ) {
	trace_t		tr;
	vec3_t		end;
	gentity_t	*tent;
	gentity_t	*traceEnt;
	...
	// send blood impact
	if ( traceEnt->takedamage && (traceEnt->client || traceEnt->npc) ) {
	...
	G_Sound(ent,CHAN_VOICE,bone_bash_index);
...
///		if ( g_gametype.integer != GT_TEAM ) {
			Bullet_Fire( ent, MACHINEGUN_SPREAD, 25 );
			G_AddEvent(&g_entities[0], EV_EARTHQUAKE, 254);	// MG42 earthquake
...
void NPC_FireWeapon(gentity_t *ent,vec3_t target_pos)
{
	...
	{
		Bullet_Fire(ent,50,20*(1+0.25*npc_skill));
	}
