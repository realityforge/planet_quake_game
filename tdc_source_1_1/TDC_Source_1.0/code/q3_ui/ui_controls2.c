...
static bind_t g_bindings[] = 
{
	...
	{"weapon 1",		"bone",			ID_WEAPON1,		ANIM_WEAPON1,	'1',			-1,		-1, -1},
	{"weapon 2",		"gun",		ID_WEAPON2,		ANIM_WEAPON2,	'2',			-1,		-1, -1},
	{"weapon 3",		"trishotgun",			ID_WEAPON3,		ANIM_WEAPON3,	'3',			-1,		-1, -1},
	{"weapon 4",		"mg42",		ID_WEAPON4,		ANIM_WEAPON4,	'4',			-1,		-1, -1},
	{"weapon 5",		"grenade launcher",	ID_WEAPON5,		ANIM_WEAPON5,	'5',			-1,		-1, -1},
	{"weapon 6",		"rocket launcher",	ID_WEAPON6,		ANIM_WEAPON6,	'6',			-1,		-1, -1},
	{"weapon 7",		"lightning",		ID_WEAPON7,		ANIM_WEAPON7,	'7',			-1,		-1, -1},
	{"weapon 8",		"railgun",			ID_WEAPON8,		ANIM_WEAPON8,	'8',			-1,		-1, -1},
...
static menucommon_s *g_weapons_controls[] = {
	...
	(menucommon_s *)&s_controls.bone,
	(menucommon_s *)&s_controls.gun,
	(menucommon_s *)&s_controls.trishotgun,          
	(menucommon_s *)&s_controls.mg42,
...
static void Controls_UpdateModel( int anim ) {
	...
	s_controls.playerViewangles[YAW] = 180 - 15; //- 30;
	s_controls.playerViewangles[PITCH]= 10;	//ANITA
	...
	case ANIM_LOOKUP:
		s_controls.playerViewangles[PITCH] = -10; //ANITA  -45;
		break;

	case ANIM_LOOKDOWN:
		s_controls.playerViewangles[PITCH] = 40; //ANITA   45;
		break;

	case ANIM_WEAPON1:
		s_controls.playerWeapon = WP_BONE;
		break;

	case ANIM_WEAPON2:
		s_controls.playerWeapon = WP_GUN;
		break;

	case ANIM_WEAPON3:
		s_controls.playerWeapon = WP_TRISHOTGUN;
		break;

	case ANIM_WEAPON4:
		s_controls.playerWeapon = WP_MG42;
		break;
/*
...
static void Controls_DrawKeyBinding( void *self )
{
	...
	if (c)
	{
		UI_FillRect( a->generic.left, a->generic.top, a->generic.right-a->generic.left+1, a->generic.bottom-a->generic.top+1, tdc_listbar_color ); 

		UI_DrawString( x - SMALLCHAR_WIDTH, y, g_bindings[a->generic.id].label, UI_RIGHT|UI_SMALLFONT, tdc_highlight_color );
		UI_DrawString( x + SMALLCHAR_WIDTH, y, name, UI_LEFT|UI_SMALLFONT|UI_PULSE, tdc_highlight_color );

		if (s_controls.waitingforkey)
		{
			UI_DrawChar( x, y, '=', UI_CENTER|UI_BLINK|UI_SMALLFONT, tdc_highlight_color);
		...
		{
			UI_DrawChar( x, y, 13, UI_CENTER|UI_BLINK|UI_SMALLFONT, tdc_highlight_color);
	...
	else
	{
		...
		{
			UI_DrawString( x - SMALLCHAR_WIDTH, y, g_bindings[a->generic.id].label, UI_RIGHT|UI_SMALLFONT, tdc_color_disabled );
			UI_DrawString( x + SMALLCHAR_WIDTH, y, name, UI_LEFT|UI_SMALLFONT, tdc_color_disabled );
		}
		else
		{
			UI_DrawString( x - SMALLCHAR_WIDTH, y, g_bindings[a->generic.id].label, UI_RIGHT|UI_SMALLFONT, color_white); //controls_binding_color );
			UI_DrawString( x + SMALLCHAR_WIDTH, y, name, UI_LEFT|UI_SMALLFONT, color_white ); //controls_binding_color );
		}
	}
}
...
/*ANITA*/
	...
	b = (menubitmap_s*) self;
	UI_DrawPlayer( b->generic.x - 40, b->generic.y + 80, b->width, b->height, &s_controls.playerinfo, uis.realtime/2 );
/*ANITA*/
...
static void Controls_ResetDefaults_Draw( void ) {
	UI_DrawNamedPic( 0 , 0 , 640 , 480 , BACKGROUND );
	UI_DrawProportionalString( SCREEN_WIDTH/2, 356 + PROP_HEIGHT * 0, "WARNING: This will reset all", UI_CENTER|UI_SMALLFONT, tdc_text_color );
	UI_DrawProportionalString( SCREEN_WIDTH/2, 356 + PROP_HEIGHT * 1, "controls to their default values.", UI_CENTER|UI_SMALLFONT, tdc_text_color  );
}
...
static void Controls_MenuInit( void )
{
	...
	s_controls.background.generic.type  = MTYPE_BITMAP;
	s_controls.background.generic.name  = BACKGROUND;
	s_controls.background.generic.flags = QMF_LEFT_JUSTIFY|QMF_INACTIVE;
	s_controls.background.generic.x     = 0;
	s_controls.background.generic.y     = 0;
	s_controls.background.width  	    = 640;
	s_controls.background.height  	    = 480;
...
//	s_controls.framer.height  	    = 334;
	...
	s_controls.looking.style			= UI_SMALLFONT|UI_RIGHT;
	s_controls.looking.color			= tdc_text_color;
	...
	s_controls.movement.style			= UI_SMALLFONT|UI_RIGHT;
	s_controls.movement.color			= tdc_text_color;
	...
	s_controls.weapons.style			= UI_SMALLFONT|UI_RIGHT;
	s_controls.weapons.color			= tdc_text_color;
	...
	s_controls.bone.generic.type	  = MTYPE_ACTION;
	s_controls.bone.generic.flags     = QMF_LEFT_JUSTIFY|QMF_PULSEIFFOCUS|QMF_GRAYED|QMF_HIDDEN;
	s_controls.bone.generic.callback  = Controls_ActionEvent;
	s_controls.bone.generic.ownerdraw = Controls_DrawKeyBinding;
	s_controls.bone.generic.id        = ID_WEAPON1;

	s_controls.gun.generic.type	    = MTYPE_ACTION;
	s_controls.gun.generic.flags     = QMF_LEFT_JUSTIFY|QMF_PULSEIFFOCUS|QMF_GRAYED|QMF_HIDDEN;
	s_controls.gun.generic.callback  = Controls_ActionEvent;
	s_controls.gun.generic.ownerdraw = Controls_DrawKeyBinding;
	s_controls.gun.generic.id        = ID_WEAPON2;

	s_controls.trishotgun.generic.type	     = MTYPE_ACTION;
	s_controls.trishotgun.generic.flags     = QMF_LEFT_JUSTIFY|QMF_PULSEIFFOCUS|QMF_GRAYED|QMF_HIDDEN;
	s_controls.trishotgun.generic.callback  = Controls_ActionEvent;
	s_controls.trishotgun.generic.ownerdraw = Controls_DrawKeyBinding;
	s_controls.trishotgun.generic.id        = ID_WEAPON3;
	
	s_controls.mg42.generic.type	    = MTYPE_ACTION;
	s_controls.mg42.generic.flags     = QMF_LEFT_JUSTIFY|QMF_PULSEIFFOCUS|QMF_GRAYED|QMF_HIDDEN;
	s_controls.mg42.generic.callback  = Controls_ActionEvent;
	s_controls.mg42.generic.ownerdraw = Controls_DrawKeyBinding;
	s_controls.mg42.generic.id        = ID_WEAPON4;
/*
	...
	s_controls.grenadelauncher.generic.id        = ID_WEAPON5;
	...
	s_controls.rocketlauncher.generic.id        = ID_WEAPON6;
	...
	s_controls.lightning.generic.id        = ID_WEAPON7;
	...
	s_controls.railgun.generic.id        = ID_WEAPON8;
...
*/
	...
	s_controls.name.color			= tdc_text_color;



	Menu_AddItem( &s_controls.menu, &s_controls.background );
...
//	Menu_AddItem( &s_controls.menu, &s_controls.zoomview );
	...
	Menu_AddItem( &s_controls.menu, &s_controls.bone );
	Menu_AddItem( &s_controls.menu, &s_controls.gun );
	Menu_AddItem( &s_controls.menu, &s_controls.trishotgun );
	Menu_AddItem( &s_controls.menu, &s_controls.mg42 );
