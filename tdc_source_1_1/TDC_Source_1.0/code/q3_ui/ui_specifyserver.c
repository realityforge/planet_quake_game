...
#define GAMESERVER_UNKNOWNMAP	"menu/art/loading.tga"	// EHL - modified unknownmap tga
...
static void StartServer_MenuInit( void ) {
	...
	s_startserver.back.generic.x		= 12;
	s_startserver.back.generic.y		= 480-PROP_HEIGHT;
	s_startserver.back.string  		    = "back";
	s_startserver.back.color  		    = color_red;
	s_startserver.back.style			= UI_LEFT;
...
static void ServerOptions_MenuInit( qboolean multiplayer ) {
	...
	s_serveroptions.back.generic.x		  = 32;
	s_serveroptions.back.generic.y		  = 480-36;
	s_serveroptions.back.string  		  = "back";
	s_serveroptions.back.color  		  = tdc_text_color;
	s_serveroptions.back.style	          = UI_LEFT;
	...
	for( i = 0; i < (PLAYERGRID_ROWS * PLAYERGRID_COLS); i++, j++) {
		if( j < botSelectInfo.numBots ) { 
			...
			if( BotAlreadySelected( botSelectInfo.botnames[i] ) ) {
				botSelectInfo.picnames[i].color = tdc_text_color;
			}
			else {
...
static void UI_BotSelectMenu_Init( char *bot ) {
	...
	botSelectInfo.back.generic.x		= 32;
	botSelectInfo.back.generic.y		= 480-36;
	botSelectInfo.back.string			= "back";
	botSelectInfo.back.color			= tdc_text_color;
	botSelectInfo.back.style			= UI_LEFT;
