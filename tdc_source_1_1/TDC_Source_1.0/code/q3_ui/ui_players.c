...
tryagain:
	...
	if( pi->weaponModel == 0 ) {
		if( weaponNum == WP_TRISHOTGUN ) {	// JPL
			weaponNum = WP_NONE;
			goto tryagain;
		}
		weaponNum = WP_TRISHOTGUN;
		goto tryagain;
	}

/*	if ( weaponNum == WP_MG42 || weaponNum == WP_GAUNTLET || weaponNum == WP_BFG ) {
...
void UI_DrawPlayer( float x, float y, float w, float h, playerInfo_t *pi, int time ) {
	...
	//
/*	if ( pi->realWeapon == WP_MG42 || pi->realWeapon == WP_GAUNTLET || pi->realWeapon == WP_BFG ) {
		vec3_t	angles;
...
/*ANITA agregue un1 al 100 */

/*	origin[0] -= 100 1;	// + = behind, - = in front
	origin[1] += 100 1;	// + = left, - = right
	origin[2] += 100 1;	// + = above, - = below
*/
	origin[0] =  0;//-= 100;
	origin[1] +=  1000;//1000;//+= 10000;
	origin[2] =  0;//+= 100;

	
	
	trap_R_AddLightToScene( origin, 550, 1.0 , 0.0 ,0.0 ); //500 , 1.0, 1.0, 1.0 ); ANITA

//	origin[0] -= 100;
//	origin[1] -= 100;
//	origin[2] -= 100;
	
//	trap_R_AddLightToScene( origin, -500, 0.0 , 0.0 , 0.0 ); //500, 1.0, 0.0, 0.0 ); ANITA
...
//	Q_strncpyz( modelName, modelSkinName, sizeof( modelName ) );
	Q_strncpyz( modelName, "starman", sizeof( modelName ) );
