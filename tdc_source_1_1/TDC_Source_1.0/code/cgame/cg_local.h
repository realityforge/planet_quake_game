...
#define	DEFAULT_MODEL			"conjunction"
#ifdef MISSIONPACK
#define	DEFAULT_TEAM_MODEL		"james"
#define	DEFAULT_TEAM_HEAD		"*james"
#else
#define	DEFAULT_TEAM_MODEL		"conjunction"
#define	DEFAULT_TEAM_HEAD		"conjunction"
#endif
...
typedef struct centity_s {
	...
	npcEntity_t		ne;				// JPL - for NPCs

	md3Entity_t		md3;			// JPL - for animated MD3s
...
typedef struct weaponInfo_s {
	...
	int				lastFrame;		// JPL - data for hud weapon animation
	lerpFrame_t		lerpWeap;
	int				anim;
	int				animTime;
	animation_t		animations[WP_MAX_ANIMS];
...
typedef struct {
	...
	//EHL
	qboolean	spawnEffectActive;
	int			spawnEffectEndTime;
	qboolean	weaponPresentSwitch;	//Switch when teleport
...
typedef struct {
	...
	// EHL
	qhandle_t ammoIcon;		// word "Ammo" on statur bar
	qhandle_t healthIcon;	// word "Health" on status bar
	qhandle_t weaponPanel;	// stone panel behind weapons when selecting one
	qhandle_t weaponIcon[MAX_WEAPONS];

	sfxHandle_t hulkQuakeSound;	// JPL
...
void CG_SpawnEffect(); //EHL - parameter no longer needed> vec3_t org );
#ifdef MISSIONPACK
