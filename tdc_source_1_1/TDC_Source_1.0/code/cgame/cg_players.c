...
// JPL - force starman
	Q_strncpyz( newInfo.modelName, "starman", sizeof( newInfo.modelName ) );
	Q_strncpyz( newInfo.skinName, "default", sizeof( newInfo.skinName ) );
	Q_strncpyz( newInfo.headModelName, "starman", sizeof( newInfo.modelName ) );
	Q_strncpyz( newInfo.headSkinName, "default", sizeof( newInfo.skinName ) );
// JPL - end
...
// JPL (jpl@ing.puc.cl) - some modifs for animated MD3

static void CG_SetLerpFrameAnimation( animation_t *ai, lerpFrame_t *lf, int newAnimation ) {
	animation_t	*anim;
...
===============
*/
void CG_RunLerpFrame( animation_t *ai, lerpFrame_t *lf, int newAnimation, float speedScale ) {
	int		f,numFrames;
	animation_t	*anim;
	...
	if ( newAnimation != lf->animationNumber || !lf->animation ) {
		CG_SetLerpFrameAnimation( ai, lf, newAnimation );
	}
...
===============
*/
void CG_ClearLerpFrame( animation_t *ai, lerpFrame_t *lf, int animationNumber ) {
	lf->frameTime = lf->oldFrameTime = cg.time;
	CG_SetLerpFrameAnimation( ai, lf, animationNumber );
...
static void CG_PlayerAnimation( centity_t *cent, int *legsOld, int *legs, float *legsBackLerp,
	...
	if ( cent->pe.legs.yawing && ( cent->currentState.legsAnim & ~ANIM_TOGGLEBIT ) == LEGS_IDLE ) {
		CG_RunLerpFrame( &ci->animations[0], &cent->pe.legs, LEGS_TURN, speedScale );
	} else {
		CG_RunLerpFrame( &ci->animations[0], &cent->pe.legs, cent->currentState.legsAnim, speedScale );
	}
	...
	*legsBackLerp = cent->pe.legs.backlerp;
	...
	CG_RunLerpFrame( &ci->animations[0], &cent->pe.torso, cent->currentState.torsoAnim, speedScale );
...
static void CG_PlayerFlag( centity_t *cent, qhandle_t hSkin, refEntity_t *torso ) {
	...
	CG_RunLerpFrame( &ci->animations[0], &cent->pe.flag, flagAnim, 1 );
...
void CG_ResetPlayerEntity( centity_t *cent ) {
	...
	ci=&cgs.clientinfo[cent->currentState.clientNum];
	CG_ClearLerpFrame( &ci->animations[cent->currentState.legsAnim], &cent->pe.legs, cent->currentState.legsAnim );
	CG_ClearLerpFrame( &ci->animations[cent->currentState.torsoAnim], &cent->pe.torso, cent->currentState.torsoAnim );
