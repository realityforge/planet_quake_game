...
//  CG_ClearLerpFrame is not yet used!!
	...
	CG_RunLerpFrame( &ni->animations[0], &cent->ne.body, es->modelindex2, 1.0 );
...
void CG_NPC( centity_t *cent ) {
	...
	if ((npc->npcType==NPC_PILOT || 
		npc->npcType==NPC_SOLDIER1 || 
		npc->npcType==NPC_SOLDIER2)
		&& (cent->currentState.modelindex2 & ~ANIM_TOGGLEBIT)>2) // the first three animations (0-2) are DEATH!
	{
		// add the weapon
		weapon = &cg_weapons[WP_GUN];
