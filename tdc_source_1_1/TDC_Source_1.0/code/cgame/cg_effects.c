...
EHL - modified, it turns the teleport effect on
and sets the finishing time for it.
Also flips weapons (present / not present)
Old code is no longer used.   
==================
*/

#define SP_FX_LENGTH 3000

void CG_SpawnEffect() { //EHL - parameter no longer needed> vec3_t org ) {
	static int lastWeapon = 0;
	int weapon;
	
	cg.spawnEffectActive = qtrue;
	cg.spawnEffectEndTime = cg.time + SP_FX_LENGTH;
	cg.weaponPresentSwitch = !cg.weaponPresentSwitch;

	// remember present weapon and switch to last weapon
	weapon = cg.weaponSelect;
	cg.weaponSelect = lastWeapon;
	lastWeapon = weapon;
...
===============
*/
void CG_SpawnFX(void) {
	float color[4] = {1,1,1,0};

	color[3] = (float)(cg.spawnEffectEndTime - cg.time) / (float)SP_FX_LENGTH;

	if (cg.time > cg.spawnEffectEndTime) {
